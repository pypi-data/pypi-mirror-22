...
def _return_specific_comment_prs(all_prs, filters):
    ...
    if _validate_list_of_dict(all_prs):
        pr_issue = []
        for pull_request in all_prs:
            issue_comments = pull_request['issue'].get_comments()
            review_comments = pull_request['pr'].get_review_comments()
            comments = _return_comment_bodies(issue_comments, review_comments)
            logger.debug("COMMENTS: %s", comments)
            matched_comments = [comment for comment in comments if re.search(filters['comment'], comment)]
            if matched_comments:
                pr_issue.append({'pr':pull_request['pr'], 'issue':pull_request['issue']})
...
def _return_comment_bodies(issue_comments, review_comments):
    """
    Takes paginated lists from issue comments and review comments
    and returns a list of the comment bodies
    """
    comment_bodies = []
    if issue_comments:

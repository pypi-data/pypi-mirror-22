...
class ConfigData(object):
	...
	def __init__(self, script_path, variable_pool):
		...
		for ix, configfile in enumerate(config_files):
			if not configfile in files_read:
				files_read.append(configfile)
...
def exception_info():
	...
	#return sys.exc_info()[0].__name__, unicode(sys.exc_info()[1]), strace[0][0], strace[0][1], strace[0][3]
	traces = traceback.extract_tb(sys.exc_info()[2])
	xline = 0
	for trace in traces:
		if u"execsql" in trace[0]:
			xline = trace[1]
	return sys.exc_info()[0].__name__, sys.exc_info()[1], strace[0][0], xline, strace[0][3]


def exception_desc():
	exc_type, exc_strval, exc_filename, exc_lineno, exc_linetext = exception_info()
	#return u"%s: %s in %s on line %s of execsql." % (unicode(exc_type), unicode(exc_strval).replace(u'\n', u' '), unicode(exc_filename), exc_lineno)
	return u"%s: %s in %s on line %s of execsql." % (exc_type, unicode(repr(exc_strval), errors='replace'), exc_filename, exc_lineno)
...
class DataType(object):
	...
	def __init__(self, data_type_name, data_type_type):
		self.data_type_name = data_type_name
		self.data_type = data_type_type
		self.lenspec = False		# Is a length specification required for a (SQL) declaration of this data type?
		self.varlen = False		# Do we need to know if a set of data values varies in length?
		self.precspec = False		# Do we need to know the precision and scale of the data?
		self.precision = None		# Precision (total number of digits) for numeric values.
		self.scale = None			# Scale (number of digits to the right of the decimal point) for numeric values.
	def __repr__(self):
...
class DT_TimestampTZ(DataType):
	def __init__(self):
		# There is no distinct Python type corresponding to a timestamptz, so the second argument
		# to init() is not appropriate, and methods need to be overridden.
		super(DT_TimestampTZ, self).__init__("timestamptz", datetime.datetime)
	def __repr__(self):
...
class DT_Timestamp(DataType):
	def __init__(self):
		super(DT_Timestamp, self).__init__("timestamp", datetime.datetime)
	def __repr__(self):
...
class DT_Date(DataType):
	def __init__(self):
		super(DT_Date, self).__init__("date", datetime.date)
	def __repr__(self):
		return u"DT_Date()"
	date_fmts = ("%x",
			"%m/%d/%Y",
...
class DT_Time(DataType):
	def __init__(self):
		super(DT_Time, self).__init__("time", datetime.time)
	def __repr__(self):
		return u"DT_Time()"
...
class DT_Boolean(DataType):
	def __init__(self):
		super(DT_Boolean, self).__init__("boolean", bool)
	def __repr__(self):
		return u"DT_Boolean()"
...
class DT_Integer(DataType):
	def __init__(self):
		super(DT_Integer, self).__init__("integer", int)
	def __repr__(self):
		return u"DT_Integer()"
	def _is_match(self, data):
		if type(data) == int:
...
class DT_Long(DataType):
	def __init__(self):
		super(DT_Long, self).__init__("long", long)
	def __repr__(self):
		return u"DT_Long()"
	def _from_data(self, data):
		if type(data) == long:
...
class DT_Float(DataType):
	def __init__(self):
		super(DT_Float, self).__init__("float", float)
	def __repr__(self):
		return u"DT_Float()"
	def _from_data(self, data):
		if type(data) == float:
...
class DT_Decimal(DataType):
	def __init__(self):
		super(DT_Decimal, self).__init__("decimal", Decimal)
		self.precspec = True
	def __repr__(self):
...
class DT_Character(DataType):
	def __init__(self):
		super(DT_Character, self).__init__("character", unicode)
		self.lenspec = True
	def __repr__(self):
		return u"DT_Character()"
	def _is_match(self, data):
...
class DT_Varchar(DataType):
	def __init__(self):
		super(DT_Varchar, self).__init__("varchar", unicode)
		self.lenspec = True
		self.varlen = True
	def __repr__(self):
		return u"DT_Varchar()"
...
class DT_Text(DataType):
	def __init__(self):
		super(DT_Text, self).__init__("character", unicode)
	def __repr__(self):
		return u"DT_Text)"
	def _is_match(self, data):
		if type(data) == bytearray:
...
class DT_Binary(DataType):
	def __init__(self):
		super(DT_Binary, self).__init__("binary", bytearray)
	def __repr__(self):
		return u"DT_Binary)"
...
class Database(object):
	...
	def populate_table(self, schema_name, table_name, rowsource, column_list, tablespec_src):
		...
		for col in column_list:
			if not col in ts_colnames:
				raise ErrInfo(type="error", other_msg="Column %s is not in the data source." % col)
	...
	def import_file(self, schema_name, table_name, csv_file_obj, skipheader):
		...
		f = csv_file_obj.reader()
		f.next()
		self.populate_table(schema_name, table_name, f, colhdrs, get_ts)
...
class AccessDatabase(Database):
	...
	connection_strings = ("DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=%s;",
	...
	def open_dao(self):
		...
		try:
			...
			raise ErrInfo(type="exception", exception_msg=exception_desc(), other_msg=u"Can't open Access database %s using DAO" % self.db_name)
	...
	def populate_table(self, schema_name, table_name, rowsource, column_list, tablespec_src):
		...
		# The tablespec argument is a callback function to produce a DataTable object.
		sq_name = self.schema_qualified_table_name(None, table_name)
		if column_list:
			colsel = ",".join([self.type.quoted(c) for c in column_list])
		else:
			colsel = "*"
		try:
			self.dao_flush_check()
			...
			raise ErrInfo(type="db", command_text=sql, exception_msg=exception_desc(), other_msg=u"Can't determine column names in table %s" % sq_name)
		columns = [self.type.quoted(d[0]) for d in curs.description]
		colspec = ",".join(columns)
		paramspec = ",".join(['?' for c in columns])
		sql = "insert into %s (%s) values (%s);" % (sq_name, colspec, paramspec)
		...
		def load_line(line):
			...
			if not conf.empty_strings:
				...
				for i in range(len(line)):
					if line[i] == u'':
						line[i] = None
			lt = [type_funcs[i](col) if col is not None else None for i, col in enumerate(line)]
			try:
				curs.execute(sql, lt)
			except ErrInfo:
				raise
		...
		#self.commit()
	def import_file(self, schema_name, table_name, csv_file_obj, skipheader):
		# The schema name is ignored.
		# Column names are taken from the table.  Names, order, and types must be
		# the same in the input file.
		sq_name = self.schema_qualified_table_name(None, table_name)
		...
		try:
			self.dao_flush_check()
			curs = self.cursor()
			curs.execute("select top 1 * from %s;" % sq_name)
		except ErrInfo:
			raise
		except:
			raise ErrInfo(type="db", exception_msg=exception_desc(), other_msg=u"Can't determine column names in table %s" % sq_name)
		columns = [self.type.quoted(d[0]) for d in curs.description]
		colspec = ",".join(columns)
		paramspec = ",".join(['?' for c in columns])
		sql = "insert into %s (%s) values (%s);" % (sq_name, colspec, paramspec)
		...
		def load_line(line):
			...
			try:
				...
				#print("Data: %s" % lt)
				#print("SQL: %s" % sql)
				raise ErrInfo(type="db", command_text=sql, exception_msg=exception_desc(), other_msg=u"Import of line {%s} from file into table %s" % (line, sq_name))
		curs = self.cursor()
		csv_file_obj.evaluate_line_format()
		if csv_file_obj.quotechar:
			...
			# Read as a plain text file (as unicode) and split at delimiters.
			f = csv_file_obj.open("rt")
			if skipheader:
				f.next()
			# Read chunks of lines until EOF and send them to the table.
			while True:
				lines = f.readlines()
				if not lines:
					break
				for line in lines:
					while len(line) > 0 and line[-1] in (u"\n", u"\r"):
						line = line[:-1]
					if len(line) > 0:
						if csv_file_obj.delimiter:
							line = line.split(csv_file_obj.delimiter)
						else:
							line = [line]
						load_line(line)
		#self.commit()
...
class SqlServerDatabase(Database):
	...
	def open_db(self):
		if self.conn is None:
			...
			ssdrivers = ('SQL Server Native Client 10.0', 'SQL Native Client', 'SQL Server')
			...
			if not conf.empty_strings:
				...
				for i in range(len(line)):
					if line[i] == u'':
						line[i] = None
			# Convert data types
			tr = zip(line, type_funcs)
			lt = [col[1](col[0]) if col[0] is not None else None for col in tr]
			#l = [col.encode(self.encoding) if isinstance(col, basestring) else col for col in lt]
			try:
				#curs.execute(sql, tuple(l))
...
class PostgresDatabase(Database):
	...
	def import_file(self, schema_name, table_name, csv_file_obj, skipheader):
		...
		def load_line(line):
			...
			line.extend([None]*(len(columns)-len(line)))
			try:
				curs.execute(sql_template, line)
			except ErrInfo:
				raise
			except:
				self.conn.rollback()
		...
		# If there is no quotechar and encodings match, use copy_from.
		# If there is no quotechar and encodings don't match, read lines, split on delimiters, and fix types
		# If there is a quotechar, read as CSV and fix types.
		# Need to fix types if either there is a quotechar or encodings don't match
		if csv_file_obj.quotechar or csv_file_obj.encoding != self.encoding:
			curs = self.cursor()
			try:
				sql = "select * from %s limit 1;" % sq_name
				curs.execute(sql)
			except ErrInfo:
				raise
			except:
				self.conn.rollback()
				raise ErrInfo(type="db", command_text=sql, exception_msg=exception_desc(), other_msg=u"Can't determine column names in table %s" % sq_name)
			data_table_desc = curs.description
			#coltypelist = ", ".join([str(d[1]) for d in data_table_desc])
			#curs.execute("select typelem, typname from pg_type where typelem > 0 and typcategory='A' and typarray=0 and typnamespace=11 and typelem in (%s);" % coltypelist)
			#pg_types = curs.fetchall()
			curs.close()
			columns = [self.type.quoted(d[0]) for d in data_table_desc]
			colspec = ",".join(columns)
			paramspec = ",".join(['%s']*len(columns))
			sql_template = u"insert into %s (%s) values (%s);" % (sq_name, colspec, paramspec)
		if csv_file_obj.quotechar:
			# Read as CSV
			f = csv_file_obj.reader()
			if skipheader:
			...
			# No quotechar; encodings may or may not match
			curs = self.cursor()
			rf = csv_file_obj.open("rt")
			if skipheader:
				rf.next()
			if csv_file_obj.encoding.lower() == self.encoding.lower():
				if not csv_file_obj.delimiter:
					# Copy_from() requires a delimiter, but if there is none, feed it a non-printing
					# character (ASCII unit separator, which, if it had been used for its intended purpose,
					# should have been identified as the delimiter, so presumably it has not been used.)
					try:
						curs.copy_from(rf, sq_name, chr(31), null='')
					except ErrInfo:
						raise
					except:
						self.conn.rollback()
					...
					try:
						curs.copy_from(rf, sq_name, sep=csv_file_obj.delimiter, null='')
					except ErrInfo:
						raise
					except:
						self.conn.rollback()
				...
				# No quotechar; encodings differ.
				# Read chunks of lines until EOF and send them to the table.
				while True:
					global conf
					lines = rf.readlines(conf.import_buffer)
					if not lines:
						break
					for line in lines:
						while len(line) > 0 and line[-1] in (u"\r", u"\n"):
							line = line[:-1]
						if len(line) > 0:
							if csv_file_obj.delimiter:
								line = line.split(csv_file_obj.delimiter)
...
class SQLiteDatabase(Database):
	...
	def populate_table(self, schema_name, table_name, rowsource, column_list, tablespec_src):
		...
		sq_name = self.schema_qualified_table_name(None, table_name)
		if column_list:
			columns = [self.type.quoted(c) for c in column_list]
		else:
			columns = []
			try:
				pragma_cols, pragma_data = self.select_data("pragma table_info(%s);" % sq_name)
				name_index = pragma_cols.index('name')
			except ErrInfo:
				raise
			except:
				raise ErrInfo(type="exception", exception_msg=exception_desc(), other_msg=u"Can't determine column names in table %s" % sq_name)
			for d in pragma_data:
				columns.append(self.type.quoted(d[name_index]))
		colspec = ",".join(columns)
		paramspec = ",".join(['?' for c in columns])
		sql = u"insert into %s (%s) values (%s);" % (sq_name, colspec, paramspec)
		curs = self.cursor()
		for line in rowsource:
			...
			if not (len(line) == 1 and line[0] is None):
				...
				for i in range(len(line)):
					if type(line[i]) in (datetime.datetime, datetime.time, Decimal):
						line[i] = str(line[i])
				try:
	...
	def open_db(self):
		def db_conn():
			if self.user and self.password:
				return mysql_lib.connect(host=self.server_name, database=self.db_name, port=self.port, user=self.user, password=self.password, charset=self.encoding)
			else:
				return mysql_lib.connect(host=self.server_name, database=self.db_name, port=self.port, charset=self.encoding)
		if self.conn is None:
			try:
				if self.user and self.need_passwd and not self.password:
...
class CsvFile(EncodedFile):
	...
	def __repr__(self):
		return u"CsvFile(%r, %r)" % (super(CsvFile, self).filename, super(CsvFile, self).encoding)
			...
			if self.quotechar:
				...
				for e in datarow:
					if isinstance(e, basestring):
						if (self.quotechar in e) or (self.delimiter and self.delimiter in e):
	...
	def evaluate_column_types(self):
		...
		#for n in colnames:
		#	self.cols.append(Column(n))
		#self.filerows = 1	# Header is row 1.
		#for datarow in inf:
		#	self.filerows += 1
		#	if len(datarow) > 0:
		#		self.datarows += 1
		#		chkcols = len(self.cols)
		#		if len(datarow) < chkcols:
		#			self.shortrows += 1
		#			chkcols = len(datarow)
		#		else:
		#			if len(datarow) > chkcols:
...
class MetaCommand(object):
	def __init__(self, matching_regexes, exec_func, name=None, description=None, run_in_batch=False, run_when_false=False):
	...
	def run(self, cmd_str):
		...
		for rx in self.regexes:
			...
			if m:
				...
				except (SystemExit, ErrInfo):
					raise
				except:
					raise ErrInfo("cmd", command_text=cmd_str, exception_msg=exception_desc())
				return True, rv
		return False, None
	def __str__(self):
		if self.name:
...
metacommands.append(MetaCommand((
	r'^CONNECT\s+TO\s+MYSQL\s*\(\s*SERVER\s*=\s*(?P<server>[A-Z0-9][A-Z0-9_\-\.]*)\s*,\s*DB\s*=\s*(?P<db_name>[A-Z][A-Z0-9_\-]*)(?:\s*,\s*USER\s*=\s*(?P<user>[A-Z][A-Z0-9_]*)\s*,\s*NEED_PWD\s*=\s*(?P<need_pwd>TRUE|FALSE))?(?:\s*,\s*PORT\s*=\s*(?P<port>\d+))?(?:\s*,\s*ENCODING\s*=\s*(?P<encoding>[A-Z][A-Z0-9_-]+))?\s*\)\s+AS\s+(?P<db_alias>[A-Z][A-Z0-9_]*)\s*$',
	r'^CONNECT\s+TO\s+MARIADB\s*\(\s*SERVER\s*=\s*(?P<server>[A-Z0-9][A-Z0-9_\-\.]*)\s*,\s*DB\s*=\s*(?P<db_name>[A-Z][A-Z0-9_\-]*)(?:\s*,\s*USER\s*=\s*(?P<user>[A-Z][A-Z0-9_]*)\s*,\s*NEED_PWD\s*=\s*(?P<need_pwd>TRUE|FALSE))?(?:\s*,\s*PORT\s*=\s*(?P<port>\d+))?(?:\s*,\s*ENCODING\s*=\s*(?P<encoding>[A-Z][A-Z0-9_-]+))?\s*\)\s+AS\s+(?P<db_alias>[A-Z][A-Z0-9_]*)\s*$',
	), x_connect_mysql))
...
def gui_connect(alias, message, cmd=None):
	...
	if exit_status == 1:
		try:
			...
			if cmd:
				raise ErrInfo(type="cmd", command_text=kwargs["metacommandline"], other_msg=u"Could not connect to %s" % db)
			else:
...
def main():
	...
	if conf.server is None and conf.db is None and conf.db_file is None:
		...
		if conf.db_type == "a":
			...
			db = db_Access(conf.db_file, pw_needed=conf.passwd_prompt, user=conf.access_username, encoding=conf.db_encoding)
		# Use Postgres
		elif conf.db_type == "p":

...
class constrained_epoch(argparse.Action):
        def __call__(self, parser, namespace, values, option_string=None):
                ...
                user_epoch = datetime.utcfromtimestamp(git.committime('HEAD')) - td(**units)

                # We cannot go before the beginning of the commit history.
                git_epoch = git.committime(git.initialcommit())
                timestamp = max(time.mktime(user_epoch.timetuple()), git_epoch)
...
parser.add_argument("-e", "--every", type=str, default='week', action=select_time_iterator, choices=timeunits, help="Length of time between snapshots.")

'''
  ...
  ``Server`` is responsible for high-level dispatching, *GET* and *POST*,
...
if sys.version_info >= (3, 0):
    ...
    bytestr    = str
    unicodestr = unicode  # @UndefinedVariable
...
class Request(object):
  ...
  def serialize(self, obj):
    '''Serialisation for objects that not handled by ``json`` package'''
    ...
    raise TypeError(u'{0!r} is not JSON serialisable'.format(obj))
...
class Server(object):
  ...
  def __init__(self, requestClass = Request, locatorClass = ServiceLocator):
    '''Creates a RPC server:
       ...
       requestClass: class of request object, provides override for custom serialisation
  ...
  def _get(self, request):
    ...
    scriptTransportId   = cherrypy.request.params.get('_ScriptTransport_id', None)
    scriptTransportData = cherrypy.request.params.get('_ScriptTransport_data', None)
    ...
    if scriptTransportId and scriptTransportData:
      ...
      if cherrypy.response.headers.get('content-type') == 'application/json':
        ...
        response = b''.join(('{0}({1},'.format(
          callback, scriptTransportId).encode(), response, b');'))
    else:
  ...
  def _post(self, request):
    ...
    elif all(k in cherrypy.request.params
        for k in ('_ScriptTransport_data', '_ScriptTransport_id')):

'''
  ...
  There are several supposed ways to launch the test suite:
    ...
    3) ``unittest`` direct module invocation

  Because Python warning settings vary across these three, some normalisation is required to
...
# To do simple test skip CherryPy < 10 uses ``nose`` and >= 10 uses ``pytest``.
# Emulate for standard runner. However skip() is not used anyway.
for n in ('nose', 'pytest'):
  try:
    __import__(n)
  except ImportError:
...
if sys.version_info >= (3, 0):
  ...
  warnings.filterwarnings('ignore', 'unclosed file <_io.', ResourceWarning)
  # cherrypy._cperror: reported #1330
  warnings.filterwarnings('ignore', 'cgi.escape is deprecated, use html.escape instead',
    DeprecationWarning)
...
class TestCase(helper.CPWebCase):
  ...
  def setUp(self):
    self.__class__.setup_server = classmethod(
      lambda cls: cherrypy.tree.mount(Root(), config = config))
    # nose setup
    self.setup_class()

...
class TestGeneral(test.TestCase):
  ...
  def testServiceMethodNotFound(self):
    ...
    response = json.loads(self.body.decode('ascii'))
    self.assertTrue(response['result'] is None)
    service = r'<qxcpjsonrpc.test.Service object at 0x[\w]+>'
  ...
  def testServiceGet(self):
    ...
    pattern = re.escape('qx.io.remote.transport.Script._requestFinished(1,sub);') \
      .replace('sub', '(.+)')
    self.assertEqual(
      {'error': None, 'id': 1, 'result': 25},
      json.loads(re.findall(pattern, self.body.decode())[0]))
  ...
  def testRequestJsonUnserializationHttpError(self):
    ...
    self.assertHeader('content-type', 'text/html;charset=utf-8')
    self.assertTrue(
      b'Error decoding JSON request: No JSON object could be decoded' in self.body or
      # Python 3.4+
      b'Error decoding JSON request: Expecting value: line 1 column 1 (char 0)' in self.body
    )
  ...
  def testResponseJsonSerializationServerError(self):
    ...
    self.assertEqual({
      'code'    : error.ErrorCode.Unknown,
      'origin'  : error.ErrorOrigin.Server,
      'message' : '[ServerError] Error JSON encoding response: (1-2j) is not JSON serialisable'
    },response['error'])

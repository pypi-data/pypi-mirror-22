...
def getRigolMSO2302A(addr=CLIENT_SETTINGS['RIGOL_MSO2302A_ADDRESS']):
...
def post_logfile(test_run_id=""):
    logfile = os.path.join(DIRPATH, CLIENT_SETTINGS['DEFAULT_LOGFILE'])
    zlogfile = logfile + '.gz'
    if not os.path.isfile(logfile):
        logger.warning("Missing logfile!")
        return
    with open(logfile) as f_in, gzip.open(zlogfile, 'wb') as f_out:
    ...
    logger.debug("post_logfile response.reason: %s" % response.reason)
    logger.debug("post_logfile response.status_code: %s"

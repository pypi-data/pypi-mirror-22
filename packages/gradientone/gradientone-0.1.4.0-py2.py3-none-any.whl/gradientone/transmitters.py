...
class CANOpenTransmitter(Transmitter):
    ...
    def format_csv(self, datafile=None, filename='tempfile.csv'):
        if not datafile:
            datafile = os.path.join(TMPDIR, filename)
        fileblob = open(datafile, 'w')
    ...
    def transmit_result_blob(self, datafile='', filename='tempfile.csv'):
        """Creates CANOpen result file and transmits

        The 'infile' is ignored since it's generated here
        """
        if not datafile:
            datafile = os.path.join(TMPDIR, filename)
        self.format_csv(datafile=datafile)
        super(CANOpenTransmitter, self).transmit_result_blob(datafile)
    ...
    def transmit_config(self):
        ...
        payload = {
            ...
            'config_data': {
                ...
                'hybrid_config': self.trace_dict['raw_setup'],
                'instrument_type': self.trace_dict['instrument_type'],
                'company_nickname': COMMON_SETTINGS['COMPANYNAME'],
            }
        }
        create_config_url = "https://" + \
            COMMON_SETTINGS['DOMAIN'] + "/create_config"
        _response = self.post(

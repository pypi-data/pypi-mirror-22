...
class Api:
    ...
    def __init__(self, persist_file=DEFAULT_CACHE_FILE, flush=False, log_file=None, sleep=0, timeout=5, **config):
        self.log_file = stderr if log_file is None else open(log_file, 'a+')
    ...
    def req(self, method='get', url='/', params=None, data=None, headers=None, **kwargs):
        """
    ...
    def flush(self):
        if self.user_id is 0:
            return
        data = self.req('get', '/user/%s/profile' % self.user_id).get('data', {})
        user = data.get('user', {})
        self.user_id = user.get('id', 0)
        self.profile = user
        self.persist()

...
__AUTHOR__ = 'Mohamed Cherkaoui <chermed@gmail.com>'
__WEBSITE__ = 'http://mohamedcherkaoui.com'
__DATE__ = '2017-06-03'
...
home = expanduser("~")
CONFIG_FILE = os.path.join(home, 'oo.ini')
...
@click.argument('section', type=click.STRING, required=True)
@click.pass_context
def create_section(ctx, section):
...
@click.argument('section', type=click.STRING, required=True)
@click.pass_context
def update_section(ctx, section):
    """Update a section"""
    config = ctx.obj['config_obj']
    config_path = ctx.obj['config_path']
...
@click.pass_context
def delete_section(ctx, section):
    """Delete a section"""
    config = ctx.obj['config_obj']
    config_path = ctx.obj['config_path']
...
@click.option('--update-list', is_flag=True, default=False, type=click.BOOL)
@click.pass_context
def update(ctx, modules, module, update_list):
    """Upgrade modules"""
    odoo = ctx.obj['action_login']()
    if update_list:
        ctx.obj['update_list']()
...
def refs(ctx, model, params):
    ...
    for action in Action.browse(action_ids):
        ...
        if not menu_ids:
            menu_data.append({'name': '', 'id': '', 'xml_id': ''})
        first_line = True

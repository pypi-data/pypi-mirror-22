...
import sys, os, json, logging, datetime, uuid
import requests
...
class RedTenClient(object):
    ...
    def __init__(self, name="rt", user="", password="", url=""):
        ...
        try:
            ...
            print "Failed to login with user=" + str(self.rt_user) + " url=" + str(self.rt_user) + " with exception=" + str(e)
            self.last_login = {}
            self.rt_user_id = 0
            self.user_token = ""
        # end of try/ex
    ...
    def rest_full_login(self, org_username="", org_password="", api_url="", debug=False):
        ...
        if org_username != "":
            username = strorg_username().strip().lstrip()
        if org_password != "":
            ...
            if post_response.status_code != 201 and post_response.status_code != 200:
                ...
                if not retry:
                    if "Signature has expired." in str(post_response.text):
                        ...
                        self.run_job(query_params=query_params, post_data=post_data, headers=auth_headers, user_id=user_id, retry=True, debug=debug)
                    else:

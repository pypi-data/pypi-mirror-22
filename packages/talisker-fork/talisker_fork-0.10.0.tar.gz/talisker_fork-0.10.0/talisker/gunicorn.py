...
from . import logs, statsd, util, wsgi
...
gunicorn.http.wsgi.Response.should_close = _should_close
...
def run():
  ...
  app = TaliskerApplication("%(prog)s [OPTIONS] [APP_MODULE]", config['devel'], config['debuglog'])
  return app.run()

...
def tokenize(text):
    tokens = re.split(REGEX_TOKENIZER, text.lower())
    return (VOCAB[token] for token in tokens if token)
...
def evaluate(text):
    ...
    # evaluate generator here because we want to raise the exception
...
def evaluate_lenient(text, greedy=False):
    tokens = re.split(REGEX_TOKENIZER, text.lower())
    results = []

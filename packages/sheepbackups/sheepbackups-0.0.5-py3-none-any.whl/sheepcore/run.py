...
from sheepcore.utils import retrieve_backup, checklist, Checklist

def run_ping(raw=False):
    # Ping api with config, to see if either api is down, or config is invalid
    r_any = api_request('ping_any')
    r_auth = api_request('ping_auth')
    
    if raw:
        return [r_any, r_auth]
    else:
        return 'uh idk good probs'
...
def entries_lookup(prefix=None, match=None, limit=10, dataset=None):
    ...
    for result in j['entries']:
        ...
        num += 1
    return tabulate(table, headers=['', 'uuid', 'prefix', 'date', 'meta'])
...
def run_restore(idmatch=None):
    ...
    if not CONFIG.includes(entry['prefix']):
        return "ERROR: The specified backup type ({1}) was not found in the config file ({0}). This should be under the `backups` entry.\n=> Unable to perform restore.\nHelp: https://docs.sheepbackups.com/etc".format(CONFIG.path, entry['prefix'])
    ...
    output = restore.run()
    # Ask confirmation
    # Y = proceed, ignoring errors, n or anything else = stop
    
    # Upload restore log
    restore_log = os.path.join('/tmp', 'sheepbackups', 'restore.log')
    postdata = create_api_payload('restore', prefix=entry['prefix'], meta={'more':1}, log=restore_log)
    resp = api_request('new_restore', post=json.dumps(postdata))
...
def loader(testopt=False):
    ...
    parser.add_argument('action', nargs='?', help='What action to perform (backup, restore, list, check, ping)')
    ...
    if opts.action == 'backup':
        ...
        return entries_lookup(opts.prefix, opts.match, opts.limit)
    elif opts.action == 'config':
        return CONFIG.show_info()
    elif opts.action == 'check':
        return run_check()
    elif opts.action == 'ping':
        return run_ping()
    elif opts.version:
        return sheep_version
        
    # Used in test suite/low level usage
    elif testopt is not False:
        if testopt == 'opts':
            return opts
        if testopt == 'config':
            return CONFIG

...
def _check_for_updates():
    """Check if installed version is older than the latest PyPI release."""
    try:
        ...
        if current < latest:
            ...
            click.secho(template.format(current, latest), err=True,
                        fg='yellow')
    except:
        pass
...
def _server_by_name(project_id, server_name):
    """Resolve a project ID and server name to a server ID."""
    client = sml.galleon.Galleon()
    matching_servers = client.get_servers(project_id, server_name)
    if len(matching_servers) == 1:
        return matching_servers[0]
    else:
        if not matching_servers:
            tpl = 'no available server of name "{}"'
        else:
            tpl = ('more than one server of name "{}", please select by '
                   'server ID instead')
        raise AmbiguousNameError(tpl.format(server_name))


def _resolve_server(project, server=None):
    """Resolve project and server names to project and server IDs."""
    project_id = _resolve_project(project)
    try:
        server_id = uuid.UUID(server)
    except ValueError:
        server_id = _server_by_name(project_id, server).id_
    except TypeError:
        server_id = _running_server(project_id)
...
def _running_server(project_id):
    """Get a running server from project.

    If multiple servers running, any one will be returned.
    """
    client = sml.galleon.Galleon()
    servers_ = client.get_servers(project_id, status='running')

    if not servers_:
        _print_and_exit('No server running in project.', 78)
...
def list_servers(project, all, verbose):
    ...
    if verbose:
        if not servers_:
            ...
            click.echo(tabulate(
                [(s.name, s.status, s.id_) for s in servers_],
                ('Server Name', 'Status', 'ID'),
                tablefmt='plain'
            ))
    else:
...
def new(project, cores, memory, type_, name):
    """Create a new SherlockML server."""
    # pylint: disable=too-many-arguments
    _check_credentials()
    project_id = _resolve_project(project)
    client = sml.galleon.Galleon()
    server_id = client.create_server(project_id, type_, cores * 1000, memory,
                                     name)
...
def shell(project, server, ssh_opts):
    ...
    with _save_key_to_file(key) as filename:
        cmd = [
            ...
            'StrictHostKeyChecking=no', '-p', str(port), '-i', filename,
...
@click.argument('project')
@click.argument('server')
@click.option('--step', '-s', type=int,
              help='Display only the logs for this step')
def logs(project, server, step):
    ...
    if step is None:
        ...
        try:
            ...
            _print_and_exit('step {} out of range'.format(step), 64)

    for s in steps:
        for line in client.stream_environment_execution_step_logs(s):
            click.echo(line)
...
def put(project, local, remote, server):
    ...
    with _save_key_to_file(key) as filename:
        subprocess.call([
            'scp', '-o', 'IdentitiesOnly=yes', '-o',
            'StrictHostKeyChecking=no', '-i', filename, '-P', str(port),
            os.path.expanduser(local),
            '{}@{}:{}'.format(username, hostname, remote)
        ])
...
def _rsync(project, local, remote, server, rsync_opts, up):
    ...
    if up:
        path_from = local
        path_to = '{}@{}:{}'.format(username, hostname, remote)
    else:
        path_from = '{}@{}:{}'.format(username, hostname, remote)
        path_to = local

    with _save_key_to_file(key) as filename:
        ssh_cmd = ('ssh -o IdentitiesOnly=yes -o StrictHostKeyChecking=no '
                   '-p {} -i {}').format(port, filename)

...
def validate_host_volumes(paths):
    """
    Check the paths of the host volume directories exist.
    ...
    for path in paths:
        ...
        if not (os.path.isfile(full_path) or os.path.isdir(full_path)):
            msg = "Given path path does not exist: {}"
            raise biobox.exception.BioboxInputVolumeNotFound(msg.format(full_path))

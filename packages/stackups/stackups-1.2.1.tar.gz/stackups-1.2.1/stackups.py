...
class Stacks():
    ...
    def append(self, stack=[], title='use default_Stack_title' ):        
        '''Append a Stack to other Stack objects.  (Note that the functionality
        of this append method of Stacks is different to that of Stack.  See
        Stack.append for more info.)
        ...
        for  b2 in b:
            if 'pn' in b2:
                ...
                if suold:
                    b2['su'] = suold
                    if (not math.isclose(b2['d'], sudic[suold]['d'], rel_tol=1e-6) or
                              not math.isclose(b2['dt'], sudic[suold]['dt'], rel_tol=1e-6) or
                              not math.isclose(b2['sigma'], sudic[suold]['sigma'], rel_tol=1e-3)):
    ...
    def contents(self):
        '''Show a list of titles for all the stacks, i.e., a table of contents.
    ...
    def help(self):
        ...
        print()
        print('  Stacks methods: a (or append), e (or extend), i (insert)')
        print('  o (search), u (update), contents, help, su, move (m), pn')
        print('  pop, renamepn, renumber, save, search, sync, su, title')
        print('  , wu (whereused')
        print()
        ...
        print()
        print('  Also see the Stack class (child of Stacks) which has its')
        print('  methods')
    ...
    def save(self, filename=None, pagebreak=True, lines_per_page=58):
        ...
        '''
        if filename:        
           savestk(self, filename, pagebreak, lines_per_page)
        else:
           savestk(self, self.filename, pagebreak=True, lines_per_page=58) 
...
class Stack(Stacks):
    ...
    def help(self):        
        '''List of methods belonging to Stack:'''
        print()
        print('  Stack methods: a (or append), e (or extend), i (insert)')
        print('  invert, help, j, pn, pop, sync, title')
        print()
        print('  To obtain help on, for example, the append method:')
        print()
        print('  >>> help(Stack.append)')
        print()
        print('  Note that class is a child of Stacks, and thus methods of')
        print('  that class are inherited by this one.')
    ...
    def j(self, *args):
        ...
        >>> stk.j{{5:-7, 6:-8}}  # update rows 7 and 8 with multiple Stackunits
        '''
        for x in args:
...
class Stackunit(Stacks):
    ...
    {'su': 5, 'sigma': 6, 'd': 3.3400, 'dt': 0.0100, 'pn': 'MTI012346', 'n': 'groove to end'}
    
    Where:
    
    - su : sunt
        An integer used to identify a Stackkunit.  For a given name and part 
        number, the su value is unique.  This value may change when new 
        Stackunit items are appended to a Stack.  This value is assigned 
        automatically and should not be altered by the user (accept with the
        renumber method).
...
def stkopen(filename):
    ''' Same as openstk '''
    return openstk(filename)

    
def savestk(stkobject, filename, pagebreak=True, lines_per_page=58):
...
def stksave(stkobject, filename, pagebreak=True, lines_per_page=58):
    ''' Same as savestk '''
    savestk(stkobject, filename, pagebreak, lines_per_page)

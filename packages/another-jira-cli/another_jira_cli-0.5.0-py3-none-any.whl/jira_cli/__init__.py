...
def lookup(search_alias):
    """
    Calls a previously saved search.
    """
    config = read_config()
...
def cleanup_day(date):
    ...
    jira = get_jira()
    tickets = get_tickets_logged_on_date(arr)
...
def fill_day(date, comment, day_hours, day_start, re_init):
    """
    Fills a day (=8h) with a default ticket. You can also specify other tickets
    you have worked on with a time-ratio using the --tickets option.
    Example:
    """
    start_h, start_m = parse_start_time_str(day_start)
    start_time = parse_date_str(date).replace(hour=start_h, minute=start_m)
    config = read_config()
    ...
    in_hours = int(available_secs // 3600)
    in_mints = int((available_secs % 3600) // 60)
    ...
    if available_secs <= 60:
        sum_h = logged_work_secs // 3600

        print("You have already >8 hours logged, not filling up.")
    else:
...
def list_worklogs(date):
    """
    Lists all logs for a given day.
    """
...
def list_work(date):
    """
    Lists on which tickets you worked how long on a given day.
    """
    date_obj = parse_date_str(date)
    tickets_and_logs = get_worklogs_for_date_and_user(date_obj)
    logged_work_secs = calculate_logged_time_for(tickets_and_logs)

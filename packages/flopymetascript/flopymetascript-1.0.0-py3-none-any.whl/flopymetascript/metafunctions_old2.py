...
class Model(object):
    ...
    def __init__(self, load_nam='', add_pack=[], load_only=None):
        ...
        if load_nam:
            ...
            self.sw = flopy.seawat.Seawat.load(
                f, version='seawat', exe_name='swt_v4', verbose=False,
    ...
    def script_kwargs2string(self, name, print_descr=True, width=99,
                             bonus_space=0):
        sout = [
    ...
    def script_constructor2string(self, name, width=99, bonus_space=0,
                                  use_yapf=True):
        ...
        if use_yapf:
            ...
            sout = textwrap.wrap(
                s, width=width, initial_indent=' ' * bonus_space,
    ...
    def script_package2string(self, name, print_descr=True, width=99,
                              bonus_space=0):
        kwargs = self.script_kwargs2string(name, print_descr=print_descr,
                                           width=width,
                                           bonus_space=bonus_space)
        constr = self.script_constructor2string(name, width=width,
                                                bonus_space=bonus_space)
    ...
    def script_model2string(self, print_descr=True, width=99, bonus_space=0):
        ...
        for mod in [item for item in all_modules if item in unique_modules]:
            ...
            out.append(
                self.script_package2string(mod, print_descr=print_descr, width=
                width, bonus_space=bonus_space))
            ...
            for name, item in self.packages.items():
                ...
                out.append(
                    self.script_package2string(name, print_descr=print_descr,
                                               width=width, bonus_space=
                                               bonus_space))
        ...
        for module in [item for item in all_modules if item in unique_modules]:
            ...
            out.append(
                nbf.v4.new_code_cell(
                    self.script_package2string(module, print_descr=print_descr,
                                               width=width, bonus_space=
                                               bonus_space)))
            ...
            for name, item in self.packages.items():
                ...
                out.append(
                    nbf.v4.new_code_cell(
                        self.script_package2string(name, print_descr=
                        print_descr, width=width,
                                                   bonus_space=bonus_space)))
    ...
    def write_script_model2string(self, fn='', print_descr=True, width=99,
                                  bonus_space=0):
        nb = self.script_model2nb(print_descr=print_descr, width=width,
                                  bonus_space=bonus_space)
    ...
    def parse_array_str(ar, orig_shape, compres, precision=8):
        ...
        if compres == 0:
            ...
            ar_str = np.array2string(
                ar, max_line_width=max_line_width, precision=precision,
                suppress_small=suppress_small, separator=',', prefix=pre)
    ...
    def print_string2(self, key, width=99, bonus_space=0, use_yapf=True):
        ...
        if use_yapf:
            ...
            if key == 'stress_period_data':
                s1 = self.print_string1(s, prelude, width=width - 1,
                                        bonus_space=bonus_space)
                s1 = '\\\n'.join(s1)
            else:
                s1 = self.print_string1(s, prelude, width=width,
                                        bonus_space=bonus_space)
                s1 = '\n'.join(s1)
...
def uniquend(ar, return_index=False, return_inverse=False, return_counts=False,
             axis=None):

...
class Client(BaseClient):
    ...
    def get_active_orders(self):
        ...
        """
        order_list = self.get_orders_list()
        jason = order_list.json()
        for order in jason:
            jason = self.get_order_status(order).json()
            status = self.get_order_status(order).json()['status']
            if status != 'purged':
                yield order
    ...
    def _active_items(self, order_id, verbose=False):
        ...
        """
        status = self.get_item_status(order_id).json()[order_id]

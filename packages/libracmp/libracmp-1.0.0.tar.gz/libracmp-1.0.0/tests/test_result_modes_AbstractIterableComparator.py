...
import libracmp
from libracmp.AbstractIterableComparator import AbstractIterableComparator
import libracmp.exceptions


class TestResultAbstractIterableComparator(TestCase):

    def test_result_lista_listNone_default_neq(self):
        a = ['a']
        b = [None]
        comp = AbstractIterableComparator(a, b)
        self.assertFalse(comp.result)

    def test_result_lista_listNone_strict_neq(self):
        a = ['a']
        b = [None]
        comp = AbstractIterableComparator(a, b, mode=libracmp.MODE_STRICT)
        self.assertFalse(comp.result)

    def test_result_lista_listNone_relaxed_eq(self):
        a = ['a']
        b = [None]
        comp = AbstractIterableComparator(a, b, mode=libracmp.MODE_RELAXED)
        self.assertTrue(comp.result)

    def test_result_list1_listf1_typestrict_neq(self):
        a = [1]
        b = [1.0]
        comp = AbstractIterableComparator(a, b, mode=libracmp.MODE_TYPESTRICT)
        self.assertFalse(comp.result)

    def test_result_lista_listfb_ignore_eq(self):
        a = ['a']
        b = ['b']
        comp = AbstractIterableComparator(a, b, mode=libracmp.MODE_IGNORE)
        self.assertTrue(comp.result)
    ...
    def test_result_mode_default_none_exc(self):
        ...
        comp = AbstractIterableComparator(a, b, mode=mode_default_none)

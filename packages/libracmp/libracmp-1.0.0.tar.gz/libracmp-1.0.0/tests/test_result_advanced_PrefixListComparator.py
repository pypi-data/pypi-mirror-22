...
class TestResultPrefixListComparator(TestCase):
    ...
    def test_result_listEmpty_listStra_eq(self):
        a = []
        b = ['a']
        comp = PrefixListComparator(a, b)
        self.assertTrue(comp.result)

    def test_result_listStra_listEmpty_eq(self):
        a = ['a']
        b = []
        comp = PrefixListComparator(a, b)
        self.assertTrue(comp.result)

    def test_result_listStra_listStraa_eq(self):
        a = ['a']
        b = ['a', 'a']
        comp = PrefixListComparator(a, b)
        self.assertTrue(comp.result)

    def test_result_listStraa_listStra_eq(self):
        a = ['a', 'a']
        b = ['a']
        comp = PrefixListComparator(a, b)
        self.assertTrue(comp.result)
    ...
    def test_result_mlistab_mlistablistc_neq(self):
        a = [[[[[['a', 'b']]]]]]
        b = [[[[[['a', 'b', ['c']]]]]]]
        # this is not compared by PrefixListComparator
        # inner values are compared according to mode and/or filters
        comp = PrefixListComparator(a, b)
        self.assertFalse(comp.result)

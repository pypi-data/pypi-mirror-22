...
def db_unique_random(obj, initial_length = 10, max_length = 32, attribute = "identity", post_prefix = "", post_suffix = ""):
	...
	raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.NO_UNIQUE_RANDOM_FOUND)
...
def db_unique_string(pattern, db_obj, initial_length = 20, attribute = "identity", to_int = False, post_prefix = "", post_suffix = ""):
	...
	for up_to in range(initial_length, max_length + 1):
		...
		try:
			...
			raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.QUERY_SET)
		
		if obj == None:
			string = tmp_string
			break
	else:
		raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.NO_UNIQUE_STRING_FOUND)
	...
	for var in vars:
		...
		if var[0] == 'db':
			...
			if var[2] == type(params[var[1]]):
				...
				raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.INVALID_DB_TYPE)
			...
			if is_string(params[var[1]], var[2]):
				...
				raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.INVALID_STRING)
			...
			if is_email_valid(email = params[var[1]]):
				...
				raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.INVALID_EMAIL)
			...
			if is_password_safe(params[var[1]], **var[2]) == True:
				...
				raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.INVALID_PASSWORD)
			...
			if params[var[1]] == for_true:
				...
				raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.INVALID_BOOL)
			...
			if type(params[var[1]]) == var[2]:
				...
				raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.WRONG_TYPE)
			...
			if is_url_safe(url = params[var[1]]) == True:
				...
				raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.INVALID_URL)
			...
			if is_file_safe(file_name = params[var[1]]) == True:
				...
				raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.INVALID_FILE)
			...
			if is_upload_file_safe(u_file = params[var[1]]) == True:
				...
				raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.INVALID_UPLOAD_FILE)
			...
			if is_ip_address(ip = params[var[1]]) == True:				
				...
				raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.INVALID_IP_ADDRESS)
			...
			if is_request(request = params[var[1]]) == True:
				...
				raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.INVALID_REQUEST)
			...
			if is_custom_request(request = params[var[1]], to_check = var[2]) == True:
				...
				raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.INVALID_CUSTOM_REQUEST)
			...
			if var[2] == 'email':
				...
				if is_email_valid(email = params[var[1]]) == True:
					...
					raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.INVALID_USERNAME)
			...
			if is_mix_type(var = params[var[1]],types = var[2]) == True:
				...
				raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.INVALID_MIX_TYPE)
			...
			if re.match(var[2], var[1]):
				...
				raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.INVALID_REGEX)
			...
			if is_number(params[var[1]]):
				...
				raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.INVALID_NUMBER)
			...
			if is_valid_device(params[var[1]]):
				...
				raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.INVALID_DEVICE)

		else:

			raise ohm2_handlers_light_definitions.OHM2HandlersLightRunException(**ohm2_handlers_light_errors.INVALID_TYPE)
...
def process_uploaded_image(image, **kwargs):
	...
	f1 = tempfile.NamedTemporaryFile(suffix = "." + image.name.rsplit(".", 1)[-1], delete = True)
	f1.write(content)
	...
	f2 = tempfile.NamedTemporaryFile(suffix = kwargs.get("suffix", ".jpg"))
	im.save(f2.name,

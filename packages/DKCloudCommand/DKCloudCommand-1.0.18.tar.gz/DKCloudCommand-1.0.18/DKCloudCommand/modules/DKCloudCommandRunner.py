...
class DKCloudCommandRunner(object):
    ...
    @staticmethod
    def tree_view(node_name, tree_kitchen_dict, depth=1, output=[]):
        ...
        if childs is not None:
            for child in childs:
                output.append('%s+---- %s\n' % ('    ' * depth, child))
                DKCloudCommandRunner.tree_view(child, tree_kitchen_dict, depth+1, output)
        return ''.join(output)
    ...
    def list_kitchen(dk_api):
        ...
        if rc.ok():
            ...
            if kl is not None and len(kl) > 0:
                ...
                tree_kitchen_dict = dict()
                for k in sorted_list:
                    # populate kitchen dict as pre condition to build the tree view
                    if k['name'] == k['parent-kitchen'] == u'master':
                        continue
                    kitchen_list = tree_kitchen_dict.get(k['parent-kitchen'], None)
                    if kitchen_list is None:
                        kitchen_list = []
                    kitchen_list.append(k['name'])
                    tree_kitchen_dict[k['parent-kitchen']] = kitchen_list
                rs += DKCloudCommandRunner.tree_view(u'master', tree_kitchen_dict)
                rc.set_message(rs)
            else:

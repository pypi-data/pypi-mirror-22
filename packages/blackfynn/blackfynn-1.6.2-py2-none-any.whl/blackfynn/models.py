...
class File(BaseDataNode):
    ...
    def __init__(self, name, s3_key, s3_bucket, file_type, **kwargs):
...
class TimeSeries(DataPackage):
    ...
    def insert_layer(self, name, description=None):
        layer = TimeSeriesAnnotationLayer(name=name, time_series_id = self.id, description=description)
        self.add_layer(layer)
        return layer

    def add_layer(self,layer):
        tmp_layer = self._api.timeseries.create_annotation_layer(self,layer)
...
class TimeSeriesAnnotationLayer(BaseNode):
    ...
    def add_annotations(self, *annotations):
        ...
        for annot in annotations:
            tmp =  self._api.timeseries.create_annotation(ts=ts, layer=self, annot=annot)
            annot.__dict__.update(tmp.__dict__)

...
class TimeSeriesAPI(APIBase):
    ...
    def create_annotation_layer(self, ts, layer):
        ts_id = self._get_id(ts)
        path = self._uri('/{id}/layers', id=ts_id)
        resp = self._post(path, data=layer.as_dict())
    ...
    def create_annotation(self, ts, layer, annot):
        """
        ...
        path = self._uri('/{ts_id}/layers/{layer_id}/annotations',
                    ts_id=ts_id, layer_id=layer_id )
        resp = self._post(path, data=annot.as_dict())

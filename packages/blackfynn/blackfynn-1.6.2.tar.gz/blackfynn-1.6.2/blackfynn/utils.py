...
def infer_epoch(thing):
    if isinstance(thing, datetime.datetime):
        ...
        # assume seconds
        log.info('Using float in infer_epoch. Previously input was assumed to be seconds and converted to usecs. Please double check')
        return thing
    else:
        raise Exception("Cannot parse date")
...
def usecs_to_datetime(us):
    ...
    return datetime.datetime.utcfromtimestamp(0) + datetime.timedelta(microseconds=us)

# -*- coding: utf-8 -*-
"""
Created on Wed May 10 15:08:41 2017
...
def rename_pdf(pdf_path):
    '''函数的作用是将pdf论文改名，形式为‘数字.pdf’，数字的顺序从0开始。
       函数有一个参数，为存放pdf论文的目录的绝对路径'''
    files = os.listdir(pdf_path)
    for i in range(len(files)):
        if(os.name == 'nt'):
            oldname = pdf_path + '\\' + files[i]
            newname = pdf_path + '\\' + str(i) + '.pdf'
        if(os.name == 'posix'):
            oldname = pdf_path + '/' + files[i]
            newname = pdf_path + '/' + str(i) + '.pdf'
...
def pdf_to_txt(pdf_path,txt_path):
    '''函数将pdf文件名改名，新名称是从0开始的数字，
       并将pdf文件转换为txt纯文本文件。
       函数有两个参数，第一个是保存pdf文件的目录的绝对路径，
       第二个是保存txt文件的目录的绝对路径'''
    rename_pdf(pdf_path)
    files = os.listdir(pdf_path)
    ...
    for file in files:
        ...
        #生成每一个pdf文件的绝对路径，并生成相应txt文件的绝对路径
        if(os.name == 'nt'):
            abs_path_txt = txt_path + '\\'+ file_name + '.txt'
            abs_path_pdf = pdf_path + '\\'+ file_name + '.pdf'
        if(os.name == 'posix'):
            abs_path_txt = txt_path + '/'+ file_name + '.txt'
            abs_path_pdf = pdf_path + '/'+ file_name + '.pdf'
        #创建txt文件并将pdf的内容写入
        with open(abs_path_txt,'w') as a:
            for i in get_pages(abs_path_pdf):
                a.write(i)

if __name__ == '__main__':
    #输入pdf与txt文件夹绝对路径
    pdf_path = 'D:\\anaconda_prj\\jaccord\\cn_pdfs'
    txt_path = 'D:\\anaconda_prj\\jaccord\\cn_txts'

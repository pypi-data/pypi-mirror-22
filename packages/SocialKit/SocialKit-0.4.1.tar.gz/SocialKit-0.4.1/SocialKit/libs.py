...
def async_clound_get(spide_urls, urls, method, limit=10, spider_proxy="socks5://127.0.0.1:1080"):
    asyncer = Exe(limit)
    for res in asyncer.map(clound_get, [[spide_urls, url ,"", "browser", spider_proxy] for url in urls]):
        if res['code'] != 400:
...
class Google:
    ...
    def search(self, key):
        ...
        if self.mode == 'text':
            ...
            return [GoogleMsg(pq(i)) for i in pq(self.web.page)("div.rg_di.rg_bx.rg_el.ivg-i")]

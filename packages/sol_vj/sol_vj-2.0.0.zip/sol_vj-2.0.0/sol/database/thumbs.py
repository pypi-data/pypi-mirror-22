...
class ThumbMaker:
    def __init__(self,desired_width,aspect_width=16,aspect_height=9):
        if not os.path.exists(C.SCROT_DIR): os.makedirs(C.SCROT_DIR)
    ...
    def create_shot(self,f_in,f_out,seek_time=0,width=0,height=0):
        ...
        if seek_time > 0:
            ...
            file_info = ''
            try:
                command = '{} -i "{}" -show_entries format=duration -v quiet -of csv="p=0"'.format(self.correct_ff_path('ffprobe'),f_in)
    ...
    def gen_thumbnail(self,f_name,n_frames=1):
        ...
        if n_frames > 1:
            ...
            file_info = ''
            try:
                command = '{} -i "{}" -show_entries format=duration -v quiet -of csv="p=0"'.format(self.correct_ff_path('ffprobe'),f_name)
                file_info = subprocess.check_output(command, 
                            stderr=subprocess.STDOUT, shell=True)
            except subprocess.CalledProcessError as e:
                file_info = e.output

    ...
    def record_command(self, command, output, env=os.environ):
        ...
        def _write_stdout(data):
            ...
            codelynx_api.handle_data('data', quiet=True, rec=data.decode(),
                                     session_id=session_id, google_api_key=google_api_key,
                                     shell=shell, terminal_user=terminal_user,
                                     term_program=term_program)
            # print data on terminal
            os.write(pty.STDOUT_FILENO, data)
            # if cmdlast == b'\r':
            #     nonlocal my_data
            #     my_data += str(data, 'utf-8')

            #     if my_data.endswith('$ '):
            #         my_data = re.split(r'\r\n', my_data)
            #         nonlocal ecode
            #         data_length = len(my_data)
            #         ecode = my_data.pop(data_length - 2)
            #         # Remove echo line
            #         del my_data[data_length - 3]
            #         my_data = "\r\n".join(my_data)

            #         # Send data to server
            #         codelynx_api.handle_data('', my_data)
            #         # Print command output
            #         os.write(pty.STDOUT_FILENO, my_data.encode())
            # else:
            #     os.write(pty.STDOUT_FILENO, data)
        ...
        def _copy(signal_fd):
            ...
            while True:
                ...
                if pty.STDIN_FILENO in rfds:
                    ...
                    cmdlast = data
                    # logic: turn exe signal to exe and echo exit code of last
                    # cmd
                    # if data == b'\r':
                    #     data = b'\recho $?\r'

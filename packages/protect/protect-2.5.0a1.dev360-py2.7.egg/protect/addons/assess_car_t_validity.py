...
def assess_car_t_validity(job, gene_expression, univ_options, reports_options):
    ...
    :param dict gene_expression: Results form running rsem
    :param dict univ_options: Dict of universal options used by almost all tools
    :param dict reports_options: Options specific to assessing the MHC genes
    :return: The results of running assess_mhc_genes
    :rtype: toil.fileStore.FileID
    ...
    overexpressed = []
    # See if tumor_type is acceptable
    try:
        cancer = TCGAToGTEx[tumor_type]
    except KeyError:
        cancer = 'NA'
    # Write the report
    with open('car_t_target_report.txt', 'w') as car_t_report:
        if cancer in target_data.index:
            car_t_report.write('Available clinical trials for specified cancer\n')

            car_t_report.write(('\t{:10}{:<20}{:<20}{:<10}{:<40}{:<12}\n'.format('Gene',
                                                                                 'Threshold_GTEX',
                                                                                 'Threshold_TCGA_N',
                                                                                 'Observed',
                                                                                 'DOI for gene papers',
            ...
            for index, row in target_data.iterrows():
                if index == cancer:
                    ...
                    if observed != 'NA':
                        if float(gtex) <= float(observed) and float(tcga) <= float(observed):
                            car_t_report.write((
                                         '\t{:10}{:<20}{:<20}{:<10}{:<40}{:<12}\n'.format(row['TARGET'],
                                                                                          gtex, tcga,
                                                                                          observed,
            ...
            # See if there are other clinical trials for other cancer types
            car_t_report.write(
                '\nBased on the genes overexpressed in this cancer type, here\'s a list of clinical\
                 trials for other types of cancer\n')
            car_t_report.write(('\t{:10}{:<40}{:<20}{:<12}\n'.format('Gene', 'DOI for gene papers',
                                                                     'Clinical Trial', 'Cancer')))

            for index, row in target_data.iterrows():
                if row['ENSG'] in overexpressed and index != cancer:
                    car_t_report.write(
                        ('\t{:10}{:<40}{:<20}{:<12}\n'.format(row['TARGET'], row['DOI'],
                                                              row['Clinical trials'], index)))

        else:
            car_t_report.write('Data not available')

...
def assess_itx_resistance(job, gene_expression, univ_options, reports_options):
    ...
    :param dict univ_options: Dict of universal options used by almost all tools
    :param dict reports_options: Options specific to assessing the MHC genes
    :return: The fsID for the mhc pathway report file
    :rtype: toil.fileStore.FileID
    """
    job.fileStore.logToMaster('Running create itx resistance report on %s' % univ_options['patient'])
    work_dir = os.getcwd()
    tumor_type = univ_options['tumor_type']

    # Read background tcga and gtex values
    input_files = {
    ...
    with open('immunotherapy_resistance_report.txt', 'w') as report_file:
        # See if data exsits for specified tumor type
        try:
            pathways = json_data['Cancer_to_pathway'][tumor_type]
        except KeyError:
            report_file.write("Data not available")
        else:
            ...
            for pathway in pathways:
                ...
                report_file.write('Pathway: ' + pathway + '\n')
                description = json_data['Pathways'][pathway]['description']
                report_file.write('Description of pathway:\n ' + textwrap.fill(description, width=100)
                                  + '\n')
                report_file.write('Pathway genes: \n')
                report_file.write('\t{:10}{:<20}{:<20}{:<12}\n'.format('Gene', 'Threshold_GTEX',
                ...
                for gene in json_data['Pathways'][pathway]['genes']:
                    ...
                    tpmValue = '{0:.2f}'.format(float(patient_df.loc[gene, 'TPM'])) \
                                if gene in patient_df.index else 'NA'
                    ensg = json_data['Pathways'][pathway]['genes'][gene]
                    report_file.write('\t{:10}{:<20}{:<20}{:<12}\n'.format(ensg, gtex, tcga, tpmValue))
                    if gtex != 'NA' and tpmValue != 'NA':
                        tcga_bool = comp_fn(float(tpmValue), float(tcga))
                        gtex_bool = comp_fn(float(tpmValue), float(gtex))
                        status.append(tcga_bool and gtex_bool)

                # Based on the number of genes with expression values above normal, assess the status
                if sum(status) >= 0.75 * len(status):
                    report_file.write('\nStatus: Based on the results the patient is recommended for\
                                       immunotherapy\n\n')
                else:
                    report_file.write('\nStatus: Based on the results the patient is not recommended\
                                      for immunotherapy\n\n')

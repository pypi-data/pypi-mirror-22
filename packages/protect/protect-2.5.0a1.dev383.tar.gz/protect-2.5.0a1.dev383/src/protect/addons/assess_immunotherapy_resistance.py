...
def assess_itx_resistance(job, gene_expression, univ_options, reports_options):
    ...
    with open('immunotherapy_resistance_report.txt', 'w') as report_file:
        ...
        try:
            ...
            print('Data not available', file=report_file)
        else:
            ...
            for pathway in pathways:
                ...
                print('\t{:10}{:<20}{:<20}{:<12}'.format('Gene', 'Threshold_GTEX',
                                                                       'Threshold_TCGA_N', 'Observed'), file=report_file)
                status = []
                ...
                for gene in json_data['Pathways'][pathway]['genes']:
                    ...
                    print('\t{:10}{:<20}{:<20}{:<12}'.format(ensg, gtex, tcga, tpmValue), file=report_file)
                ...
                if sum(status) >= 0.75 * len(status):
                    print('\nStatus: Patient might react well to the treatment given the'
                                      ' tumor gene signatures.', file=report_file)
                else:
                    print('\nStatus: Patient might not react well to the treatment given '
                                      'the tumor gene signatures.', file=report_file)

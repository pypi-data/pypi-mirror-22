...
def assess_car_t_validity(job, gene_expression, univ_options, reports_options):
    ...
    with open('car_t_target_report.txt', 'w') as car_t_report:
        if tissue_of_origin in target_data.index:
            print('Available clinical trials for specified cancer', file=car_t_report)

            print(('\t{:10}{:<20}{:<20}{:<10}{:<40}{:<12}'.format('Gene','Threshold_GTEX',
                                                                    'Threshold_TCGA_N', 'Observed',
                                                                     'DOI for gene papers',
                                                                    'Clinical Trials')), file=car_t_report)
                ...
                if index == tissue_of_origin:
                    ...
                    if observed != 'NA':
                        if float(gtex) <= float(observed) and float(tcga) <= float(observed):
                            print(( '\t{:10}{:<20}{:<20}{:<10}{:<40}{:<12}'.format(row['TARGET'],
                                                                                    gtex, tcga,
                                                                                    observed,
                                                                                    row['DOI'],
                                                                                    row['Clinical trials'])),
            ...
            print(('\t{:10}{:<40}{:<20}{:<12}'.format('Gene', 'DOI for gene papers', 'Clinical Trial',
                                                        'Cancer')), file=car_t_report)

            for index, row in target_data.iterrows():
                if row['ENSG'] in overexpressed and index != tissue_of_origin:
                    print(('\t{:10}{:<40}{:<20}{:<12}'.format(row['TARGET'], row['DOI'],

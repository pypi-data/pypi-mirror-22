...
def run_itx_resistance_assessment(job, rsem_files, univ_options, reports_options):
    ...
    :param dict mhc_genes_options: Options specific to reporting modules
        ...
        try:
            ...
            for pathway in pathways:
                ...
                if sum(status) >= 0.75 * len(status):
                    report_file.write('\nStatus: Patient might react well to the treatment given the \
                                      tumor gene signatures.\n\n')
                else:
                    report_file.write('\nStatus: Patient might not react well to the treatment given \
                                      the tumor gene signatures.\n\n')

...
class Show(OptionsClass):
    ...
    def blank_lines(self, *args, **kwargs):
        kwargs = self.method_push(None, 'blank_lines', kwargs)
        opts = self.options.push(kwargs)
        if opts.show:
    ...
    def hr(self, *args, **kwargs):
        ...
        kwargs = self.method_push(None, 'hr', kwargs)
        opts = self.options.push(kwargs)
        if opts.show:
    ...
    def title(self, *args, **kwargs):
        ...
        # self.meth_defaults['title'].setdefault('char',  six.u('\u2500'))

        kwargs.setdefault('_callframe', inspect.currentframe().f_back)
        kwargs = self.method_push(None, 'title', kwargs)
        opts = self.options.push(kwargs)
        if opts.show:
            self.say.title(*args, **kwargs)

    def sep(self, *args, **kwargs):
        # self.meth_defaults['sep'].setdefault('char', six.u('\u2500'))
        # self.meth_defaults['sep'].setdefault('width', 2)
        kwargs.setdefault('_callframe', inspect.currentframe().f_back)
        kwargs = self.method_push(None, 'sep', kwargs)
        opts = self.options.push(kwargs)
        if opts.show:
            self.say.sep(*args, **kwargs)

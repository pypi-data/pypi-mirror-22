...
def _reader(opener, _url):
    ...
    logging.info('%s: HEADER (%s): %s', filename, len(_headers), _headers)
    num_headers = len(_headers)
    i = 0
    for i, row in enumerate(_reader):
        row = [str(x).strip() for x in row]
        logging.info('%s: ROWS %(%s): %s', filename, len(row), row)
        values = set(row)
...
def stream_reader(_resource, _url, _ignore_missing):
    ...
    if schema is None:
        ...
        logging.info('SCHEMA FIELDS (%d): %s',
                     len(schema['fields']), schema['fields'])

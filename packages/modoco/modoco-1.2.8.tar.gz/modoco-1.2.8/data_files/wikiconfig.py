...
class Config(multiconfig.DefaultConfig):
    ...
    ########################
    # Modoco custom settings
    ########################

    # applied last to override any earlier settings

    # identity
    sitename = u'Modoco'
    logo_string = u'<img src="%s/modernized/img/modoco-logo.png" alt="Modoco Logo">' % url_prefix_static
    page_front_page = u"FrontPage"

    # force theme to modernized because we deploy custom css there only
    theme_default = 'modernized'
    theme_force = True
    stylesheets = [('screen', '%s/modernized/css/draft-not-sop.css' % url_prefix_static)]

    # custom formatting for draft pages
    from MoinMoin.formatter import text_html                                    
    from modoco.monkey import startContent as patchedStartContent
    text_html.Formatter.startContent = patchedStartContent

    # editor
    comment_required = True

    # custom login page with no account creation link
    from MoinMoin.auth import MoinAuth
    from modoco.monkey import login_hint as patched_login_hint
    MoinAuth.login_hint = patched_login_hint

    # may be overridden by custom SecurityPolicy...
    acl_rights_before = u"WikiAdmin:read,write,delete,revert,admin -PublisherAgent:read"
    acl_rights_default = u"+Known:read,write"

    # more lenient package scripting for install - no users have this
    packagepages_actions_excluded = ['renamepage', 'deletepage', 'delattachment'] 

    # superusers
    superuser = [u"WikiAdmin", ]

    # bot user with special privilege
    publisher_agent_user = "PublisherAgent"

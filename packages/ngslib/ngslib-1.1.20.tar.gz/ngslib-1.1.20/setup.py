#!/usr/bin/python
#Last-modified: 23 Mar 2017 03:13:51 PM
...
if __name__ == '__main__':
    ...
    elif '--without-kent' in sys.argv:
        print >>sys.stderr, "Skip KentLib ..."
        sys.argv.remove('--without-kent')
    else:
        print >>sys.stderr, "Compile KentLib ..."
        ret = os.system('cd external/KentLib && make && cd ../..')
        if ret != 0:
            sys.exit("ERROR: Compiling KentLib error!")
        exts = [Extension('wWigIO',['external/KentLib/wWigIO/wWigIO.c'],extra_link_args=['-DMACHTYPE_x86_64','-lz','-lm','external/KentLib/lib/jkweb.a'],extra_compile_args='-w -shared -fPIC -p -Iexternal/KentLib/inc'.split(' ')),

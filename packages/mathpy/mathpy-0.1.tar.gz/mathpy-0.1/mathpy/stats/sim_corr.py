"""
Module containing several correlation matrix simulation routines as in
(Hardin, Garcia, & Golan, 2013). Methods are more or less a translation
of the supplementary R code here: http://projecteuclid.org/euclid.aoas/1380804814#supplemental.
Methods are called using the interface function simulate_corr_matrix().
...
def simulate_corr_matrix(k=None, nk=None, rho=None, M=None, power=None, method=None):
    ...
    x = CorMatrixSim(k, nk, rho, M, power)
    if method is None:
...
def add_noise(cor, epsilon=None, M=None):
    ...
    cor = CorMatrixSim._generate_noise(cor, n, M, epsilon)

    return cor


class CorMatrixSim(object):

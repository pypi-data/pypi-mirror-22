...
import numpy as np
from mathpy.numtheory.factorial import factorial


def binom_coeff(n, k, method=None):
    r"""
    Interface function to the BinomailCoefficient class which contains several methods for
    ...
    x = BinomialCoefficient()
    if method is None:
        f = getattr(x, x.method)(n, k)
    elif hasattr(x, binom_method[method]):
        f = getattr(x, binom_method[method])(n, k)
    else:
...
class BinomialCoefficient(object):
    ...
    def binomial_recursive(self, n, k):
        ...
        binom_coeff : function
            Front-end interface function for computing the binomial coefficient given
            :math:`n` and :math:`k`
    ...
    @staticmethod
    def binomial_multiplicative(n, k):
        r"""
    ...
    def binomial_factorial(n, k):
        ...
        if k <= 0 or k <= 0 or k > n:
            raise ValueError('incorrect arguments given for binomial coefficient')
        ...
        if nk >= 100:
            with localcontext() as ctx:
                ctx.prec = 50
                bico = Decimal(factorial(n)) / (Decimal(factorial(k)) * Decimal(factorial(nk)))
        else:
            bico = float(factorial(n)) / float(factorial(k) * factorial(nk))

...
def norm(x, order=None, p=None):
    ...
    if x.ndim == 1:
        ...
        if order is None:
            v = getattr(x, x.order)
        elif hasattr(x, vec_norm[order]):
            v = getattr(x, vec_norm[order])
        elif order is 'p':
            v = getattr(x, 'p')
            return float(v(p))
        else:
            raise ValueError('unknown norm, ' + str(order) + ' given.')
    else:
        x = MatrixNorm(x)
        if order is None:
            v = getattr(x, x.order)
        elif hasattr(x, matrix_norm[order]):
...
class VectorNorm(object):
    ...
    def norm2(self):
        ...
        References
        ----------
        .. [1] Golub, G., & Van Loan, C. (2013). Matrix computations (3rd ed.). Baltimore (MD): Johns Hopkins U.P.
    ...
    def pnorm(self, p):
        ...
        if isinstance(p, int) is False:
            ...
            raise ValueError('p must be coercible into an integer')

        if np.iscomplex(p) or p < 1:
            raise ValueError('p must be at least 1 and real')

        pn = np.sqrt(np.sum(np.power(np.absolute(self.x), 1 / p)))

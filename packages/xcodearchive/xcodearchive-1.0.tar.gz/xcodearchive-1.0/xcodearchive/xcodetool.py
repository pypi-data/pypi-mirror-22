...
enter_method = ['build project', 'xcodearchive project', 'export ipa','clean build']
...
def archive_project():
    ...
    if work_space_file:
        archive_cmd = 'xcodebuild xcodearchive -archivePath %s -workspace %s -scheme %s -configuration %s ' % (
        os.path.join (base_dir, result[3]), os.path.join (root_dir, work_space_file), result[1], result[2])
    else:
        archive_cmd = 'xcodebuild xcodearchive -archivePath %s -project %s -scheme %s -configuration %s ' % (
        os.path.join (base_dir, result[3]), os.path.join (root_dir, work_space_file), result[1], result[2])

...
def worker(q, fn, queue_timeout):
    while True:
        try:
            #item = q.get(timeout=timeout)
            #fn(item)
            args, kwargs = q.get(queue_timeout=queue_timeout)
            fn(*args, **kwargs)
            q.task_done()
...
class async():
    ...
    def _revive_thread(self):
        self.t = threading.Thread(target=worker, args=(self.q, self.fn, self.queue_timeout))
    ...
    def call(self, *args, **kwargs):
        ...
        if not self.t.isAlive():
            # print("Restarting thread and queue after queue timeout")
            self._revive_thread()

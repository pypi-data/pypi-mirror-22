...
def compute_vipm_icon_path():
    ...
    if platform.system().lower() == 'linux':
        return os.path.join(base_path, 'vipm-no-background.png')
    elif platform.system().lower() == 'windows':
        return os.path.join(base_path, 'vipm-no-background-windows.ico')
    else:
...
def on_linux():
    ...
    try:
        path_to_exec = six.text_type(subprocess.check_output(['which', 'virtual-ipm-gui']).strip())

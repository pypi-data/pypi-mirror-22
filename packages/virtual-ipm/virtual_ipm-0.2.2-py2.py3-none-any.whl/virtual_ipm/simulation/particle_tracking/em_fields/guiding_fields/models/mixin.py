...
class CSVAdaptor3D(GuidingFieldModel):
    ...
    def prepare(self):
        ...
        # We called it in the following Ex, Ey, Ez however this is similarly valid for magnetic
        # field values.
        ...
        self._y_min = numpy.min(xs)
        self._y_max = numpy.max(xs)
        self._z_min = numpy.min(xs)
        self._z_max = numpy.max(xs)
    ...
    def eval(self, position_four_vectors, progress):
        ...
        # Stack along first dimension and use dimensionality 2 -> '0,2'.
        return numpy.r_[str('0,2'), Ex, Ey, Ez]

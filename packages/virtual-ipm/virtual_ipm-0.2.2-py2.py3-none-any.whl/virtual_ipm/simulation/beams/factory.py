...
from .bunch_trains import BunchTrain, CircularBunchTrain, LinearBunchTrain
...
@parametrize(
    PhysicalQuantity(
        'TotalIonizationCrossSection',
        unit='m^2',
        default=1.0,
        info='The total ionization cross section is used to determine how many particles per beam '
             'should be created. The default value is unphysical and just serves as a convenience '
             'if one beam is specified. If you specify the total ionization cross section for one '
             'beam make sure you also specify it for all other beams.'
    ),
...
class Beam(Component):
    ...
    @property
    def total_ionization_cross_section(self):
        return self._total_ionization_cross_section
...
class BeamFactory(object):
    ...
    def create(self, configuration):
        ...
        bunch_train_type = configuration.get_text(BunchTrain.TYPE_INDICATOR_PATH)
        if bunch_train_type == LinearBunchTrain.__name__:
            bunch_train = LinearBunchTrain(
                bunch_shape,
                bunch_electric_field_model,
                configuration
            )
        elif bunch_train_type == CircularBunchTrain.__name__:
            bunch_train = CircularBunchTrain(
                bunch_shape,
                bunch_electric_field_model,
                configuration
            )
        else:
            raise ValueError('Unknown bunch train type: %s' % bunch_train_type)

...
class LogView(Widgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._handler.records.observe_on(current_thread_scheduler)\
...
class ProgressView(Widgets.QWidget):
    ...
    def __init__(self, progress, parent=None):
        ...
        progress.observe_on(current_thread_scheduler) \
    ...
    def __init__(self, status_updates, parent=None):
        ...
        def set_label_for_status(status):
            self._statistics.setItem(
                ...
                0,
                Widgets.QTableWidgetItem(Particle._statuses[status])
            )
            self._statistics.setItem(
                self.row_number_per_status[status],
                1,
        ...
        status_updates.observe_on(current_thread_scheduler).subscribe(on_next=on_status_update)
...
class ParticleMonitor(Widgets.QWidget):
    ...
    def __init__(self, position_updates, parent=None):
        ...
        position_updates.subscribe(on_next=self.new_position)
...
class MainView(Widgets.QWidget):
    def __init__(self, config, parent=None):
        ...
        progress_view = ProgressView(self._thread.progress)
        particles_view = ParticlesView(self._thread.status_updates)
        log_view = LogView()
        ...
        particle_monitor = ParticleMonitor(self._thread.position_updates)

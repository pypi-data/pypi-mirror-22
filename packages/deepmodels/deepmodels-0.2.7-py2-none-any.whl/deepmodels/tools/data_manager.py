...
from deepmodels.core.commons import DataFileType
...
def gen_data_filename(fn_prefix, data_type=DataFileType.DATA_TRAIN):
  ...
  Returns:
    generated filename.
  """
  if data_type == DataFileType.DATA_TRAIN:
    return "{}__train.csv".format(fn_prefix)
  if data_type == DataFileType.DATA_TEST:
    return "{}__test.csv".format(fn_prefix)
  if data_type == DataFileType.DATA_LABEL:
    return "{}__labels.txt".format(fn_prefix)


def convert_data_filename(fn, dst_data_type):
  """Convert one data filename to another.

  Args:
    fn: input data filename.
    dst_data_type: target data file type.
  Returns:
    converted data filename.
  """
  sep_idx = fn.find("__")
  assert sep_idx != -1, "invalid input data filename."
  return gen_data_filename(fn[:sep_idx], dst_data_type)
...
def load_crop_imgs(img_fns, img_bboxes, img_sz, use_bgr=True):
  ...
  if not use_bgr:
    nb_channels = 1
  imgs = np.ndarray(
      (len(img_fns), nb_channels, img_sz[0], img_sz[1]), np.float32)
  print imgs.shape
  for i in range(len(img_fns)):
    im = cv2.imread(img_fns[i])

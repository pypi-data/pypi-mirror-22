...
longitude(192)) m s-1>]
    ...
    def regrids(self, dst, src_cyclic=None, dst_cyclic=None,
                ...
                axis_order=None, i=False, _compute_field_mass=None):
        '''
...
>>> h = f.regrids(g, axis_order='ZT')
        ...
        # Retrieve the source field's axes indices for ordering
        axis_indices = []
        ...
        if axis_order is not None:
            for axis in axis_order:
                ...
                if axis_key is not None:
                    if axis_key in (x_axis_s, y_axis_s):
                        raise ValueError('Cannot loop over X or Y axis')
                    #--- End: if
                    try:
        ...
        # Reorder keys by list of axes to minimise how often the mask is
        # likely to change.
        if axis_indices:
        ...
        for k in section_keys:
            ...
            if _compute_field_mass is not None and method == 'conservative':
                if not type(_compute_field_mass) == dict:
                    raise ValueError('Expected compute_field_mass to be a dictionary.')
...
#            f.varray
    ...
    def regridc(self, dst, axes, method='auto', use_dst_mask=False,
                fracfield=False, i=False, _compute_field_mass=None):
        '''

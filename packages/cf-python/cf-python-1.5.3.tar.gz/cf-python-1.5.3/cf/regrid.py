...
:Returns:
    ...
    out: ESMF.Grid
        ...
        if ndim == 1:
            if not use_bounds:
                # For 1D nonconservative  regridding the extra dimension should
                # already have been added in cf.Field.regridc.
                raise ValueError('Cannot create a Cartesian grid from one ' +
                                 'dimension coordinate with no bounds.')
    ...
    f_2D : bool
        ...
        if not x.Units.islongitude or not y.Units.islatitude:
            ...
            if x_size == 1 or y_size == 1:
                raise ValueError('Neither the longitude nor latitude dimension' +
                                 ' coordinates of the ' + name + ' field can' +
                                 ' be of size 1.')
        #--- End: if

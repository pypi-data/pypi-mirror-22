...
:Parameters:
    ...
    def regrids(self, dst, src_cyclic=None, dst_cyclic=None,
                ...
                i=False, _compute_field_mass=None):
        '''
...
>>> h = f.regrids(g, axis_order='ZT')
        ...
        # Assume that if either src_axes or dst_axes are specified that we
        # have a tripolar grid and it is necessary to set ignore_degenerate
        ignore_degenerate = src_axes is not None or dst_axes is not None
...
#            f.varray
    ...
    def regridc(self, dst, axes, method='auto', use_dst_mask=False,
                fracfield=False, axis_order=None, i=False,
        ...
        if n_axes == 1 and self.squeeze().ndim > 1:
            ...
            if src_shape[src_axis_indices].prod()*max_length*8 < CHUNKSIZE():
                ...
                src_axis_indices_ext, src_order_ext = \
                    self._get_axis_indices(axis_keys_ext + src_axis_keys)
        
        # Calculate shape of each regridded section
        shape = self._get_section_shape([coord.size for coord in dst_coords +
                                         coords_ext], src_axis_indices_ext)

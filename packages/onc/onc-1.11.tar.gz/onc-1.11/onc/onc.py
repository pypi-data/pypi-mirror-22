...
class ONC:
    ...
    def getScalarObservations(self, 
        ...
        observations = []
        payload = self.getScalarArray(parameters)
        sensorData = payload['sensorData']
#         serviceMetadata = payload['serviceMetadata']
        
        sensorCount = len(sensorData)
#         totalSamples = int(serviceMetadata['totalActualSamples'])
#         totalObservations = int(totalSamples/sensorCount)
    ...
    '''
    Method to print to the console and return scalar data from the scalardata service in JSON Object format
    which match filter criteria defined by a dictionary of parameters.
    
    see https://wiki.oceannetworks.ca/display/help/scalardata+service for usage and available parameters
    '''                                          
    def printScalar(self, 
                  parameters={}): #dictionary of web service filter parameters
        ...
        payload = self.getScalar(parameters)
        ...
        serviceMetadata = payload['serviceMetadata']
        if(serviceMetadata):
            print('{} samples from sensors {}'.format(serviceMetadata['totalActualSamples'],','.join(sensorNames)))

        
        return observations
        
    '''
    Method to return observations in the payload from the scalardata service in JSON Array format
    which match filter criteria defined by a dictionary of parameters.
    
    see https://wiki.oceannetworks.ca/display/help/scalardata+service for usage and available parameters
    '''    
    def getScalarArray(self, 
                       parameters={}): #dictionary of web service filter parameters
        ...
        parameters['method'] = "getByStation"
        parameters['token'] = self.token
        parameters['outputFormat'] = 'Array' #Options Object or Array
        parameters['metadata'] = 'Full'
    ...
    '''
    Method to print observations to the console, which are returned in the payload from the scalardata service
    in JSON Array format, which match filter criteria defined by a dictionary of parameters.
    
    see https://wiki.oceannetworks.ca/display/help/scalardata+service for usage and available parameters
    '''    
    def printScalarArray(self, 
                       parameters={}): #dictionary of web service filter parameters
            
        print('Location')
        self.getLocations({'locationCode':parameters['station']})
        print('Device Category')
        self.getDeviceCategories({'deviceCategoryCode':parameters['deviceCategory']})
        ...
        if (payload):
            ...
            print('{} payload for: {}'.format(payload,parameters))
        ...
        if (dpRequest):
            ...
            if not showsFileCount or (numFiles >= 1 and showsFileCount):
                ...
                if (ids):
                    for id in ids:
                        if id > 0:
                            result = self.downloadDataProduct(id,numFiles,estimatedProcessingTime,maxRetries,downloadResultsOnly)
                            for r in result:
...
#                     [downloadUrls.append(self.downloadDataProduct(id,numFiles,estimatedProcessingTime,maxRetries,downloadResultsOnly)[:]) for id in ids if id > 0]
    ...
    def requestDataProduct(self, 
                           parameters={},
                           returnError=False): #If True, the function will return the 
...
#                 ids = r['searchIds']
    ...
    def downloadDataProduct(self, 
                            ...
                            fileCount,                  # The actual or estimated file count, which is returned from the dataProductDelivery request method
                            ...
                            downloadResultsOnly=False,  # Determines if files will be downloaded by the downloadDataProductIndex method, or if it returns information about the files so that they can be downloaded later.
        ...
        while True:
            ...
            dict = self.downloadDataProductIndex(runId,url,parameters,downloadResultsOnly,multiThreadMessages,indx,fileCount,estimatedProcessingTime,maxRetries)
            if (dict):
        ...
        if (includeMetadataFile):
            ...
            dict = self.downloadDataProductIndex(runId,url,parameters,downloadResultsOnly,multiThreadMessages,1,1,1,maxRetries)
            if (dict):
                downloadResults.append(dict)
                if 'downloaded' in dict:
    ...
    '''
    Method to download a data product for a runId and Index (file number or 'meta').
    '''
    def downloadDataProductIndex(self, 
                                 ...
                                 downloadResultsOnly,       # Determines if files will be downloaded by the downloadDataProductIndex method, or if it returns information about the files so that they can be downloaded later.
    ...
    '''
    '''
    def getJsonDataProduct(self, 
                           parameters, 
                           maxRetries=100):
        
        urls = self.orderDataProduct(parameters, maxRetries, True)
    ...
    def getDataProductUrls(self, 
        ...
        return [r['url'] for r in self.orderDataProduct(parameters, maxRetries, True) if 'url' in r]
    ...
    '''
    def getDataProductUrlsPerformance(self, 
                                      parameters={}, 
                                      maxRetries=10, 
                                      requestCount=10):
    ...
    compare the performance between equivalent requests in 1.0 and 2.0 services.
    '''
    def getWriteDataProductToFilesPerformance(self, 
                                              parameters={}, 
                                              maxRetries=10, 
                                              requestCount=10):
        stats = {}

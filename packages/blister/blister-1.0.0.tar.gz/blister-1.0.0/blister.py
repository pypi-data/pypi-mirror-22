...
class _BlisterImp(pkgutil.ImpImporter):
    ...
    def find_module(self, fullname, path=None):
        ...
        names = self._scan_modules(self.__path)
        importData = names.get(moduleName)
        if not importData:
            return None
    ...
    def load_module(self, fullname):
        # First check for an existing module
        existing = sys.modules.get(fullname, None)
    ...
    def _scan_modules(path):
        ...
        names = _BlisterImp._cache.get(path)
        if names is not None:
            return names

        _log.debug("Scanning path '%s'", path)
        if not os.path.isabs(path) or os.access(path, os.W_OK):
        ...
        # Organize files into packages or files for each suffix
        names = {}
        suffix_files = dict((s[0], []) for s in _BlisterImp._suffixes)
        pkgSuffix = ("", "", imp.PKG_DIRECTORY)
        for file_name in dir_list:
            file_path = os.path.join(path, file_name)
            if re.match(_BlisterImp._retoken, file_name) and os.path.isdir(file_path):
                names[file_name] = (file_path, pkgSuffix)
            else:
                modName, ext = os.path.splitext(file_name)
                files = suffix_files.get(ext, None)
                if files is not None:
                    if re.match(_BlisterImp._retoken, modName):
                        files.append((modName, file_path))

        # Pick files based on suffix
        for suffix in _BlisterImp._suffixes:
            for file_name, file_path in suffix_files[suffix[0]]:
                previous = names.get(file_name)
                if previous:
                ...
                if os.path.isdir(file_path):
                    continue
                if previous:
                    pkgNames = _BlisterImp._scan_modules(previous[0])
                    if "__init__" in pkgNames:
                        continue
                names[file_name] = (file_path, suffix)
        _BlisterImp._cache[path] = names
        return names

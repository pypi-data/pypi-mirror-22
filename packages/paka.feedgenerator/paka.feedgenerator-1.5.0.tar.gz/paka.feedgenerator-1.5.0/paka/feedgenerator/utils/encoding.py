...
class FeedgeneratorUnicodeDecodeError(UnicodeDecodeError):
...
def force_text(s, encoding='utf-8', strings_only=False, errors='strict'):
    ...
    try:
        ...
        if not isinstance(s, Exception):
            raise FeedgeneratorUnicodeDecodeError(s, *e.args)
        else:
...
def iri_to_uri(iri):
    ...
    Takes an IRI in UTF-8 bytes (e.g. '/I \xe2\x99\xa5 Feedgenerator/') or unicode
    (e.g. '/I ♥ Feedgenerator/') and returns ASCII bytes containing the encoded result
    (e.g. '/I%20%E2%99%A5%20Feedgenerator/').
    """
...
def uri_to_iri(uri):
    ...
    Takes an URI in ASCII bytes (e.g. '/I%20%E2%99%A5%20Feedgenerator/') and returns
    unicode containing the encoded result (e.g. '/I \xe2\x99\xa5 Feedgenerator/').
    """
    if uri is None:
        return uri
    uri = force_bytes(uri)

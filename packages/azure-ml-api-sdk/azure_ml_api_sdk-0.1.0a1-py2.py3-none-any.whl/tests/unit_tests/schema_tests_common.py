...
class SchemaUnitTests(unittest.TestCase):
    ...
    sample_json = os.path.join(tests_folder, 'resources', 'sample_json.json')
    sample_csv = os.path.join(tests_folder, 'resources', 'sample_csv.csv')
    sample_tsv = os.path.join(tests_folder, 'resources', 'sample_tsv.tsv')
    sample_arff = os.path.join(tests_folder, 'resources', 'sample_arff.arff')
    sample_parquet = os.path.join(tests_folder, 'resources', 'sample_parquet.parquet')
    ...
    def _validate_swagger_schemas(self, expected_schema, generated_schema):
        ...
        if isinstance(expected_example, list) or isinstance(expected_example, dict):
            if sys.version_info[0] == 3:
                self.assertCountEqual(expected_example, fixed_floats_example)
            else:
                self.assertItemsEqual(expected_example, fixed_floats_example)
        else:

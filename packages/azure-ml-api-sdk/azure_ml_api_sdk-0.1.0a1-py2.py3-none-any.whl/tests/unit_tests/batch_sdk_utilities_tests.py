...
import numpy as np
from azure.ml.api.batch import _batch_sdk_utilities
from azure.ml.api.schema.sampleDefinition import SampleDefinition
from azure.ml.api.schema.sampleDefinition import DataTypes
...
class BatchSdkUtilitiesTests(unittest.TestCase):
    ...
    @patch('azure.ml.api.schema.schemaUtil.save_service_schema', autospec=True)
    ...
    def test_add_argument_parser_to_main(self):
        ...
        self.assertEqual(result, self.expected_arg_parser_lines)
    ...
    def test_add_schema_handling_to_main(self):
        ...
        self.assertEqual(result, self.expected_schema_handling_lines)

    @patch('azure.ml.api.batch._batch_sdk_utilities._add_argument_parser_to_main', autospec=True)
    @patch('azure.ml.api.batch._batch_sdk_utilities._add_schema_handling_to_main', autospec=True)
    ...
    @patch('azure.ml.api.batch._batch_sdk_utilities.generate_main')
    def test_generate_driver_file(self, mock_generate_main):
        mock_generate_main.return_value = self.expected_main

        test_result_driver_path = 'tests\\unit_tests\\resources\\test_result_driver_file.txt'
        test_expected_driver_path = 'tests\\unit_tests\\resources\\test_expected_driver_file.txt'
    ...
    @patch('azure.ml.api.batch._batch_sdk_utilities.generate_schema_file', autospec=True)
    @patch('azure.ml.api.batch._batch_sdk_utilities.generate_driver_file', autospec=True)
    @patch('azure.ml.api.batch._batch_sdk_utilities.parse_func_defaults', autospec=True)

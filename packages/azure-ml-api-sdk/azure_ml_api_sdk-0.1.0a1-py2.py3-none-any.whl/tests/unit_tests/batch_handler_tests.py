...
class BatchHandlerTests(unittest.TestCase):
    ...
    @patch('azure.ml.api.batch.batch_handler.schemaUtil.parse_batch_input', autospec=True)
    @patch('azure.ml.api.batch.batch_handler.schemaUtil.parse_service_input', autospec=True)
    def test_batch_parse_service_input(self, mock_parse_service_input, mock_parse_batch_input):
        ...
        mock_parse_service_input.assert_called_once_with(json.dumps(non_input_args), service_schema.input, self.default_dict)

        self.assertEqual(return_value, parsed_args_after_df_replace)

    @patch('azure.ml.api.batch.batch_handler.prepare_publish_setup', autospec=True)
    @patch('azure.ml.api.batch.batch_handler.generate_create_command', autospec=True)

...
def _create_service_package(main, input_types, output_types,
    ...
    try:
        ...
        if input_types is not None or output_types is not None:
            save_service_schema(os.path.join(output_directory_name, schema_file_name), input_types, output_types)
        _save_main_file(main, os.path.join(output_directory_name, DRIVER_FILE_NAME))
...
def prepare(run_func, init_func, input_types=None, output_types=None, runtime="pyspark", attached_files=None, pip_requirements_file_location=None):
    if run_func is None:
    ...
    if input_types is not None or output_types is not None:
        schema_file_name = "serviceschema_{0}.json".format(_get_unique_time_stamp())

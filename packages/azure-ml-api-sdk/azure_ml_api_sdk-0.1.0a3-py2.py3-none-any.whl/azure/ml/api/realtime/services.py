...
def init():
    ...
    aml_serialized_input_globals = util.deserialize("<defaults>")
    user_init_func()
    '''
...
def _construct_run_function_signature(args, default_value):
    arg_list = ""
    for arg in args:
        if arg in default_value:
            arg_list += arg + "=" + str(default_value[arg]) + ", "
        else:
            arg_list += arg + ", "
    arg_list = arg_list.rstrip().rstrip(",")
    return "def " + USER_RUN_FUNCTION_NAME + "(" + arg_list + "):\n"


def _process_run_func(run_func, input_schema, output_schema, main, schema_filename=None):
    run_args = _get_args(run_func)
    default_values = _get_args_defaults(run_func)
    if len(run_args) > 0 and input_schema is None:
        raise Exception("Provided run function has arguments, input schema needs to be provided")
    if input_schema is not None:
        ...
        for arg in run_args:
            ...
            if arg not in input_schema:
                raise Exception("Argument mismatch: Provided run function argument {0} is not "
                                "present in input types dictionary which contains: ({1})"
                                .format(arg, ", ".join(input_schema.keys())))
    source = _construct_run_function_signature(run_args, default_values) + _get_source(run_func)
    if input_schema is not None:
        main = main.replace('<schema_file>', schema_filename)
    else:
        main = main.replace('"<schema_file>"', "None")
    main = main.replace('<user_run_function>', source)
    main = main.replace('user_run_func', USER_RUN_FUNCTION_NAME)
    main = main.replace('<defaults>', util.serialize(default_values))
    return main
...
def _create_service_package(main, input_types, output_types,
                            schema_file_name, attached_files=None, requirements_file=None, drop_folder=None):
    output_directory_name = "output_" + _get_unique_time_stamp()
    output_path = output_directory_name
    if drop_folder is not None:
        output_path = os.path.join(drop_folder, output_directory_name)

    try:
        _create_directory(output_path)
        if input_types is not None or output_types is not None:
            save_service_schema(os.path.join(output_path, schema_file_name), input_types, output_types)
        _save_main_file(main, os.path.join(output_path, DRIVER_FILE_NAME))
        if attached_files is not None:
            _copy_attached_files(attached_files, output_path)

        if requirements_file is not None:
            _copy_requirements_file(requirements_file, output_path)
        return output_path
    except:
...
def _save_main_file(main, file_name):
    try:
        with open(file_name, "w") as py_file:
            py_file.write(main)
    except:
        print("Failed to create {0} file".format(file_name))
        raise
...
def _get_main_body(run_func, init_func, input_schema, output_schema, schema_file_name):
    main = _get_main_template()
    print("processing init function")
    main = _process_init_func(init_func, main)
    print("processing run function")
    main = _process_run_func(run_func, input_schema, output_schema, main, schema_file_name)
    return main


def prepare(run_func, init_func, input_types=None, output_types=None, runtime="pyspark", attached_files=None, pip_requirements_file_location=None, drop_folder=None):
    if run_func is None:
    ...
    if input_types is not None or output_types is not None:
        schema_file_name = "service_schema.json"
    main = _get_main_body(run_func, init_func, input_types, output_types, schema_file_name)
    print("setting up output directory")
    output_dir = _create_service_package(main, input_types, output_types, schema_file_name, attached_files,
                                         pip_requirements_file_location, drop_folder)
    print("Done setting up output directory, available here {0}".format(output_dir))
    return output_dir


def generate_main(run_func, init_func, schema_file, main_file_name):
    service_schema = load_service_schema(schema_file)
    main = _get_main_body(run_func, init_func, service_schema.input, service_schema.output, schema_file)
    _save_main_file(main, main_file_name)

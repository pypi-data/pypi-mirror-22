...
class Connect:
    ...
    def add_candidate_from_csv(self, csv_file, season=None, name="", prepend_cand_info=True):
        ...
        Args:
            csv_file (str): File path for csv file containing at minimum columns 'date', 'ra', 'dec'. Other recognized column
                names include 'objid', 'expnum', 'exptime', 'band', 'ccd', 'mag', 'ml_score', 'fakeid', and
                'designation'.
    ...
    def add_candidate(self, can_table, season=None, name="", designation="", prepend_cand_info=True):
        ...
        try:
            ...
            if not designation:
                statcmd = "INSERT INTO LZULLO.TNOSTAT(ID, EXTENDED, VISUAL, ACCEPT, QUAL_FLAG, INSPECT_TIME, ANALYST, ANALYST_COMMENT) \
                 VALUES ('" + canid + "', '0', '0','0', '0' ,'UNINSPECTED', 'LZULLO', 'NONE')"
            else:
                statcmd = "INSERT INTO LZULLO.TNOSTAT(ID, EXTENDED, VISUAL, ACCEPT, QUAL_FLAG, INSPECT_TIME, ANALYST, ANALYST_COMMENT) \
                                 VALUES ('" + designation + "', '0', '0','1', '0' ,'UNINSPECTED', 'LZULLO', 'KNOWN_OBJECT')"
    ...
    def __write_obs(self, index, can_table):
        ...
        for i in index:
            ...
            objcmds += ["INSERT INTO lzullo.TNOBS (DATE_OBS, RA, DEC, EXPNUM, EXPTIME, BAND, CCD, MAG , ML_SCORE, OBJID, FAKEID, NITE, FLUX, FLUX_ERR, SEASON, YEAR, PIXELX, PIXELY, FWHM ) \
            VALUES (" + date_obs + "," + ra + "," + dec + "," + expnum + "," + exptime + "," + band + "," + ccd + "," + mag + ","
                        + ml_score + "," + objid + "," + str(fakeid) + "," + nite + "," + str(flux) + "," + str(flux_err) + "," +
                        str(season) + "," + str(year) + "," + pixelx + "," + pixely + "," + fwhm + ")"]
        return objcmds
    ...
    def __write_orb(self, can_table, canid, orbid, designation=""):
        ...
        c = "INSERT INTO LZULLO.TNORBIT (ID, CHISQ , NDOF , A, E, INC, AOP, NODE, PERI_JD, PERI_DATE, EPOCH_JD, MEAN_ANOMALY, PERIOD, \
    A_ERR,E_ERR, INC_ERR, AOP_ERR, NODE_ERR, PERI_ERR, PERIOD_ERR, LAT0, LON0, XBARY, YBARY, ZBARY, ABG_A, ABG_B, ABG_G, ABG_ADOT, ABG_BDOT, \
    ABG_GDOT, ABG_A_ERR, ABG_B_ERR, ABG_G_ERR, ABG_ADOT_ERR, ABG_BDOT_ERR, ABG_GDOT_ERR, ORBID, DESIGNATION) \
        VALUES ('" + canid + "','" + chisq + "','" + ndof + "','" + a + "','" + e + "','" + inc + "','" + aop + "','" + node + "', '" + peri_jd + \
            "', '" + peri_date + "', '" + epoch_jd + "', '" + mean_anomaly + "', '" + period + "','" + a_err + "','" + e_err + "','" + inc_err + \
            "','" + aop_err + "','" + node_err + "','" + peri_err + "','" + period_err + "','" + latO + "','" + lonO + "','" + xbary + "','" \
            + ybary + "','" + zbary + "','" + abg_a + "','" + abg_b + "','" + abg_g + "','" + abg_adot + "','" + abg_bdot + "','" + abg_gdot + "','" \
            + abg_a_err + "','" + abg_b_err + "','" + abg_g_err + "','" + abg_adot_err + "','" + abg_bdot_err + "','"\
            + abg_gdot_err + "'," + orbid + ",'" + designation + "')"
    ...
    def __duplicate_orbit(self, a, e, inc):
        ...
        Returns:
            Checks to see if orbit is already in TNORBIT. Compares given a, e, and inc to orbits in TNORBIT.  If there is a duplication, then 
            returns orbid of duplicate. Else returns False
        """
        query = "SELECT * FROM LZULLO.TNORBIT WHERE LZULLO.TNORBIT.A = '" + a + "'AND LZULLO.TNORBIT.E = '" + e + "'AND LZULLO.TNORBIT.INC = '" + inc + "'"
        results = pd.read_sql(query, self.desoper)
        if results.empty:
            return False
        else:

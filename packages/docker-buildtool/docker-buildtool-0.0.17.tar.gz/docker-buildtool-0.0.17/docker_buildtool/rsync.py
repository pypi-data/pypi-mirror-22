...
def remote_build(dryrun: bool, build, docker_args: List[str], s3_location=None, run_docker_build=True):
    ...
    try:
        ...
        if run_docker_build:
            ...
            remote_dockerfile = os.path.join(remote_dest, os.path.relpath(build.dockerfile, build.build_root))
            command = ['docker-buildtool', '-f', remote_dockerfile, 'build']
            ...
            utils.execute_command(dryrun, [
                'ssh', '-t', '{0}@{1}'.format(user, host),
                'bash -l -c "rm -rf {0}; mv {1} {0}; {2}"'.format(remote_dest, remote_temp, command)])
    finally:
        if dockerfile:
            dockerfile.close()
        if i:

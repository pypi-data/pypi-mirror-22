...
class XDSM(object):
    ...
    def addComp(self, name, style, string, stack=False):
        ...
        :param name: label of the component, using EMPTY heading keywort to specify EMPTY node (e.g. overall inputs and outpus)
        :type name: str
            ...
            for i, chn in enumerate(self.chains):
                ...
                for e in chn[1:]:
                    if '-' in e or '-' in last_node:
                        w(r'       \chainin (%s)    [join=by ProcessHV];' % e)  # used to be ProcessTip
                    else:
    ...
    def _compilepdf(self, filename):
        ...
        subprocess.check_output('pdflatex ' + filename + '.tex', shell=True, stderr=subprocess.STDOUT, cwd=os.path.dirname(filename))

...
from ..utilities.general import transform_data_into_strings, transform_string_into_format, make_camel_case, \
    unmake_camel_case, get_list_entries, open_file, make_plural, color_list, test_attr_cond, get_schema
...
from mixin_vispack import VispackMixin
...
class KadmosGraph(nx.DiGraph, VispackMixin):
    ...
    PROBLEM_ROLES_VAR_SUFFIXES = ['__desVar__',    # 0
                                  ...
                                  '__conVar__',    # 2
                                  '__statVar__']   # 3
    CMDOWS_ROLES_DEF = [['designVariable', ['nominal_value', 'lower_bound', 'upper_bound', 'samples']],   # 0
                        ['objectiveVariable', []],                                                        # 1
                        ['constraintVariable', ['constraint_type', 'reference_value',                     # 2
                                                'required_equality_precision']],
                        ['stateVariable', []]]                                                            # 3
    ...
    def __init__(self, *args, **kwargs):
        ...
        # TODO: Make kb_path optional
        if 'kb_path' in kwargs:
            self.graph['kb_path'] = kwargs['kb_path']
        elif args:
            assert 'kb_path' in self.graph, \
                'kb_path needs to be defined for input graph or as keyword argument "kb_path".'
            self.graph['kb_path'] = args[0].graph['kb_path']
        # if 'kb_path' not in self.graph:
        #    self.graph['kb_path'] = 'unknown'
    ...
    def check_cmdows_integrity(self, mpg=None):
        ...
        # Initial Settings
        tempdir = tempfile.mkdtemp(prefix='KADMOS_')
        ...
        try:
            ...
            # This is needed as some (dictionary) elements of the graph might not be in the correct order after loading
                ...
                if test_list == check_list:
                    ...
                    result_a = False
                    logger.info('The following elements are not loaded: ' +
                                str([x for x in test_list if x not in set(check_list)]))
            # Also check if the file adheres to its schema
            try:
                test_file = etree.parse(os.path.join(tempdir, 'testfile1.xml'))
                url_prefix = '{http://www.w3.org/2001/XMLSchema-instance}noNamespaceSchemaLocation'
                version = test_file.getroot().attrib[url_prefix].split('/')[-2]
                schema = get_schema(version)
                result_b = schema.validate(test_file)
            except:
                result_b = False
        ...
        if result:
            ...
            if not result_a:
                logger.warning('Not all elements are written to/read from the CMDOWS file.')
            # The following is already provided by the load_as_cmdows function
            # if not result_b:
    ...
    def create_dsm(self, file_name, destination_folder=None, open_pdf=False, mpg=None, include_system_vars=True,
                   summarize_vars=False, function_order=None):
        ...
        if mpg is None:
            ...
            for idx in range(0, graph_mpg.number_of_nodes()):
                ...
                assert len(node_list) == 1, 'Somehow, a unique diagonal position %d could not be found in the PSG' \
                                            ' data graph.' % idx
                node = node_list[0]
                if graph_mpg.node[node]['architecture_role'] == graph.ARCHITECTURE_ROLES_FUNS[0]:  # coordinator
                    node_style = 'Coordinator'
                elif graph_mpg.node[node]['architecture_role'] == graph.ARCHITECTURE_ROLES_FUNS[1]:  # optimizer
                    node_style = 'Optimization'
                elif graph_mpg.node[node]['architecture_role'] == graph.ARCHITECTURE_ROLES_FUNS[2]:  # converger
                    node_style = 'Converger'
                elif graph_mpg.node[node]['architecture_role'] == graph.ARCHITECTURE_ROLES_FUNS[3]:  # doe
                    node_style = 'DOE'
                elif graph_mpg.node[node]['architecture_role'] == graph.ARCHITECTURE_ROLES_FUNS[4]:  # pre-coupling an.
                    node_style = 'PreAnalysis'
                elif graph_mpg.node[node]['architecture_role'] == graph.ARCHITECTURE_ROLES_FUNS[5]:  # pre-iterator an.
                    node_style = 'PreAnalysis'
                elif graph_mpg.node[node]['architecture_role'] == graph.ARCHITECTURE_ROLES_FUNS[6]:  # post-iterator an.
                    node_style = 'PreAnalysis'
                elif graph_mpg.node[node]['architecture_role'] == graph.ARCHITECTURE_ROLES_FUNS[7]:  # coupled an.
                    node_style = 'CoupledAnalysis'
                elif graph_mpg.node[node]['architecture_role'] == graph.ARCHITECTURE_ROLES_FUNS[8]:  # post-coupling an.
                    node_style = 'PostAnalysis'
                elif graph_mpg.node[node]['architecture_role'] == graph.ARCHITECTURE_ROLES_FUNS[9]:  # cons. const. fun.
                    node_style = 'PostAnalysis'
                else:
                    raise AssertionError('Architecture role %s is not supported for creation of XDSMs.'
                                         % graph_mpg.node[node]['architecture_role'])
        ...
        for idx1 in range(0, dimension - 1):
            for idx2 in range(idx1 + 1, dimension):
                ...
                if node_set_upp:
                    co(node2, node1, data_type, (str_num_up + str_upp).encode('unicode-escape').replace('_', '\_'))

                # Lower triangle of (X)DSM
                if node_set_low:
                    co(node1, node2, data_type, (str_num_low + str_low).encode('unicode-escape').replace('_', '\_'))

        # Compile PDF and delete temporary files
        #dsm.write(file_name.encode('unicode-escape'), compilepdf=True)
...
#
    ...
    def _save_cmdows(self, file_path,
        ...
        if isinstance(self, RepositoryConnectivityGraph):
            graph = RepositoryConnectivityGraph(self)
            ...
            if not isinstance(mpg, MdaoProcessGraph):
        ...
        graph_design_competences = graph.find_all_nodes(category='function',
        ...
        for graph_designCompetence in graph_design_competences:
            ...
            cmdows_design_competence.set('uID', graph_designCompetence)
            Child(cmdows_design_competence, 'label', graph.node[graph_designCompetence].get('name'))
            ...
            graph_inputs = graph.in_edges(graph_designCompetence)
            graph_outputs = graph.out_edges(graph_designCompetence)
            ...
            graph_data = graph.node[graph_designCompetence]
            ...
            Child(cmdows_general_info, 'version')
            # cmdows_creator = Child(cmdows_general_info, 'creator')
            # cmdows_specialist = Child(cmdows_general_info, 'specialist')
        ...
        # Create executableBlocks/functions
        # cmdows_design_competences = Child(cmdows_executable_blocks, 'functions')
        ...
        if isinstance(graph, FundamentalProblemGraph) or isinstance(graph, MdaoDataGraph):
            ...
            cmdows_problem_definition = Child(cmdows, 'problemDefinition')
            ...
            cmdows_problem_formulation = Child(cmdows_problem_definition, 'problemFormulation')
            graph_problem_formulation = graph.graph.get('problem_formulation')
            ...
            if graph_problem_formulation.get('doe_settings') is not None:
                graph_keys = ['doe_method', 'doe_seed', 'doe_runs']
                ChildSequence(cmdows_doe_settings, graph_problem_formulation['doe_settings'].items(), graph_keys)
            ...
            cmdows_parameters = Child(cmdows_problem_roles, 'parameters')
            # Create problemDefinition/problemRoles/parameters/...
            for cmdows_parameterIndex, cmdows_parameterDef in enumerate(self.CMDOWS_ROLES_DEF):
                cmdows_parameter = Child(cmdows_parameters, cmdows_parameterDef[0]+'s')
                graph_attr_cond = ['problem_role', '==', self.PROBLEM_ROLES_VARS[cmdows_parameterIndex]]
                graph_parameter = graph.find_all_nodes(category='variable', attr_cond=graph_attr_cond)
                for graph_problem_role in graph_parameter:
                    cmdows_problem_role = Child(cmdows_parameter, cmdows_parameterDef[0])
                    ...
                    for cmdows_problem_role_attr in cmdows_parameterDef[1]:
                        if cmdows_problem_role_attr == 'samples':
                            ...
                            # Make e.g. problemDefinition/problemRoles/parameters/designVariables/designVariable/samples
                            ...
                            if graph.node[graph_problem_role].get(cmdows_problem_role_attr) is not None:
                                for idx, itm in enumerate(graph.node[graph_problem_role].get(cmdows_problem_role_attr)):
                                    Child(cmdows_samples, 'sample', itm, attrib={'position': str(idx + 1)})
                        else:
        ...
        if isinstance(graph, MdaoDataGraph):
            ...
            # Create workflow
            cmdows_workflow = Child(cmdows, 'workflow')
            # Create workflow/problemDefinitionUID
            # noinspection PyUnboundLocalVariable
            Child(cmdows_workflow, 'problemDefinitionUID',
                  str(graph_problem_formulation.get('mdao_architecture')) +
                  str(graph_problem_formulation.get('convergence_type')))
            # Create workflow/dataGraph
            cmdows_data_graph = Child(cmdows_workflow, 'dataGraph')
            Child(cmdows_data_graph, 'name', graph.graph.get('name'))
            # Create workflow/dataGraph/edges
            cmdows_edges = Child(cmdows_data_graph, 'edges')
            for u, v, w in graph.edges_iter(data=True):
                # Create workflow/dataGraph/edges/edge
                cmdows_edge = Child(cmdows_edges, 'edge')
                Child(cmdows_edge, 'fromUID', u)
                Child(cmdows_edge, 'toUID', v)
            # Create workflow/processGraph
            cmdows_process_graph = Child(cmdows_workflow, 'processGraph')
            Child(cmdows_process_graph, 'name', mpg.graph.get('name'))
            # Create workflow/processGraph/edges
            cmdows_edges = Child(cmdows_process_graph, 'edges')
            for u, v, w in mpg.edges_iter(data=True):
                # Create workflow/dataGraph/edges/edge
                cmdows_edge = Child(cmdows_edges, 'edge')
                Child(cmdows_edge, 'fromUID', u)
                Child(cmdows_edge, 'toUID', v)
                Child(cmdows_edge, 'processStepNumber', w.get('process_step'))
            # Create workflow/processGraph/nodes
            cmdows_nodes = Child(cmdows_process_graph, 'nodes')
            for n, data in mpg.nodes_iter(data=True):
                # Create workflow/dataGraph/nodes/node
                cmdows_node = Child(cmdows_nodes, 'node')
                Child(cmdows_node, 'referenceUID', n)
                graph_keys = ['process_step', 'converger_step', 'diagonal_position']
                graph_dict = {'process_step': 'processStepNumber', 'converger_step': 'convergerStepNumber'}
                ChildSequence(cmdows_node, data.items(), graph_keys, graph_dict)

            # Create architectureElements
            cmdows_architecture_elements = Child(cmdows, 'architectureElements')
            ...
            for architecture_roles_var in graph.ARCHITECTURE_ROLES_VARS:
                ...
                graph_parameter_nodes = graph.find_all_nodes(attr_cond=['architecture_role', '==',
                                                                        architecture_roles_var])
            ...
            for architecture_roles_fun in self.CMDOWS_ARCHITECTURE_ROLE_SPLITTER:
                if architecture_roles_fun == 'optimizer':
                    data_list = [['label', 'label'],
                                 ['settings', 'settings'],
                                 ['design_variables', ['designVariables', self.PROBLEM_ROLES_VAR_SUFFIXES[0]]],
                                 ['objective_variable', ['objectiveVariables', self.PROBLEM_ROLES_VAR_SUFFIXES[1]]],
                                 ['constraint_variables', ['constraintVariables', self.PROBLEM_ROLES_VAR_SUFFIXES[2]]]]
                    cmdows_executable_blocks.append(ChildGroup(graph, 'architecture_role',
                                                               self.ARCHITECTURE_ROLES_FUNS[1], data_list))
                elif architecture_roles_fun == 'doe':
                    data_list = [['label', 'label'],
                                 ['settings', 'settings'],
                                 ['design_variables', ['designVariables', self.PROBLEM_ROLES_VAR_SUFFIXES[0]]]]
                    cmdows_executable_blocks.append(ChildGroup(graph, 'architecture_role',
                                                               self.ARCHITECTURE_ROLES_FUNS[3], data_list))
                else:
                    data_list = [['label', 'label'],
                                 ['settings', 'settings']]
                    cmdows_executable_blocks.append(ChildGroup(graph, 'architecture_role',
        ...
        # Write CMDOWS file
        ElementTree(cmdows).write(file_path,
                                  pretty_print=pretty_print,
                                  method='xml',
            ...
            for arg in matched_nodes:
                if len(matched_nodes[arg]) > 1:
                    ...
                    if user_select == 1:
                        ...
                        user_sel = prompting.user_prompt_select_options(message=select_nodes_prompt, *matched_nodes[arg])
    ...
    def split_variables(self, *args, **kwargs):
        ...
        for idx, node in enumerate(nodes):
            if print_in_log and idx in progress_steps:
                print '   splitted nodes %d of %d (%d percent)...' % (idx, len(nodes),
                                                                      progress_steps.index(idx)*step_size)
            assert self.has_node(node), 'Node %s is not present in the graph' % node
            assert self.get_node_subcategory(node) in relevant_node_subcats, 'Node %s is not a problematic variable.' \
                                                                             % node
            ...
            if self.get_node_subcategory(node) in function_order_required_cats:
    ...
    def find_all_nodes(self, category='all', subcategory='all', attr_cond=None, attr_include=None, attr_exclude=None,
        ...
        assert len({category}.intersection(possible_categories)) != 0, \
            "Specified category '%s' does not exist. Please choose from %s." % (category, possible_categories)
        assert len({subcategory}.intersection(possible_subcategories)) != 0, \
            "Specified subcategory '%s' does not exist. Please choose from %s." % (subcategory, possible_subcategories)
    ...
    def expand_node(self, node_name, elements):
        """Method to crete a note from temporary node(s) created by the load function

        Used to expand and clean-up nodes read in by the load functions when _ui_d is given in the dictionary. Only
        used for loading from CMDOWS files. The node is then automatically added to the graph.

        :param node_name: name of the node to be expanded
        :type node_name: str
        :param elements: structure of the node
        :type elements: list
        :return: expanded and cleaned-up node
        """
        # TODO: Put this method at another place? In the load function itself?
                ...
                for data_key, data_value in initial_element_data.iteritems():
                    for sub_data_key, sub_data_value in data_value.iteritems():
                        if sub_data_key.endswith('_ui_d'):
                            ...
                            new_subelement_data = dict(zip(element[1], [self.node[new_subelement_id].get(e) for
                                                                        e in element[1]]))
...
def _load_cmdows(file_path):
    ...
    # Parse
    cmdows = etree.parse(file_path).getroot()
    ...
    schema = get_schema(graph.CMDOWS_VERSION)
    if not schema.validate(cmdows):
        logger.warning('The CMDOWS file ' + file_path + ' does not adhere to the current CMDOWS schema version ' +
                       str(graph.CMDOWS_VERSION) + '. You may use a previous version of KADMOS to ensure proper ' +
                       'loading of the CMDOWS file.')
    ...
    for variable in cmdows.findall('parameters/parameter'):
        graph.add_node(variable.get('uID'), category='variable', shape='o', label=variable.findtext('label'))

    # Create function nodes
    for function in cmdows.findall('executableBlocks/designCompetences/designCompetence'):
        graph.add_node(function.get('uID'),
                       category='function',
                       shape='s',
                       name=function.findtext('label'),
                       label=function.findtext('label'),
                       level=function.findtext('metadata/performanceInfo/fidelityLevel'),
                       description=function.findtext('metadata/generalInfo/description'),
                       run_time=function.findtext('metadata/performanceInfo/runTime'),
                       precision=function.findtext('metadata/performanceInfo/precision'),
                       standard_deviation=function.findtext('metadata/performanceInfo/standardDeviation'))
        # Create function input edges
        for inp in function.findall('inputs/input'):
            graph.add_edge(inp.findtext('parameterUID'), function.get('uID'))
        # Create function output edges
        for output in function.findall('outputs/output'):
            graph.add_edge(function.get('uID'), output.findtext('parameterUID'))

    # The following nodes are only generated/modified for FPGs and MDGs
    if isinstance(graph, FundamentalProblemGraph) or isinstance(graph, MdaoDataGraph):
        ...
        for cmdows_doe_setting in list(cmdows_doe_settings):
            graph.graph['problem_formulation']['doe_settings'][unmake_camel_case(cmdows_doe_setting.tag)] = cmdows_doe_setting.text
        ...
        for role in graph.FUNCTION_ROLES:
            ...
            if cmdows_blocks is None:
                ...
                for cmdows_block in list(cmdows_blocks):
                    if graph.node.get(cmdows_block.text) is None:

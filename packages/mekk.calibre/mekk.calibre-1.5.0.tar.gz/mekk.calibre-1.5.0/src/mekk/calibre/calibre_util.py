...
def find_calibre_file_names(use_cache=False, search=None):
    """
    Yields all files registered in Calibre database (just filenames)

    :param use_cache: is using cached catalog allowed? Meaningful only if search is not given
    :param search: searching rule (in calibre syntax) or None if whole database is to be returned
    """
    for elem in calibre_query_files(use_cache=use_cache, search=search):
        try:
            files = [fmtel.text
...
_re_book_id = re.compile('^Added book ids: (?P<id>\d+)$', re.MULTILINE)
...
def add_to_calibre(filename,
    ...
    Attempts to return added book id (but that is volatile and
    language-setting dependant), returns None if the guess failed.
    """

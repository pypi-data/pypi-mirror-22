...
class IENAM(IENA):
    ...
    def unpack(self,buf,ExceptionOnLengthError=False):
        super(IENAM, self).unpack(buf,ExceptionOnLengthError=False)
        (self.paramID, self.delay, self.datasetlength) = struct.unpack(IENAM._FORMAT_,self.payload[:6])

...
class Client():
    def __init__(self, environment, impersonate=None, internal_token=None, password=None, ssl_verify=None, username=None,):
    ...
    def _ensure_has_credentials(self):
        if isinstance(self.internal_token, str):
        ...
        if os.path.exists(CREDENTIALS_FILE):
            ...
            if self.internal_token is True:
                self.internal_token = credentials.get('internal-token')
                if not self.internal_token:
                    raise diax.errors.CredentialsError(
                        "You requested using stored internal token, but there is no token for {} in {}".format(
                            self.environment, CREDENTIALS_FILE
                    ))
            else:
            ...
            if self.ssl_verify is None and credentials.get('certificate-bundle'):
                self.ssl_verify = credentials.get('certificate-bundle')
                LOGGER.info("Using SSL certificate bundle %s from credentials file", self.ssl_verify)
            return
...
def create(args):
    ...
    return Client(
        ...
        internal_token      = args.internal_token or args.use_internal_token,
        ssl_verify          = args.certificate_bundle,

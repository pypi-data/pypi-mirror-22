...
def import_station_df(path):
    ...
    """
    df = pd.read_csv(path, 
     converters={'WBAN': str, 'WMO': str, 'CallSign': str, 
...
def _filter_stations(stations, include_types, tmy_only=False):
    if tmy_only:
        return stations[stations.apply(lambda r: any([r[s] for s in include_types]) and r['TMY3'] is True, axis=1)]
    else:
        return stations[stations.apply(lambda r: any([r[s] for s in include_types]), axis=1)]
...
def import_address_workbook(path):
    ...
    options = pd.read_excel(path, 
                            ...
                            converters={'Station Type': str, 'Include': bool_convert}, 
...
def get_nearest_station(coords, stations, n=1):
    ...
    """
    distances = stations.apply(lambda x: vincenty(coords, (x['Latitude'], x['Longitude'])).miles, axis=1)  
    ...
    if len(str(closest_station['WBAN'])) < 5:
        closest_station['WBAN'] = (5 - len(str("183"))) * '0' + str(closest_station['WBAN'])
...
def geocode_addresses(addresses, geolocator, batch=False, pause=1):
    ...
    """
    if batch:
        raise NotImplementedError("Batch processing has not yet been implemented")
    
    return addresses.apply(_geocode_row, axis=1, sleep=pause, geolocator=geolocator)

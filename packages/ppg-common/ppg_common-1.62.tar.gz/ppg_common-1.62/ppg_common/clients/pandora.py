...
class PandoraClient(BaseClient):
    ...
    def create_box(self, box_name):
        ...
        return self.post(query_box_url, query={'name': box_name})
    ...
    def rename_file(self, file_id, name):
        ...
        return self.patch(url, headers={"filename": name})

    def rename_box(self, box_id, name):
        """
        Function for renaming box
        """
        url = self.urls.get("box").format(box_id) + '/rename'
        return self.patch(url, headers={"boxname": name})

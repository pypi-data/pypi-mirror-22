...
class VectorValue {
	...
	function __construct($n=null) {
		if (is_resource($n) && get_resource_type($n) === '_p_std__vectorT_Value_p_t') {
			$this->_cPtr=$n;
			return;
		}
		switch (func_num_args()) {
		case 0: $this->_cPtr=new_VectorValue(); break;
		default: $this->_cPtr=new_VectorValue($n);
		}
	}

	function size() {
		return VectorValue_size($this->_cPtr);
	}

	function capacity() {
		return VectorValue_capacity($this->_cPtr);
	}

	function reserve($n) {
		VectorValue_reserve($this->_cPtr,$n);
	}

	function clear() {
		VectorValue_clear($this->_cPtr);
	}

	function push($x) {
		VectorValue_push($this->_cPtr,$x);
	}

	function is_empty() {
		return VectorValue_is_empty($this->_cPtr);
	}

	function pop() {
		$r=VectorValue_pop($this->_cPtr);
		if (is_resource($r)) {
			$c=substr(get_resource_type($r), (strpos(get_resource_type($r), '__') ? strpos(get_resource_type($r), '__') + 2 : 3));
			if (class_exists($c)) return new $c($r);
			return new Value($r);
	...
	function get($i) {
		$r=VectorValue_get($this->_cPtr,$i);
		if (is_resource($r)) {
			$c=substr(get_resource_type($r), (strpos(get_resource_type($r), '__') ? strpos(get_resource_type($r), '__') + 2 : 3));
			if (class_exists($c)) return new $c($r);
			return new Value($r);
	...
	function set($i,$val) {
		VectorValue_set($this->_cPtr,$i,$val);
	}
}
...
class Value {
	...
	function getName() {
		return Value_getName($this->_cPtr);
	}

	function getDescription() {
		return Value_getDescription($this->_cPtr);
	}

	function getProperty() {
		$r=Value_getProperty($this->_cPtr);
		if (is_resource($r)) {
			$c=substr(get_resource_type($r), (strpos(get_resource_type($r), '__') ? strpos(get_resource_type($r), '__') + 2 : 3));
			if (class_exists($c)) return new $c($r);
...
class Property {
	...
	function __construct($res=null) {
		if (is_resource($res) && get_resource_type($res) === '_p_Property') {
			$this->_cPtr=$res;
			return;
		}
		$this->_cPtr=new_Property();
	}

	function getName() {
		return Property_getName($this->_cPtr);
	}

	function getDescription() {
		return Property_getDescription($this->_cPtr);
	}

	function getCategory() {
		return Property_getCategory($this->_cPtr);
	}

	function getValueType() {
		return Property_getValueType($this->_cPtr);
	}

	function getDefaultValue() {
		$r=Property_getDefaultValue($this->_cPtr);
		if (is_resource($r)) {
			$c=substr(get_resource_type($r), (strpos(get_resource_type($r), '__') ? strpos(get_resource_type($r), '__') + 2 : 3));
			if (class_exists($c)) return new $c($r);
			return new Value($r);
	...
	function getValues() {
		$r=Property_getValues($this->_cPtr);
		if (is_resource($r)) {
			$c=substr(get_resource_type($r), (strpos(get_resource_type($r), '__') ? strpos(get_resource_type($r), '__') + 2 : 3));
			if (class_exists($c)) return new $c($r);
...
class Provider {
	...
	function getProperty($propertyName) {
		$r=Provider_getProperty($this->_cPtr,$propertyName);
		if (is_resource($r)) {
			$c=substr(get_resource_type($r), (strpos(get_resource_type($r), '__') ? strpos(get_resource_type($r), '__') + 2 : 3));
			if (class_exists($c)) return new $c($r);
			return new Property($r);

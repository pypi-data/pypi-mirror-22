...
class InstanceV(DockerV):
    ...
    def db_config(self):
        ...
        if env_vars is None:
            ...
            logger.debug("Instance config dict from json: %s",
                         str(instance_cfg))
            res = {
                'db_host': instance_cfg.get('db_host', '127.0.0.1'),
            ...
            if self.config.get('postgres_container') and\
                    self.basic_info.get('ports').get('5432'):
    ...
    def restore_database(self, db_name=False, backup_dir=False):
        ...
        if use_template:
            ...
            if not backup_dir:
                backup_dir = path.expanduser(self.config.get('backup_folder'))

...
def batch_service_run(service_name, verb, inputs,
    ...
    try:
        parameter_dict.update({param_name: update_asset_path(context, verb,
                                                             ...
                                                             is_input=input_output_dict[param_name][1])[1]
...
def batch_service_create(driver_file, service_name, title, verb, inputs,
                         outputs, parameters, dependencies, custom_ice_url,
                         context=cli_context):
    """
    Processing for creating a new batch service
    ...
    if context.env_is_k8s:
        docker_url = create_docker_image(driver_file, [], context=context, service_name=service_name,
                                               target_runtime='spark-py', verb=verb, custom_ice_url=custom_ice_url)

...
class KubernetesOperations:
    ...
    def deploy_deployment(self, dep, max_deployment_time_s, desired_replica_num, secret_name,
                          verbose=False):
        """
...
def deploy_realtime_frontend(k8s_ops, acr_email):
    ...
    k8s_ops.deploy_deployment_file(os.path.join(Constants.DATA_DIRECTORY, 'azureml-fe-dep.yaml'), 120, 1, 'amlintfeacrkey')
    k8s_ops.expose_frontend(os.path.join(Constants.DATA_DIRECTORY, 'azureml-fe-service.yaml'))
...
def deploy_batch_frontend(k8s_ops, storage_acct_name, storage_acct_key, acr_key):
    ...
    batch_dep_path = os.path.join(Constants.DATA_DIRECTORY, 'spark_batch_dep.yaml')
    batch_service_path = os.path.join(Constants.DATA_DIRECTORY, 'spark_batch_service.yaml')

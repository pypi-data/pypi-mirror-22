...
def create_docker_image(score_file, dependencies,  service_name, verb, target_runtime,
                        requirements='', schema_file='', model='', custom_ice_url='', context=cli_context):
    """Create a new realtime web service."""
    from ._k8s_util import test_acs_k8s
    from .service._realtimeutilities import try_add_sample_file
    from .service._realtimeutilities import upload_dependency
    verbose = verb

    is_known_runtime = target_runtime in Constants.supported_runtimes or target_runtime in Constants.ninja_runtimes
    if score_file == '' or service_name == '' or not is_known_runtime:
        print(Constants.create_cmd_sample)
        return
    ...
    utilities_filename = resource_filename(__name__, 'service/azuremlutilities.py')
    ...
    if schema_file != '':
        ...
        if sample_added:
            get_sample_code = \
                resource_string(__name__, 'service/data/getsample.py').decode('ascii').replace('PLACEHOLDER', sample_filename)

    ...
    def subplots(self, hide_axes=False, dx=0.055, dy=0.025):
        ...
        for rix, yf in enumerate(self.__yfac_lvls):
            for cix, xf in enumerate(self.__xfac_lvls):
                ...
                if ax.is_last_col():
                    ax2 = ax.twinx()
                    if hasattr(self.__yfac[1], 'format'):
                        ax2.set_ylabel(self.__yfac[1].format(yf),
                                       rotation=270)
                    else:
                        ax2.set_ylabel(self.__yfac[1](yf), rotation=270)
                ...
                if ax.is_first_row():
                    if hasattr(self.__xfac[1], 'format'):
                        ax.set_title(self.__xfac[1].format(xf))
                    else:
                        ax.set_title(self.__xfac[1](xf))
    ...
    def subplots(self, hide_axes=False, dx=0.055, dy=0.025):
        ...
        for rix in range(self.__num_rows):
            for cix in range(self.__num_cols):
                ...
                ax.xaxis.get_label().set_alpha(0)
                ax.yaxis.get_label().set_alpha(0)
                if hasattr(fac[1], 'format'):
                    ax.set_title(fac[1].format(lvl))
                else:
                    ax.set_title(fac[1](lvl))
...
def series(ax, data, x, y, scales, legend_cols=True, **kwargs):
    ...
    :param scales: A list of "scales" to apply to each line series; each scale
        ...
        - ``kwvals`` is a list of values that the keyword argument will take;
          and
        - ``label_fmt`` is a format string for the legend keys.
    ...
    ::
       ...
       scales = [
           ...
           ('disp', 'color', brewer_qual('Set1', False), r'$k = {:.0f}$'),
    ...
    levels = [sorted(np.unique(data[col])) for col in cols]
    rng = range(len(cols))
    ...
    for ix, cols in enumerate(cols):
        args = [{} for _ in levels[ix]]
        for lix, val in enumerate(levels[ix]):
            kw = args[lix]
            kw.update(kwargs)
            kw.update({names[ix]: values[ix][lix],
                       'label': fmts[ix].format(levels[ix][lix])})
            if names[ix] != 'color':
                kw['color'] = 'black'

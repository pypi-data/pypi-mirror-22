...
class AR1TransitionDistribution(ConditionalDistribution):
    ...
    :math:`\pi({\bf Z}_{k+1}\vert {\bf Z}_{k}, \Theta)`
    ...
    Args:
      pi (:class:`Distribution<TransportMaps.Distributions.Distribution`):
        distribution :math:`\pi:\mathbb{R}^d\rightarrow\mathbb{R}`
    ...
    def grad_x_log_pdf(self, x, y, params=None, idxs_slice=slice(None,None,None)):
        ...
        out[:,:self.dim] = self.pi.grad_x_log_pdf(x - frw)
        out[:,self.dim:] = - np.einsum('...i,...ij->...j', out[:,:self.dim], gx_frw)
        return out

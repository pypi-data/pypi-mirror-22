...
class CropFaces:
  ...
  def cropAllSquaresHorizontal (self, img, imagePath):
    ...
    if face is not None:
      #print 'face = ' + str(face)
      pivotX = int(face.width / 2) + face.left
      pivotY = face.top + face.height
      left = pivotX - face.width
      right = pivotX + face.width
      top = pivotY - int(2.0*face.height)
      bottom = pivotY + int(2.0*face.height)

      left = left if left >= 0 else 0
      right = right if right <= img.width else img.width
      top = top if top >= 0 else 0
      bottom = bottom if right <= img.height else img.width
  ...
  def cropAllSquaresVertical (self, img, imagePath):
    ...
    if face is not None:
      ...
      pivotY = face.top + face.height
      left = pivotX - face.width
      top = pivotY - int(1.5*face.height)
      right = pivotX + face.width
      bottom = pivotY + int(1.5*face.height)
      newFile = self.saveCrop(img.crop((left, top, right, bottom)), imagePath, 'face', exif)
      self.cropAgainMoreSquares(newFile)

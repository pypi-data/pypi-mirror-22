...
class CropFaces:
  ...
  def cropHeadSquare (self, img, imagePath, exif):
    ...
    if face is not None:
      ...
      facesize = int(min(6*face.height, min(img.width, img.height)) / 6)

      left = pivotX - (3*facesize)
      right = pivotX + (3*facesize)
      top = pivotY - (2*facesize)
      bottom = pivotY + (4*facesize)
      ...
      if left < 0 or right > img.width or top < 0 or bottom > img.height:
        fScale = None
        if (right - left) > img.width:
          fScale = ((right - left) / img.width)
        elif (bottom - top) > img.height:
          fScale = ((bottom - top) / img.height)

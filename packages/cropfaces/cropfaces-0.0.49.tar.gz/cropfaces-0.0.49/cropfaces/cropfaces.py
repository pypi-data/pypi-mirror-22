...
class CropFaces:
  ...
  def cropAgainMoreSquares (self, imagePath):
    ...
    if size > 1200:
      ...
      self.saveCrop(img.crop((0, 0, newsize, newsize)), imagePath, 0, exif)
      self.saveCrop(img.crop((size - newsize, 0, size, newsize)), imagePath, 1, exif)
      self.saveCrop(img.crop((0, size - newsize, newsize, size)), imagePath, 2, exif)
      self.saveCrop(img.crop((size - newsize, size - newsize, newsize, size)), imagePath, 3, exif)

      newsize2 = int(size * 0.80)
      offset2 = int(size * 0.10)
      self.saveCrop(img.crop((offset2, 0, offset2 + newsize2, newsize2)), imagePath, 4, exif)
      self.saveCrop(img.crop((offset2, size - newsize2, offset2 + newsize2, size)), imagePath, 5, exif)
  ...
  def cropAllSquaresHorizontal (self, img, imagePath):
    ...
    for i in range(0, numSplits):
      ...
      newFile = self.saveCrop(cropped, imagePath, i, exif)

function facebook_pixel(a, b, c, d) {
    /**
     ...
     *
     * Vendored from https://connect.facebook.net/en_US/fbevents.js, 2017-03-08
     */
    'use strict';
    var e = "2.5.1"
        ...
        ya.prototype = {
            ...
            _append: function ab(bb, cb, db) {
                if (Object(cb) !== cb) {
                    ...
                    this._appendPrimitive(bb, za(cb));
            },
            _appendPrimitive: function ab(bb, cb) {
                if (cb != null)
                    this.list.push([bb, cb]);
            },
            ...
            each: function ab(bb) {
                var cb = this.list;
                for (var db = 0, eb = cb.length; db < eb; db++)
                    bb(cb[db][0], cb[db][1]);
            },
        ...
        var wa = /^[+-]?\d+(\.\d+)?$/
            ...
            , ya = 'currency_code'
            , za = {
            AED: 1, ARS: 1, AUD: 1, BOB: 1, BRL: 1, CAD: 1,
            CHF: 1, CLP: 1, CNY: 1, COP: 1, CRC: 1, CZK: 1,
            DKK: 1, EUR: 1, GBP: 1, GTQ: 1, HKD: 1, HNL: 1,
            HUF: 1, IDR: 1, ILS: 1, INR: 1, ISK: 1, JPY: 1,
            KRW: 1, MOP: 1, MXN: 1, MYR: 1, NIO: 1, NOK: 1,
            NZD: 1, PEN: 1, PHP: 1, PLN: 1, PYG: 1, QAR: 1,
            RON: 1, RUB: 1, SAR: 1, SEK: 1, SGD: 1, THB: 1,
            TRY: 1, TWD: 1, USD: 1, UYU: 1, VEF: 1, VND: 1,
            ZAR: 1
        }
            , ab = {
            value: {
        ...
        , s = a.fbq;
    if (!s)
        return p.logError('Pixel code is not installed correctly on this page');
    var t = Array.prototype.slice
        ...
        , da = function ua(va) {
        if (Object.keys)
            return Object.keys(va).length === 0;
        for (var wa in va)
    ...
    function ga(ua, va, wa) {
        if (u.call(aa, ua)) {
            if (va && da(aa[ua].userData)) {
                ...
                p.logError('Duplicate Pixel ID: ' + ua);
            return;
        }
        var xa = {
            agent: !!wa && wa.agent,
    ...
    function ja(ua, va) {
        ...
        for (var ya = 0, za = z.length; ya < za; ya++) {
            var ab = z[ya];
            if (xa && wa.allowDuplicatePageViews === false && ca[ab.id] === true)
                continue;
            na(ab, ua, va);
            if (xa)
                ca[ab.id] = true;
    ...
    function ma(ua, va) {
        for (var wa = 0, xa = z.length; wa < xa; wa++)
            na(z[wa], ua, va);
    }
    ...
    qa();
    if (w && x || a.fbq !== a._fbq)
        p.logWarning('Multiple pixels with conflicting versions were detected on this page');
    if (z.length > 1)
        p.logWarning('Multiple different pixels were detected on this page');
    (function ua() {
        if (s.disablePushState === true)
            return;
...
$(function () {
    ...
    var clean_url = document.location.toString().replace(/^(.*)\/order\/([A-Z0-9]+)\/([A-Z0-9]+)\/(.*)$/gi, "$1/order/CODE/secret/$4");
    var fake_location = {
        'href': clean_url
    };

    if ($("meta[data-tracking-provider=ga]").length) {
        ...
        ga('create', $("meta[data-tracking-provider=ga]").attr("data-id"), 'auto');
        ga('set', 'location', clean_url);
        ga('send', 'pageview');
        ga('set', 'anonymizeIp', true);

        if ($("meta[data-tracking-order-id]").length) {
            ga('require', 'ecommerce');
            ga('ecommerce:addTransaction', {
                'id': $("meta[data-tracking-order-id]").attr("data-tracking-order-id"),
                'affiliation': $("meta[data-tracking-event-name]").attr("data-tracking-event-name"),
                'currency': $("meta[data-tracking-event-currency]").attr("data-tracking-event-currency"),
                'revenue': $("meta[data-tracking-order-total]").attr("data-tracking-order-total"),
                'tax': $("meta[data-tracking-order-tax]").attr("data-tracking-order-tax"),
            });
            ga('ecommerce:send');
        }
    }

    if ($("meta[data-tracking-provider=fb]").length) {
        !function (f, b) {
            if (f.fbq) return;
            var n = function () {
                n.callMethod
            ...
            n.version = '2.0';
            n.queue = [];
            facebook_pixel(window, document, fake_location, history);
        }(window, document);


        fbq('init', $("meta[data-tracking-provider=fb]").attr("data-id"));
        fbq('track', 'PageView');

        if ($("meta[data-tracking-order-id]").length) {
            fbq('track', 'Purchase', {
                currency: $("meta[data-tracking-event-currency]").attr("data-tracking-event-currency"),
                value: $("meta[data-tracking-order-total]").attr("data-tracking-order-total"),
            });
        }

        if (location.href.search("/checkout/questions/") > 0) {

...
def getdf_fuwu(startdate,enddate,xzqh):
     ...
     sql="""
        ...
        ,ztsfdm ,qylx as '企业类型',jyfw as '经营范围'
     ...
     sql1="""
    with a as (%s)
    select a.*,b.word as 街道 from a left join test_dmyy.dbo.dz_sist_jd as b on a.ztsfdm=b.ztsfdm
    """%sql
     df=db_query(sql1)
     col=['统一社会信用代码','机构代码','注册号','工商号','机构名称', '机构类型', '法定代表人人', '固定电话', '移动电话', '机构地址', '成立日期', '注册资金',
       '职工人数', '机构简介', '所属行业', 'jjhydm', '新行业类别',  '街道','企业类型','经营范围']
     shift(df,col)
     return df
...
def getdf_xz(startdate,enddate,xzqh):
     ...
     sql0="""
        select xx1.jjhy as '行业大类',xx.* ,xx2.gb as '投资国别' from
        (select ztsfdm as entid,

        jgdm as '机构代码',zch as '注册号',tydm as '统一社会信用代码',fddbr as '法定代表人',
        jyfw as '项目简介',xzqh as '落户区',jglxdm as 机构类型,jgmc as '注册公司名称',
        convert(varchar(100),clrq,23) as '注册日期', qylx as '公司性质', 
        zgrs as '职工人数', zczj as '注册资金',hbzl as '货币种类' ,zczj_rmb as '注册资金_人民币' , 
        jjhydm as '经济行业代码', jjhy as '经济行业小类',gbdm , 
        jgdz as '机构地址', 
        ...
        left join basedb.dbo.dict_gb as xx2 on xx.gbdm=xx2.gbdm
     ...
     sql3="""with a as (%s) 
     select a.*,ttt.jjhy as '门类',b.word as 街道 from a  left join  test_dmyy.dbo.dz_sist_jd as b on a.ztsfdm=b.ztsfdm
     left join  basedb.dbo.dict_jjhy ttt on substring([经济行业代码],1,1)=ttt.jjhydm
     """%sql0
     ...
     dx=['主要投资方','次要合伙投资方','主要投资方法人(联系人)','项目投资总额（亿人民币）','行业大类',
        '项目简介','机构代码','注册号','统一社会信用代码','法定代表人',
...
def getdf_xzz(startdate,enddate,xzqh):
     ...
     sql0="""
         ...
         jyfw as '项目简介',xzqh as '落户区',jglxdm as 机构类型,jgmc as '注册公司名称',convert(varchar(100),clrq,23) as '注册日期', qylx as '公司性质', 
        zgrs as '职工人数', zczj as '注册资金',hbzl as '货币种类' ,zczj_rmb as '注册资金_人民币' , jjhydm as '经济行业代码', jjhy as '经济行业小类',gbdm , 
...
def getdf_daxing_nei(startdate,enddate,xzqh):
     ...
     sql0="""
        ...
        (select ztsfdm as entid,
        jgdm as '机构代码',zch as '注册号',tydm as '统一社会信用代码',
        jyfw as '项目简介',xzqh as '落户区',jgmc as '注册公司名称',convert(varchar(100),clrq,23) as '注册日期', qylx as '公司性质', 
        zgrs as '职工人数', zczj as '注册资金',hbzl as '货币种类' ,zczj_rmb as '注册资金_人民币' , jjhydm as '经济行业代码', jjhy as '经济行业小类','' as '投资国别', 
        jgdz as '机构地址', case when qylxdm like '[567w]%%' then '外资' else '内资' end as '内外资' 
        ,ztsfdm
        from basedb.dbo.t_zzjgjz where xzqhdm='%s' and zczj_rmb>=10000  and jglxdm in('1','2') 
        and clrq>='%s' and clrq<'%s' ) xx left join 
        (select jjhydm,jjhy from basedb.dbo.dict_jjhy) xx1 on substring(xx.[经济行业代码],1,1)=xx1.jjhydm
        """%(xzqh,startdate,enddate)
     sql1="""select entid ,shname as '主要投资方',fddbr as '主要投资方法人(联系人)'  from(%s) t where rn=1 """%(sql)
     sql2="""select entid ,shname as '次要合伙投资方' from(%s) t where rn>1 """%(sql)
     sql3="""with a as(select * from (%s) tt where tt.[内外资]='内资')
     select a.*,ttt.jjhy as '门类',b.word as 街道 from a left join test_dmyy.dbo.dz_sist_jd as b on a.ztsfdm=b.ztsfdm
     left join  basedb.dbo.dict_jjhy ttt on substring([经济行业代码],1,1)=ttt.jjhydm
     """%sql0
     df1,df2=db_query(sql1),db_query(sql2)
     df1=df1.groupby('entid').agg({'主要投资方':lambda x: ','.join(map(lambda i:'' if i is None else i,x)),
        '主要投资方法人(联系人)':lambda x:','.join(map(lambda i:'' if i is None else i,x))})
     df1['entid']=df1.index
     df1.index=range(len(df1))
     df2=df2.groupby('entid').agg({'次要合伙投资方':lambda x:','.join(map(lambda i:'' if i is None else i,x))})
     df2['entid']=df2.index
     ...
     dx=['主要投资方','次要合伙投资方','主要投资方法人(联系人)','内资项目投资总额（亿人民币）','行业大类',
        '机构代码','注册号','统一社会信用代码','项目简介',
        '落户区','注册公司名称','注册日期','公司性质','外资项目新增注册资金（万美元）','内资项目新增注册资金（万元人民币）', 
        '职工人数','注册资金','货币种类','注册资金_人民币','经济行业代码','经济行业小类','投资国别','机构地址','内外资','门类','街道']
     dfx=shift(dfx,dx)
...
def getdf_daxing_wai(startdate,enddate,xzqh):
     ...
     sql0="""
        ...
        jgdm as '机构代码',zch as '注册号',tydm as '统一社会信用代码',
         ...
         jgdz as '机构地址', case when qylxdm like '[567w]%%' then '外资' else '内资' end as '内外资' 
         ,ztsfdm
         from basedb.dbo.t_zzjgjz where xzqhdm='%s' and zczj_rmb>=3100 and jglxdm in('1','2') 
         and clrq>='%s' and clrq<'%s' ) xx left join 
         (select jjhydm,jjhy from basedb.dbo.dict_jjhy) xx1 on substring(xx.[经济行业代码],1,1)=xx1.jjhydm
         """%(xzqh,startdate,enddate)
     sql1="""select entid ,shname as '主要投资方',fddbr as '主要投资方法人(联系人)'  from(%s) t where rn=1 """%(sql)
     sql2="""select entid ,shname as '次要合伙投资方' from(%s) t where rn>1 """%(sql)
     sql3="""with a as (select * from (%s) tt where tt.[内外资]='外资')
     select a.*,b.word as 街道 from a left join test_dmyy.dbo.dz_sist_jd as b on a.ztsfdm=b.ztsfdm
     """%sql0
     df1,df2=db_query(sql1),db_query(sql2)
     ...
     dx=['主要投资方','次要合伙投资方','主要投资方法人(联系人)','项目投资总额（亿人民币）','行业大类',
        '机构代码','注册号','统一社会信用代码','项目简介',
        '落户区','注册公司名称','注册日期','公司性质','外资项目新增注册资金（万美元）','内资项目新增注册资金（万元人民币）', 
        '职工人数','注册资金','货币种类','注册资金_人民币','经济行业代码','经济行业小类','投资国别','机构地址','内外资','街道']
     dfx=shift(dfx,dx)
...
def getdf_nei(startdate,enddate,xzqh):
     ...
     sql4="""with a as (
            ...
            jgmc as '企业名称',zczj_rmb as '注册资金',hbzl as '货币种类',qylx as '企业类型',substring(t1.jjhydm,1,1) as '门类代码',t2.jjhy as 门类,
                        t1.jjhy as '行业类型' 
            ,ztsfdm,zgrs as '职工人数',jyfw as '经营范围'
            from basedb.dbo.t_zzjgjz t1 left join  basedb.dbo.dict_jjhy t2 on substring(t1.jjhydm,1,1)=t2.jjhydm
                        where jglxdm in('1','2')  and ztsfdm in(%s) )
            select a.*,b.word as 街道 from a left join test_dmyy.dbo.dz_sist_jd as b on a.ztsfdm=b.ztsfdm
            """%sql0
     df3=db_query(sql4)
     df1=db_query(sql3)
     ...
     a=['统一社会信用代码','组织机构代码','注册号','企业名称','注册资金','货币种类','企业类型','股东信息','变更日期','行业类型',
     '变更前地址','变更后地址','迁移状态','门类代码','门类','街道','职工人数','经营范围']
     df=shift(df,a)
...
def getdf_wai(startdate,enddate,xzqh):
     ...
     sql1="""select EntID as entid ,EntSCCode as '统一社会信用代码',EntOrgCode as '组织机构代码',EntRegNO as '注册号', 
        ...
        left join  test_dmyy.dbo.dz_sist_jd as abc5
        on a.entid=abc5.ztsfdm
        where  
        Addr like '%%%s%%' and SUBSTRING(EntTypeCode,1,1)!='B' and  
        (EntStatusCode=-3 OR  Memo like '%%迁出%%'OR  Memo like '%%迁移至%%' or memo like '%%迁移%%' ) and entstatuscode=0 
     ...
     sql_ext1="select ztsfdm as entid, zgrs as '职工人数',jyfw as '经营范围' from basedb.dbo.t_zzjgjz where ztsfdm in (select entid from (%s)tt )"%sql1
     df_ext1=db_query(sql_ext1)
     df=pd.merge(df,df_ext1,on='entid',how='left')



     a1=['统一社会信用代码', '组织机构代码', '注册号','企业名称','注册地址','注册资金','货币种类','企业类型','股东信息','核准日期','经济行业'
     ,'迁移信息','主体记录状态','街道','职工人数','经营范围']
     df=shift(df,a1)
...
def getdf_sb(enddate,xzqh):
      ...
      sql2="""
            select  ztsfdm,jgdm,jgmc,zczj,hbzl,zczj_rmb,qylx,jjhy ,zgrs,jyfw,tydm,zch

            from basedb.dbo.t_zzjgjz where ((jgdm in(select jgdm from(%s) tt )) ) 
      ...
      sqlx="""
            select ztsfdm,a.jgdm,a.jgmc,zczj,hbzl,zczj_rmb,qylx,jjhy,zgrs,jyfw,tydm,zch,rq,b.t3,b.t2,b.t4 from
            (select  ztsfdm,jgdm,jgmc,zczj,hbzl,zczj_rmb,qylx,jjhy,zgrs,jyfw,tydm,zch,convert(varchar(100),clrq,23) as rq from basedb.dbo.t_zzjgjz where  
      ...
      sql3="""with a as (
            ...
            select a.*,b.word as 街道 from a left join dz_sist_jd as b on a.entid=b.ztsfdm
            """%(sql2)
      ...
      sqly2="""
            ...
            from basedb.dbo.t_zzjgjz where ((jgmc in(select jgmc from(%s) tt )) ) 
            and ztztdm>=0 and jglxdm in('1','2')  
            and xzqhdm='%s' and ((qylxdm not like '[567w]%%' and zczj_rmb>=10000) or (qylxdm like '[567w]%%' and zczj_rmb>=3100))
            """%(sqly1,xzqh)
      ...
      df=df.groupby('entid').agg({'sh':lambda x: ','.join(map(lambda i:'' if i is None else i,x)),'街道':lambda x:x.iat[0]})
      df['ztsfdm']=df.index
      ...
      df.insert(len(df.columns)-1,'街道',df.pop('街道'))
      df.columns=['机构代码','机构名称','注册资金','货币种类','注册资金_人民币','企业类型','股东信息','所属行业','职工人数',
      '经营范围','统一代码','注册号','成立日期','开始投保年月','最后投保年月','社保缴纳情况','街道']
...
def getdf_gjj(enddate,xzqh):
      ...
      sql="""
           ...
           ,datediff(mm,max(lastpaydate),'%s')-1 t4 
            ...
            """%(enddate,enddate)
      sql2="""
            select  ztsfdm,jgdm,jgmc,zczj,hbzl,zczj_rmb,qylx,jjhy,zgrs,jyfw,tydm,zch from basedb.dbo.t_zzjgjz where (jgdm in(select jgdm from(%s) tt )) 
            and ztztdm>=0 and jglxdm in('1','2')  
             and xzqhdm='%s' and ((qylxdm not like '[567w]%%' and zczj_rmb>=10000) or (qylxdm like '[567w]%%' and zczj_rmb>=3100))
            """%(sql,xzqh)
      sqlx="""
      ...
      df.columns=['机构代码','机构名称','注册资金','货币种类','注册资金_人民币','企业类型','股东信息','所属行业','职工人数','经营范围',
                '统一代码','注册号','成立日期',
                '公积金开始缴交年月','公积金最后缴交年月','公积金缴纳情况','街道']
...
def getdf_zz(startdate,enddate,xzqh):
     ...
     sql3="""
         ...
         +ltrim(str([出资比例],9,4))+'%%}'  
            from (%s) t  """%(sql2)
     sql4="""with a as (select ztsfdm as entid,tydm as '统一社会信用代码',jgdm as '组织机构代码',zch as '注册号', 
            jgmc as '企业名称',fddbr '法人名称',jgdz as '机构地址',jyfw as '经营范围',convert(varchar(50),clrq,23) as '成立日期',convert(varchar(50),jyqx,23) as '经营期限止',
            hbzl as '货币种类',qylx as '企业类型',jjhydm as '行业代码',jjhy as '行业类型'
            ,zgrs as '职工人数'
             from basedb.dbo.t_zzjgjz 
            where jglxdm in('1','2') and ztsfdm in(%s) )
...
#---------#7扩展
def waizi_count_peryear(date,xzqh):
    startdate=date
    date1=datetime.datetime.strptime(date,'%Y-%m-%d').date()
    date=add_months(date1,1)
...
def waizi_zczj_peryear(date,xzqh):
    startdate=date
    date1=datetime.datetime.strptime(date,'%Y-%m-%d').date()
    date=add_months(date1,1)
    enddate=datetime.datetime.strftime(date,'%Y-%m-%d')
...
def waizi_countANDzczj_mutiyear(enddate,xzqh):
    dates=[]
    date1=datetime.datetime.strptime(enddate,'%Y-%m-%d').date()
    for i in range(-6,0):
          date=add_months(date1,i)
          ...
          dates.append(date)
    print(dates)
    data=[(date,waizi_count_peryear(date,xzqh),waizi_zczj_peryear(date,xzqh)) for date in dates]
    df=pd.DataFrame(data,columns=['date','count','zczj_rmb'])
    df['countP']=df['count'].pct_change()
    df['zczjP']=df['zczj_rmb'].pct_change()
    return df

def neizi_count_peryear(date,xzqh):
    startdate=date
    date1=datetime.datetime.strptime(date,'%Y-%m-%d').date()
    date=add_months(date1,1)
    enddate=datetime.datetime.strftime(date,'%Y-%m-%d')
...
def neizi_zczj_peryear(date,xzqh):
    startdate=date
    date1=datetime.datetime.strptime(date,'%Y-%m-%d').date()
    date=add_months(date1,1)
    enddate=datetime.datetime.strftime(date,'%Y-%m-%d')
...
def neizi_countANDzczj_mutiyear(enddate,xzqh):
    dates=[]
    date1=datetime.datetime.strptime(enddate,'%Y-%m-%d').date()
    for i in range(-6,0):
          date=add_months(date1,i)
          ...
          dates.append(date)
    print(dates)
    data=[(date,neizi_count_peryear(date,xzqh),neizi_zczj_peryear(date,xzqh)) for date in dates]
...
#------7扩展
#国内外来神投资企业的明细：统一社会信用代码   组织机构代码  注册号   企业名称   注册资金   货币种类     职工人数    
#企业类型    股东信息    经济行业中文名称   经济行业代码   注册地址   街道    经营范围    投资来源地

def waizi_detail_peryear(date,xzqh):
    startdate=date
    date1=datetime.datetime.strptime(date,'%Y-%m-%d').date()
    date=add_months(date1,1)
    enddate=datetime.datetime.strftime(date,'%Y-%m-%d')
    sql="""with a1 as (
    select tydm,jgdm,zch,ztsfdm as entid,jgmc,zczj_rmb,hbzl,zgrs,qylx,convert(varchar(50),clrq,23) as '成立日期'
    ,jjhy,jjhydm,jgdz,jyfw,gbdm
     from basedb.dbo.t_zzjgjz
    where clrq>='%s' and clrq<'%s' and xzqh='%s'
    and jglxdm in ('1','2') and qylxdm like '[567w]%%')
    , a as (select a1.*,t.gb as '投资来源地' from a1 left join basedb.dbo.dict_gb t on a1.gbdm=t.gbdm)
    select a.*,b.word as 街道 from a left join dz_sist_jd as b on a.entid=b.ztsfdm
    """%(startdate,enddate,xzqh)
    df=db_query(sql)
    ...
    where clrq>='%s' and clrq<'%s' and xzqh='%s'
    and jglxdm in ('1','2') and qylxdm like '[567w]%%')
    select entid,'{'+shname+'-'+ltrim(str(capamt,9,2))+'-'+ltrim(str(capprop,9,2))+'%%}' as sh
    from regdb.dbo.tdsh where entid in(select entid from a) and entstatuscode<=0
    """%(startdate,enddate,xzqh)
    df1=db_query(sql)
    ...
    df1=df1.groupby('entid').agg({'sh':lambda x:','.join(map(lambda i:'' if i is None else i,x))})
    df1['entid']=df1.index
    df1.index=range(len(df1))
    df=pd.merge(df,df1,on='entid',how='left')
    df.rename(index=str, columns={"tydm": "统一代码", 
        "jgdm": "机构代码","zch":"注册号","entid":"工商号","jgmc":"企业名称","zczj_rmb":"注册资金",
        "hbzl":"货币种类","zgrs":"职工人数","qylx":"企业类型","jjhy":"经济类型",
        "jjhydm":"经济行业代码","jgdz":"机构地址","jyfw":"经营范围","sh":"股东信息"},inplace=True)


    return df


def neizi_detail_peryear(date,xzqh):
    startdate=date
    date1=datetime.datetime.strptime(date,'%Y-%m-%d').date()
    date=add_months(date1,1)
    enddate=datetime.datetime.strftime(date,'%Y-%m-%d')
    ...
    ,c as (
        select tydm,jgdm,zch,ztsfdm as entid,jgmc,zczj_rmb,hbzl,zgrs,qylx,convert(varchar(50),clrq,23) as '成立日期' 
        ,jjhy,jjhydm,jgdz,jyfw
         from basedb.dbo.t_zzjgjz  where ztsfdm in(select entid from b)
    )
    select c.*,t.word as 街道 from c left join dz_sist_jd as t on c.entid=t.ztsfdm
    """%(startdate,enddate,xzqh)
    df=db_query(sql)
    ...
    and len(shname)>3)
    select entid,'{'+shname+'-'+ltrim(str(capamt,9,2))+'-'+ltrim(str(capprop,9,2))+'%%}' as sh
    from regdb.dbo.tdsh where entid in(select entid from b)
    """%(startdate,enddate,xzqh)
    df1=db_query(sql)
...
def t_qianyi(startdate,enddate,path,xzqh,qianyi='迁出'):
     ...
     a=['统一社会信用代码','组织机构代码','注册号','企业名称','注册资金','货币种类','企业类型','股东信息','变更日期','门类代码','门类','行业类型','变更前地址'
     ,'变更后地址','迁移状态']
     a1=['统一社会信用代码', '组织机构代码', '注册号','企业名称','注册地址','注册资金','货币种类','企业类型','股东信息','核准日期','经济行业'
     ,'迁移信息','主体记录状态']
     #df1=shift(df1,a)
...
def t_state(startdate,enddate,path,xzqh='福田区'):
      ...
      df6_1=waizi_countANDzczj_mutiyear(enddate,xzqh)
      df6_2=neizi_countANDzczj_mutiyear(enddate,xzqh)
      date=enddate
      date=datetime.datetime.strptime(date,'%Y-%m-%d').date()
      date=add_months(date,-1)
      date=datetime.datetime.strftime(date,'%Y-%m-%d')
      df7=waizi_detail_peryear(date,xzqh)
      df8=neizi_detail_peryear(date,xzqh)
      dfs=[[df1],[df2],df3,[df4],[df5],[df6_1,df6_2],[df7],[df8]]
      sheets=['总体概况','新增企业同比','内外资 资金总额等','新增企业行业的排名','当月行业-内外资统计','国外内来深投资资数量资金(月同比)'
      ,'国外企业来深投资明细','国内非深企业来深投资明细']

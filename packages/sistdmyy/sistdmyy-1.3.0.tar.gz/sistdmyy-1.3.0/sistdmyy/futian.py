...
def getdf_fuwu(startdate,enddate,xzqh):
    ...
    """%sql
     df=db_query(sql1)
     col=['机构名称', '机构类型', '法定代表人人', '固定电话', '移动电话', '机构地址', '成立日期', '注册资金',
...
def getdf_sb(enddate,xzqh):
      ...
      sql2="""
            select  ztsfdm,jgdm,jgmc,zczj,hbzl,zczj_rmb,qylx,jjhy ,zgrs,jyfw
      ...
      sqlx="""
            select ztsfdm,a.jgdm,a.jgmc,zczj,hbzl,zczj_rmb,qylx,jjhy,zgrs,jyfw,rq,b.t3,b.t2,b.t4 from
            (select  ztsfdm,jgdm,jgmc,zczj,hbzl,zczj_rmb,qylx,jjhy,zgrs,jyfw,convert(varchar(100),clrq,23) as rq from basedb.dbo.t_zzjgjz where  
      ...
      '经营范围','成立日期','开始投保年月','最后投保年月','社保缴纳情况','街道']
...
def getdf_gjj(enddate,xzqh):
      ...
      sql="""
           ...
           ,datediff(mm,max(lastpaydate),'%s')-1 t4 
            ...
            """%(enddate,enddate)
      sql2="""
            select  ztsfdm,jgdm,jgmc,zczj,hbzl,zczj_rmb,qylx,jjhy,zgrs,jyfw from basedb.dbo.t_zzjgjz where (jgdm in(select jgdm from(%s) tt )) 
            and ztztdm>=0 and jglxdm in('1','2')  
             and xzqhdm='%s' and ((qylxdm not like '[567w]%%' and zczj_rmb>=10000) or (qylxdm like '[567w]%%' and zczj_rmb>=3100))
            """%(sql,xzqh)
      sqlx="""
      ...
      df.columns=['机构代码','机构名称','注册资金','货币种类','注册资金_人民币','企业类型','股东信息','所属行业','职工人数','经营范围','成立日期',
...
def t_state(startdate,enddate,path,xzqh='福田区'):
      ...
      dfs=[[df1],[df2],df3,[df4],[df5],[df6_1,df6_2]]
      sheets=['总体概况','新增企业同比','内外资 资金总额等','新增企业行业的排名','当月行业-内外资统计','外内资数量资金(月同比)']

...
class Appliance(object):
    ...
    def get_appliance_type(self):
        ...
        try:
            ...
            out, err, rc = self.raw('rpm -qa | grep actsys | grep -ho -e svc -e psky -e sky', statuses=[0])
            try:
            ...
            if appliance_type not in appl_types:
                msg = "Appliance type found from rpm -qa isn't {0}".format(appl_types_str)
            ...
            for di in r['result']:
                for v in di.values():
                    if v.lower() in appl_types:
                        appliance_type = v.lower()
                        break

        assert appliance_type in appl_types, "Invalid appliance type {0} found".format(appliance_type)
        self.logger.info("Successfully got appliance type {}.".format(appliance_type))
        self._appliance_type = str(appliance_type)
    ...
    def get_version(self):
        ...
        self._version = r.parse(k='version', m_k='component', m_v='srv-revision')
        if not self._version:
            logging.debug('This system looks older and may call srv-revision psrv-revision, attempting to grab.')
            self._version = r.parse(k='version', m_k='component', m_v='psrv-revision')
        logging.info('srv-version is {}'.format(self._version))

...
class ApplianceSsh(ApplianceConnection):
    ...
    def _raw(self, ssh_command, check_rc=False):
        c = self.ActCmd()
        c.ssh_command = ssh_command
        self.call(c)
        c.stdout = self.strip_newlines(c.stdout)
        c.stderr = self.strip_newlines(c.stderr)
        if check_rc and c.exit_status != 0:
            raise RuntimeError("Exit status of command {} was {}.".format(ssh_command, c.exit_status))
        return c.stdout, c.stderr, c.exit_status

    def raw(self, ssh_command, check_rc=False):
        return self._timeout(self._raw, ssh_command, check_rc)

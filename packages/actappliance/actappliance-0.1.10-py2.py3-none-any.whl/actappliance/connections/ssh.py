...
class ApplianceSsh(ApplianceConnection):
    ...
    def post_cmd(self, command_obj):
        command_obj.result = self.convert_ssh_output(command_obj.pre.out, command_obj.pre.err)
        return super(ApplianceSsh, self).post_cmd(command_obj)

    def raw(self, ssh_command, statuses=None):
        """
        Send a command with the only processing being newline stripping.
        ...
        out = strip_newlines(c.call_result.out)
        err = strip_newlines(c.call_result.err)
        output = ssh_out(out, err, c.call_result.status)

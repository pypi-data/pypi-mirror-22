...
class WykopAPIv2(BaseWykopAPI):
    ...
    def login(self, login, accountkey=None, password=None):
        post_params = {'login': login}
    ...
    def get_api_params(self, **api_params):
        ...
        return api_params_all

    def get_path(self, *rmethod_params, **api_params):
        """
        Gets request path.
        """
        method_params = self.get_method_params(*rmethod_params)
        ...
        pathparts = method_params + api_params
        return '/'.join(pathparts)

    def request(self, *rmethod, rmethod_params=[],
                api_params={}, post_params={}, file_params={},
                parser=default_parser, requester=default_requester):
        """
        Makes request.
        ...
        # sanitize data
        rmethod = map(force_text, rmethod)
        post_params = dictmap(force_bytes, post_params)
        api_params = dictmap(force_text, api_params)

...
class BlockRule(object):
    ...
    def __init__(self, ips, rids, sensors_matcher, action, exact_blocked_paths, starts_with_blocked_paths):
        self.ips = ips
        self.rids = rids
    ...
    def from_json(cls, rule_json):
        ...
        if action in cls.ACTIONS_TO_RESPONSES:
            ...
            if not ips and not rids and len(exact_blocked_paths) == 0 and len(starts_with_blocked_paths) == 0:
                LOGGER.error("Patches Policy block rule cannot be global. Specify either ips and/or route ids or blocked paths")

                return None

            sensors_matcher = SensorsMatcher.from_json(rule_json.get("sensor_matches", {}))

            return BlockRule(ips, rids, sensors_matcher, action, exact_blocked_paths, starts_with_blocked_paths)
    ...
    def __str__(self):
        return "<{} {}>".format(
            ...
            "".join([' {}: {}'.format(k, v) for k, v in [
                ("ips", self.ips),
                ("rids", self.rids),
                ("sensors", self.sensors_matcher),
                ("action", self.action),
                ("exact_blocked_paths", self.exact_blocked_paths),
                ("starts_with_blocked_paths", self.starts_with_blocked_paths)]]))

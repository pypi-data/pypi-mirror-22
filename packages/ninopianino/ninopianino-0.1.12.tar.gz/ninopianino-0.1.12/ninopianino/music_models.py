...
class Key:
    ...
    def __init__(self, root_note = 'A', scale = 'minor', b_notes = [], notes_bias = {}, low_end = 'A2', high_end = 'G#6', markov_values = None):
        ...
        self.notes = [[x] * self.base_notes.count(x[:-1]) for x in notes_list if x[:-1] in self.base_notes and notes_list.index(low_end) - 1 <= notes_list.index(x) <= notes_list.index(high_end) + 1]
        self.notes = [item for sublist in self.notes for item in sublist]

#        print 'For ', self.base_notes, ' I got notes : ', self.notes
    ...
    def generate_note_markov(self, prev_note):
        ...
        for note in prev_notes: 
            note_table = note_table[note % len(note_table)]

...
class Pane:
    ...
    def __init__(self):
        ...
        [self.set_before_keyboad_listener(i,self._count)  for  i in '123456789']
        [self.set_move_keyboard_listener(i,self.on_move) for i in 'hjkl' ]

        # self.key_map_move['j'] = move
    def _count(self, panel, key):
        self.cc = int(key)
    ...
    def show(self, d = ' | '):
        ...
        # L(self.loc,self.locs,r=self.h+1,c=30,)

    def on_call(self, k):
        f = self.key_map.get(k)
        if f:
            return f(self, k)

    def after_call(self,k):
        f = self.key_map_after.get(k)
        if f:
            return f(self, k)

    def before_call(self, k):
        f = self.key_map_before.get(k)
        if f:
            return f(self, k)  

    def move_call(self, k):   
        f = self.key_map_move.get(k)
        if f:
            return f(self, k)
    ...
    def set_before_keyboad_listener(self, k, f):
        self.key_map_before[k] = f

    def set_on_keyboard_listener(self, k, f):
        """
            set k ,f 
                f(self)
        """
    ...
    def set_after_keyboard_listener(self,k, f):
        self.key_map_after[k] = f

    def set_move_keyboard_listener(self, k, f):
        self.key_map_move[k] = f

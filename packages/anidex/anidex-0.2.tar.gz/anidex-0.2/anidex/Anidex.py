...
class Anidex(object):
    ...
    def connect(self):
        fields = {
            'login_username': (None, self.login),
            'login_password': (None, self.password)
        }
        self.session.post(constants.LOGIN_URL,
                          headers=constants.HEADERS,
                          files=fields)
        userpage = self.session.get(constants.USERPAGE_URL,
                                    headers=constants.HEADERS)
        if self.login not in userpage.text:
            raise Exception(constants.ERROR_CRENDENTIALS)
        self.connected = True
    ...
    def search(self, query=None, category='ALL', batch=True, raw=True,
        ...
        payload['category'] = helpers.translate_category(category)
        payload['batch'] = batch
        payload['raw'] = raw

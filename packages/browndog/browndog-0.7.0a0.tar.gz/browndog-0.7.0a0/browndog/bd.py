...
def get_token(url, key):
    ...
    if r.status_code == 200:
        ...
        return token['token']
    elif r.status_code == 404:  # If response is 404, return None to indicate that the key was deleted.
        return None
    else:
        return ''
...
def check_token_validity(bds, token):
    ...
    @return: Result of checking token validity (e.g. {"found": "true", "ttl": 63858}), if the request succeeds. None,
    in all other cases.
    """
    api_call = bds + '/tokens/' + token
    r = requests.get(api_call, headers={'Accept': 'application/json'})
...
def outputs(bds, input_format, token):
    ...
    """
    api_call = bds + '/dap/inputs/' + input_format
...
def extractors(bds, input_format, token):
    ...
    @param token: Brown Dog access token
    @return: A string array of available extractions.
    """
    (bds, username, password) = split_url(bds)
...
def extract(bds, input_filepath, token, wait=60):
    ...
    if input_filepath.startswith('http://') or input_filepath.startswith('https://'):
        data = {"fileurl": input_filepath}
        file_id = requests.post(bds + '/dts/api/extractions/upload_url',
                                headers={'Content-Type': 'application/json', 'Accept': 'application/json',
                                         'Authorization': token}, data=json.dumps(data)).json()['id']
    else:
        # print requests.post(bds + '/dts/api/extractions/upload_file',
        #                     headers={'Accept': 'application/json', 'Authorization': token},
        #                     files={'File' : (os.path.basename(file), open(file))})
    ...
    with open(output_filename, 'w') as f:
        for filename in os.listdir(directory):
            if not filename[0] == '.':
                ...
                metadata = extract(bds, directory + filename, token, wait)

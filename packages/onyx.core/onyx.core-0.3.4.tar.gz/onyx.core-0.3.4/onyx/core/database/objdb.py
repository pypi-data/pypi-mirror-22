...
DROP TABLE IF EXISTS ClassDefinitions CASCADE;""")
    ...
    def set_dated(self, name, attr, date, value, overwrite=False):
        ...
        with self.conn.cursor() as curs:
            ...
            record = (changed_by, time_stamp, value, name, attr, date)
    ...
    def get_dated(self, name, attr, date, strict=False):
        ...
        try:
            ...
            return date, json.loads(self[record_id], cls=custom_decoder)
        except KeyError:
            if strict:
            ...
            self[record_id] = row.value
            value = json.loads(row.value, cls=custom_decoder)

            if arc_date != date:
                # --- cache the value for the actual archive date too
                self[(name, attr, arc_date)] = row.value

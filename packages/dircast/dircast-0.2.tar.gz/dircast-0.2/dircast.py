...
class Cast:
    ...
    def make_cast_by_dir(self, path_to_dir, indicate='no'):
        ...
        i = 0
        progress(indicate, 'scan: ' + str(i), delay=False)
        es_total = 0
        es_current = 0
        ...
        progress(indicate, 'calc: ' + str(es_current), delay=False)
...
def progress(indicate, value, delay=True):
    global progress_old_dt
    if delay and indicate != 'no':
        if progress_old_dt == 0:
            progress_old_dt = datetime.datetime.now()
            return
        else:
            if datetime.datetime.now() - progress_old_dt < datetime.timedelta(milliseconds=500):
                return
            else:
                progress_old_dt = datetime.datetime.now()
    if indicate == 'cl':
    ...
    if sys.argv[1] == 'makecast':
        ...
        with open(sys.argv[3], mode='wt', encoding='utf-8') as f:
            for x in range(0, len(dups)):
                size_int = cst.cast[dups[x][0]][ES]
                if size_int > 999999999:
                    size_str = '{0:.2f} GiB'.format(round(size_int / 1073741824, 2))
                elif size_int > 999999:
                    size_str = '{0:.2f} MiB'.format(round(size_int / 1048576, 2))
                elif size_int > 999:
                    size_str = '{0:.2f} KiB'.format(round(size_int / 1024, 2))
                else:
                    size_str = str(size_int) + ' B'
                f.write(size_str + '\n')

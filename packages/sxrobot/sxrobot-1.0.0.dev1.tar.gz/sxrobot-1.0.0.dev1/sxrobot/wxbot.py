...
class WXBot:
        ...
        try:
            ...
            for group in self.group_list:
                if group['NickName'] == u'我要吃好吃的':
                    print "************************* 就是这个组 *************************"
                    reply = u'我要吃好吃的'
                    self.send_msg_by_uid(reply, group['UserName'])
                    pass
    ...
    def re_run(self):
        try:
            self.get_uuid()
            self.re_gen_qr_code(os.path.join(self.temp_pwd,'wxqr.png'))
            print '[INFO] Please use WeChat to scan the QR code .'
    ...
    def re_gen_qr_code(self, qr_file_path):
        string = 'https://login.weixin.qq.com/l/' + self.uuid
        ...
        if self.conf['qr'] == 'png':
            ...
            # show_image(qr_file_path)
            img = Image.open(qr_file_path)
            ttfont = ImageFont.truetype("/Library/Fonts/Arial.ttf", 20)
            draw = ImageDraw.Draw(img)
            draw.text((10, 10), u'Service connect error, please re-login!!', fill=0, font=ttfont)
            # draw.text((40, 40), unicode('服务出现异常', 'utf-8'), fill=0, font=ttfont)
            img.show()
        elif self.conf['qr'] == 'tty':
            print(qr.terminal(quiet_zone=1))

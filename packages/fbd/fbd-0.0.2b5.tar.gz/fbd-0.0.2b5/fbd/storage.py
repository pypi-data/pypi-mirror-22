...
class Storage:
    ...
    def save_event(self, event, commit=True):
        try:
            ...
            self.session.rollback()
            logging.error('Storage.save_event: {0}'.format(e))
    ...
    def save_topic(self, topic_dict, commit=True):
        try:
            ...
            self.session.rollback()
            logging.error('Storage.save_topic: {0}'.format(e))
    ...
    def save_place(self, place, commit=True):
        try:
            ...
            self.session.rollback()
            logging.error('Storage.save_place: {0}'.format(e))
    ...
    def update_place(self, place, commit=True):
        try:
            ...
            self.session.rollback()
            logging.error('Storage.update_place: {0}'.format(e))

...
class TestBadDependency(AssertBuildError, fixtures.BaseTestCase):
    ...
    def test(self):
        ...
        self.assertIn('references an unknown script "not existing script"', self.build_error)
...
class TestBadReference(AssertBuildError, fixtures.BaseTestCase):
    ...
    def test(self):
        self.assertIn("Reference to an unknown script: 'foo'", self.build_error)
...
class TestTitleWithComma(AssertBuildError, fixtures.BaseTestCase):
    TEST_TXT = """
    Bad title
    =========

    .. patchdb:script:: So, what?
    """

    def test(self):
        self.assertIn('ID contains comma', self.build_error)
...
class TestCircularInclude(AssertBuildError, fixtures.BaseTestCase):
    ...
    def test(self):
        self.assertIn("circular include of file 'foo.txt'", self.build_error)

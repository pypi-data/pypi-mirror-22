...
class HsmFactory(object):
  ...
  def __init__(self,**kwargs):
    ...
    if 'sequence' in kwargs:
      if kwargs['sequence'] is True:
        self.trace_formatter = self.trace_for_sequence_diagrams
  ...
  @classmethod
  def trace_for_sequence_diagrams(cls, tm, state_n_0, state_n_1, event, payload=None, chart=None):
    s = ""
    ts = time.time()
    # only keep three points of precision with milliseconds (truncated)
    ...
    if payload is None:
      payload = None
      s = "[%s] [%s] event->%s() %s->%s" % (time_stamp, chart, event, state_n_0, state_n_1)
    else:
      s = "[%s] [%s] event->%s(%s) %s->%s" % (time_stamp, chart, event, payload, state_n_0, state_n_1)
    return s

...
class Hsm(object):
  ...
  def trace(self, *args):
    ...
    """
    trace_map = self.__format_reflection__(self.trace_buffer,
                                      self.format_trace_trace,
                                      *args)
    trace_list = list(trace_map)
    return trace_list
  ...
  def spy(self, *args):
    ...
    """
    spy_map = self.__format_reflection__(self.spy_buffer,
                                      self.format_spy_trace,
                                      *args )
    spy_list = list(spy_map)
    return list(spy_list)
  ...
  def __set_sub_states__(self, hsm_dict, parent=None):
    ...
    """
    for key, _dict in hsm_dict.items():
  ...
  def substates(self, state_name):
    ...
    if key in self.hsm_dict and "node" in self.hsm_dict[key]:
      ...
      for substate_blob in state_tree.get_sub_nodes(n):
        substates.append(substate_blob.d)
  ...
  def split(self):
    for k,v in self.hsm_dict.items():
      pp(k)
      pp(v)

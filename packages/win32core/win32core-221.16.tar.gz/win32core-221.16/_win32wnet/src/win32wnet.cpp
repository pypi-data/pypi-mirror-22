...
static PyObject *PyWNetAddConnection2(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (PyArg_ParseTuple(args, "iOO|OOOi", &Type, &obLocalName, &obRemoteName, &obProviderName, &obUsername, &obPassword, &flags))
	{
		...
		if (!PyWinObject_AsTCHAR(obLocalName, &tempNetResource.lpLocalName, TRUE) || !PyWinObject_AsTCHAR(obRemoteName, &tempNetResource.lpRemoteName, FALSE) || !PyWinObject_AsTCHAR(obProviderName, &tempNetResource.lpProvider, TRUE))
			goto done;
...
#endif
	Py_END_ALLOW_THREADS if (ErrorNo != NO_ERROR)
...
PyObject *PyWNetGetLastError(PyObject *self, PyObject *args)
{
	...
	Py_END_ALLOW_THREADS if (err == NO_ERROR) return Py_BuildValue("kNN", extendederr, PyWinObject_FromTCHAR(errstr), PyWinObject_FromTCHAR(provider));
...
PYWIN_MODULE_INIT_FUNC(_win32wnet)
{
	PYWIN_MODULE_INIT_PREPARE(_win32wnet, win32wnet_functions,

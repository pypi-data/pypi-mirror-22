...
BOOL PyWinObject_AsPRINTER_DEFAULTS(PyObject *obdefaults, PPRINTER_DEFAULTS pdefaults)
{
	...
	return PyArg_ParseTupleAndKeywords(dummy_tuple, obdefaults, printer_default_format, printer_default_keys,
		   ...
		   PyWinObject_AsDEVMODE(obdevmode, &pdefaults->pDevMode, TRUE) && PyWinObject_AsTCHAR(obDataType, &pdefaults->pDatatype, TRUE);
}
...
static PyObject *PyGetPrinter(PyObject *self, PyObject *args)
{
	...
	{ // @pyparm <o PyPrinterHANDLE>|hPrinter||handle to printer object as returned by <om win32print.OpenPrinter>
	...
	*pbuf = NULL;
	...
	case 2:
	{
		...
		ret = PyArg_ParseTupleAndKeywords(dummy_tuple, obinfo, pi2_format, pi2_keys,
			  ...
			  PyWinObject_AsTCHAR(obServerName, &pi2->pServerName, TRUE) && PyWinObject_AsTCHAR(obPrinterName, &pi2->pPrinterName, TRUE) && PyWinObject_AsTCHAR(obShareName, &pi2->pShareName, TRUE) && PyWinObject_AsTCHAR(obPortName, &pi2->pPortName, TRUE) && PyWinObject_AsTCHAR(obDriverName, &pi2->pDriverName, TRUE) && PyWinObject_AsTCHAR(obComment, &pi2->pComment, TRUE) && PyWinObject_AsTCHAR(obLocation, &pi2->pLocation, TRUE) && PyWinObject_AsDEVMODE(obDevMode, &pi2->pDevMode, FALSE) && PyWinObject_AsTCHAR(obSepFile, &pi2->pSepFile, TRUE) && PyWinObject_AsTCHAR(obPrintProcessor, &pi2->pPrintProcessor, TRUE) && PyWinObject_AsTCHAR(obDatatype, &pi2->pDatatype, TRUE) && PyWinObject_AsTCHAR(obParameters, &pi2->pParameters, TRUE) && PyWinObject_AsSECURITY_DESCRIPTOR(obSecurityDescriptor, &pi2->pSecurityDescriptor, TRUE);
		break;
	}
	case 3:
	{
		...
		ret = PyArg_ParseTupleAndKeywords(dummy_tuple, obinfo, pi3_format, pi3_keys, &obSecurityDescriptor) && PyWinObject_AsSECURITY_DESCRIPTOR(obSecurityDescriptor, &pi3->pSecurityDescriptor, FALSE);
		break;
	}
	case 4:
	{
		...
		ret = PyArg_ParseTupleAndKeywords(dummy_tuple, obinfo, pi4_format, pi4_keys,
			  ...
			  PyWinObject_AsTCHAR(obPrinterName, &pi4->pPrinterName, TRUE) && PyWinObject_AsTCHAR(obServerName, &pi4->pServerName, TRUE);
		break;
	}
	case 5:
	{
		...
		ret = PyArg_ParseTupleAndKeywords(dummy_tuple, obinfo, pi5_format, pi5_keys,
			  ...
			  PyWinObject_AsTCHAR(obPrinterName, &pi5->pPrinterName, TRUE) && PyWinObject_AsTCHAR(obPortName, &pi5->pPortName, TRUE);
		break;
	}
	case 7:
	{
	...
	case 8:
	case 9:
	{ //identical structs, 8 is for global defaults and 9 is for user defaults
		...
		ret = PyArg_ParseTupleAndKeywords(dummy_tuple, obinfo, pi8_format, pi8_keys, &obDevMode) && PyWinObject_AsDEVMODE(obDevMode, &pi8->pDevMode, FALSE);
		break;
	}
	default:
...
static PyObject *PyStartDocPrinter(PyObject *self, PyObject *args)
{
	...
	if (PyWinObject_AsTCHAR(obDocName, &pDocName, FALSE) && PyWinObject_AsTCHAR(obOutputFile, &pOutputFile, TRUE) && PyWinObject_AsTCHAR(obDatatype, &pDatatype, TRUE))
	{
		...
		Py_END_ALLOW_THREADS if (0 == JobID)
...
BOOL PyWinObject_AsDOCINFO(PyObject *obdocinfo, DOCINFO *di)
{
	...
	return PyArg_ParseTuple(obdocinfo, "OOOk", &obDocName, &obOutput, &obDataType, &di->fwType) && PyWinObject_AsTCHAR(obDocName, (TCHAR **)&di->lpszDocName, TRUE) && PyWinObject_AsTCHAR(obOutput, (TCHAR **)&di->lpszOutput, TRUE) && PyWinObject_AsTCHAR(obDataType, (TCHAR **)&di->lpszDatatype, TRUE);
}
	...
	*pbuf = NULL;
	...
	case 2:
		...
		if (PyArg_ParseTupleAndKeywords(dummy_tuple, pyjobinfo, job2_format, job2_keys,
			...
			PyWinObject_AsDEVMODE(obdevmode, &job2->pDevMode, TRUE) && PyWinObject_AsSECURITY_DESCRIPTOR(obsecurity_descriptor, &job2->pSecurityDescriptor, TRUE) && ((obsubmitted == Py_None) || PyWinObject_AsSYSTEMTIME(obsubmitted, &job2->Submitted)))
			ret = TRUE;
		break;
	case 3:
		if (!PyDict_Check(pyjobinfo))
...
static PyObject *PyDocumentProperties(PyObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "OO&OOOk:DocumentProperties", &obhwnd,
		...
		return NULL;
	if (PyWinObject_AsTCHAR(obdevicename, &devicename, FALSE) && PyWinObject_AsHANDLE(obhwnd, (HANDLE *)&hwnd) && PyWinObject_AsDEVMODE(obdmoutput, &dmoutput, TRUE) && PyWinObject_AsDEVMODE(obdminput, &dminput, TRUE))
	{
...
static PyObject *PyDeviceCapabilities(PyObject *self, PyObject *args)
{
	...
	case DC_BINNAMES:
	{ // returns array of 24-char strings
		TCHAR *retname;
...
static PyObject *PyDeletePrinterDriver(PyObject *self, PyObject *args)
{
	...
	if (PyArg_ParseTuple(args, "OOO:DeletePrinterDriver", &observername, &obenvironment, &obdrivername) && PyWinObject_AsWCHAR(observername, &servername, TRUE) && PyWinObject_AsWCHAR(obenvironment, &environment, TRUE) && PyWinObject_AsWCHAR(obdrivername, &drivername, FALSE))
...
static PyObject *PyDeletePrinterDriverEx(PyObject *self, PyObject *args)
{
	...
	if (PyArg_ParseTuple(args, "OOOll:DeletePrinterDriverEx", &observername, &obenvironment, &obdrivername,
		...
		PyWinObject_AsWCHAR(observername, &servername, TRUE) && PyWinObject_AsWCHAR(obenvironment, &environment, TRUE) && PyWinObject_AsWCHAR(obdrivername, &drivername, FALSE))
...
PYWIN_MODULE_INIT_FUNC(_win32print)
{
	PYWIN_MODULE_INIT_PREPARE(_win32print, win32print_functions,

...
BOOL PyWinObject_AsPROFILEINFO(PyObject *ob, LPPROFILEINFO pi)
{
	...
	ret = PyArg_ParseTupleAndKeywords(dummy_args, ob, "O|kOOOOO:LoadUserProfile", elements,
		  ...
		  PyWinObject_AsWCHAR(obUserName, &pi->lpUserName, FALSE) && PyWinObject_AsWCHAR(obProfilePath, &pi->lpProfilePath, TRUE) && PyWinObject_AsWCHAR(obDefaultPath, &pi->lpDefaultPath, TRUE) && PyWinObject_AsWCHAR(obServerName, &pi->lpServerName, TRUE) && PyWinObject_AsWCHAR(obPolicyPath, &pi->lpPolicyPath, TRUE) && PyWinObject_AsHANDLE(obhProfile, &pi->hProfile);
...
PyObject *PyDeleteProfile(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|OO:DeleteProfile", keywords,
		...
		return NULL;
	if (PyWinObject_AsWCHAR(obstrsid, &strsid, FALSE) && PyWinObject_AsWCHAR(obprofile_path, &profile_path, TRUE) && PyWinObject_AsWCHAR(obmachine, &machine, TRUE))
	{
...
PYWIN_MODULE_INIT_FUNC(_win32profile)
{
	PYWIN_MODULE_INIT_PREPARE(_win32profile, win32profile_functions,

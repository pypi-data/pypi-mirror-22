...
typedef SERVICE_STATUS_HANDLE(WINAPI *REGSVC_EX_FN)(LPCTSTR lpServiceName,
...
static PyObject *PyRegisterServiceCtrlHandler(PyObject *self, PyObject *args)
{
	...
	if (bServiceDebug)
	{ // If debugging, get out now, and give None back.
		Py_INCREF(Py_None);
		return Py_None;
	}
...
PYWIN_MODULE_INIT_FUNC(_win32servicemanager)
{
	PYWIN_MODULE_INIT_PREPARE(_win32servicemanager, servicemanager_functions,
...
cleanup:
	...
	Py_XDECREF(start);
	Py_XDECREF(instance);
	if (pe && pe->sshStatusHandle)
	{ // Wont be true if debugging.
...
{
	if (pse->obServiceCtrlHandler == NULL)
	{ // Python is in error.
		if (!bServiceDebug)
...
static void ReportPythonError(DWORD code)
{
	if (PyErr_Occurred())
	{
		...
		if (bServiceDebug)
		{ // If debugging, restore for traceback print,

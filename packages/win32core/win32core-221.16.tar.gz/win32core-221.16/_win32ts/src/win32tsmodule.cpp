...
static PyObject *PyWTSQuerySessionInformation(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	{ // @flag WTSClientDisplay|Dict containing client's display settings
	...
	{   // @flag WTSClientAddress|Dict containing type and value of client's IP address (None if console session)
...
static PyObject *PyWTSSendMessage(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OkOOkkl:WTSSendMessage", keywords,
		...
		return NULL;
	if (PyWinObject_AsHANDLE(obh, &h) && PyWinObject_AsWCHAR(obTitle, &Title, FALSE, &TitleLen) && PyWinObject_AsWCHAR(obMessage, &Message, FALSE, &MessageLen))
	{
...
PYWIN_MODULE_INIT_FUNC(_win32ts)
{
	PYWIN_MODULE_INIT_PREPARE(_win32ts, win32ts_functions,

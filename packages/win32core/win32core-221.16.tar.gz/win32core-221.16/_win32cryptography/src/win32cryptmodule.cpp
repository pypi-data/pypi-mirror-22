...
static PyObject *PyCryptAcquireContext(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	else
	{
		...
		{ // when key is being deleted, no context is returned
			Py_INCREF(Py_None);
			ret = Py_None;
		}
		else
...
static PyObject *PyCryptDecodeMessage(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	else ret = Py_BuildValue("{s:k,s:k,s:N,s:N,s:N}",
							 "MsgType", msg_type,
							 "InnerContentType", inner_type,
...
static PyObject *PyCryptSignAndEncryptMessage(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!(PyWinObject_AsCRYPT_SIGN_MESSAGE_PARA(obcsmp, &csmp) && PyWinObject_AsCRYPT_ENCRYPT_MESSAGE_PARA(obcemp, &cemp) && PyWinObject_AsCERT_CONTEXTArray(obrecipients, &recipients, &recipient_cnt)))
		goto cleanup;
...
static PyObject *PyCryptVerifyMessageSignature(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	else
	{
		...
		else ret = Py_BuildValue("{s:N, s:N}",
								 "SignerCert", PyWinObject_FromCERT_CONTEXT(signer_cert),
								 "Decoded", PyString_FromStringAndSize((char *)output_buf, output_bufsize));
	}
	if (output_buf)
...
static PyObject *PyCryptDecryptAndVerifyMessageSignature(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	else ret = Py_BuildValue("{s:N,s:N,s:N}",
...
static PyObject *PyCryptEncodeObjectEx(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	{ // @flag X509_ENHANCED_KEY_USAGE|<o PyCTL_USAGE> (sequence of OID's)
		...
		if (!PyWinObject_AsCTL_USAGE(obstruct, (CTL_USAGE *)input_buf))
			goto cleanup;
			 ...
			 (structtype == X509_BITS))
	{ // @flag X509_BITS|<o PyCRYPT_BIT_BLOB>
...
static PyObject *PyCertNameToStr(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	Py_BEGIN_ALLOW_THREADS
		output_buflen = CertNameToStr(encoding, &cnb, strtype, output_buf, output_buflen);
	Py_END_ALLOW_THREADS if (output_buflen == 0)
...
PYWIN_MODULE_INIT_FUNC(_win32cryptography)
{
	PYWIN_MODULE_INIT_PREPARE(_win32cryptography, win32crypt_functions,
							  "Support for Windows cryptography functions");

	if (PyType_Ready(&PyCRYPTPROVType) == -1 || PyType_Ready(&PyCRYPTKEYType) == -1 || PyType_Ready(&PyCRYPTHASHType) == -1 || PyType_Ready(&PyCRYPTMSGType) == -1 || PyType_Ready(&PyCERTSTOREType) == -1 || PyType_Ready(&PyCERT_CONTEXTType) == -1 || PyType_Ready(&PyCTL_CONTEXTType) == -1)

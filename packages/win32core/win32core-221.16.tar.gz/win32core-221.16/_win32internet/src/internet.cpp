...
#define CHECK_IE_PFN(fname) \
	if (pfn##fname == NULL) \
		return PyErr_Format(PyExc_NotImplementedError, "%s is not available with this Internet Explorer version", #fname);
...
PYWIN_MODULE_INIT_FUNC(_win32internet)
{
	PYWIN_MODULE_INIT_PREPARE(_win32internet, internet_functions,

...
BOOL PyWinObject_AsCREDENTIAL_ATTRIBUTE(PyObject *obattr, PCREDENTIAL_ATTRIBUTE attr)
{
	...
	ret = PyArg_ParseTupleAndKeywords(args, obattr, "OkO:CREDENTIAL_ATTRIBUTE", keywords,
		  ...
		  PyWinObject_AsWCHAR(obKeyword, &attr->Keyword, FALSE) && PyWinObject_AsReadBuffer(obValue, &value, &valuelen) && ((attr->Value = (LPBYTE)malloc(valuelen)) != NULL);
	if (ret)
	{
...
BOOL PyWinObject_AsCREDENTIAL(PyObject *obcred, PCREDENTIAL cred)
{
	...
	ret = PyArg_ParseTupleAndKeywords(args, obcred, "|kkOOOOkOOO:CREDENTIAL", keywords,
		  ...
		  PyWinObject_AsWCHAR(obTargetName, &cred->TargetName, TRUE) && PyWinObject_AsWCHAR(obComment, &cred->Comment, TRUE) && ((obLastWritten == Py_None) || PyWinObject_AsFILETIME(obLastWritten, &cred->LastWritten)) && PyWinObject_AsWCHAR(obCredentialBlob, (WCHAR **)&cred->CredentialBlob, TRUE, &cred->CredentialBlobSize) && PyWinObject_AsCREDENTIAL_ATTRIBUTEArray(obAttributes, &cred->Attributes, &cred->AttributeCount) && PyWinObject_AsWCHAR(obTargetAlias, &cred->TargetAlias, TRUE) && PyWinObject_AsWCHAR(obUserName, &cred->UserName, TRUE);
...
BOOL PyWinObject_AsCREDENTIAL_TARGET_INFORMATION(PyObject *obtargetinfo, PCREDENTIAL_TARGET_INFORMATION targetinfo)
{
	...
	ret = PyArg_ParseTupleAndKeywords(args, obtargetinfo, "|OOOOOOOkO:CREDENTIAL_TARGET_INFORMATION", keywords,
		  ...
		  PyWinObject_AsWCHAR(obTargetName, &targetinfo->TargetName, TRUE) && PyWinObject_AsWCHAR(obNetbiosServerName, &targetinfo->NetbiosServerName, TRUE) && PyWinObject_AsWCHAR(obDnsServerName, &targetinfo->DnsServerName, TRUE) && PyWinObject_AsWCHAR(obNetbiosDomainName, &targetinfo->NetbiosDomainName, TRUE) && PyWinObject_AsWCHAR(obDnsDomainName, &targetinfo->DnsDomainName, TRUE) && PyWinObject_AsWCHAR(obDnsTreeName, &targetinfo->DnsTreeName, TRUE) && PyWinObject_AsWCHAR(obPackageName, &targetinfo->PackageName, TRUE) && PyWinObject_AsDWORDArray(cred_types, &targetinfo->CredTypes, &targetinfo->CredTypeCount, TRUE);
...
BOOL PyWinObject_AsCREDUI_INFO(PyObject *obuiinfo, PCREDUI_INFO *puiinfo)
{
	...
	*puiinfo = (PCREDUI_INFO)malloc(sizeof(CREDUI_INFO));
	...
	ret = (args != NULL) &&
		  ...
		  PyWinObject_AsWCHAR(obmessage, (WCHAR **)&(*puiinfo)->pszMessageText, TRUE) && PyWinObject_AsWCHAR(obcaption, (WCHAR **)&(*puiinfo)->pszCaptionText, TRUE) && PyWinObject_AsHANDLE(obparent, (HANDLE *)&(*puiinfo)->hwndParent) && PyWinObject_AsHANDLE(obbanner, (HANDLE *)&(*puiinfo)->hbmBanner);
...
PyObject *PyCredWriteDomainCredentials(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OO|k:CredWriteDomainCredentials", keywords,
		...
		return NULL;
	if (PyWinObject_AsCREDENTIAL_TARGET_INFORMATION(obtargetinfo, &targetinfo) && PyWinObject_AsCREDENTIAL(obcred, &cred))
	{
...
PyObject *PyCredRename(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOk|k:CredRename", keywords,
									 ...
									 &flags))		   // @pyparm int|Flags|0|Reserved, use only 0
		return NULL;
	if (PyWinObject_AsWCHAR(oboldtargetname, &oldtargetname, FALSE) && PyWinObject_AsWCHAR(obnewtargetname, &newtargetname, FALSE))
	{
...
PyObject *PyCredUIStoreSSOCredW(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOOk:CredUIStoreSSOCredW", keywords,
		...
		return NULL;
	if (PyWinObject_AsWCHAR(obrealm, &realm, TRUE) && PyWinObject_AsWCHAR(obusername, &username, FALSE) && PyWinObject_AsWCHAR(obpassword, &password, FALSE, &passwordlen))
	{
...
PYWIN_MODULE_INIT_FUNC(_win32credentials)
{
	PYWIN_MODULE_INIT_PREPARE(_win32credentials, win32cred_functions,

...
PYWIN_MODULE_INIT_FUNC(_win32kras)
{
	PYWIN_MODULE_INIT_PREPARE(_win32kras, win2kras_functions,
...
#ifdef _DEBUG
	const TCHAR *modName = _T("_win32ras_d.pyd");
#else
	const TCHAR *modName = _T("_win32ras.pyd");
#endif
	...
	if (hmod == NULL)
	{
		PyObject *tempMod = PyImport_ImportModule("_win32ras");
		Py_XDECREF(tempMod);

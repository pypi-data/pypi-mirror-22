...
PyObject *PyLoadPerfCounterTextStrings(PyObject *self, PyObject *args)
{
	BOOL bQuiet = 1;
	char *cmdLine;
	LONG(__stdcall * pfnLoadPerfCounterTextStringsA)
	(LPSTR lpAnsiCommandLine, BOOL bQuietModeArg);
...
PyObject *PyUnloadPerfCounterTextStrings(PyObject *self, PyObject *args)
{
	BOOL bQuiet = 1;
	char *cmdLine;
	LONG(__stdcall * pfnUnloadPerfCounterTextStringsA)
	(LPSTR lpServiceName, BOOL bQuietModeArg);
...
PYWIN_MODULE_INIT_FUNC(_win32perfmon)
{
	PYWIN_MODULE_INIT_PREPARE(_win32perfmon, perfmon_functions,
							  "Contains functions and objects wrapping the Performance Monitor APIs");
	if (PyType_Ready(&PyPerfMonManager::type) == -1 || PyType_Ready(&PyPERF_COUNTER_DEFINITION::type) == -1 || PyType_Ready(&PyPERF_OBJECT_TYPE::type) == -1)

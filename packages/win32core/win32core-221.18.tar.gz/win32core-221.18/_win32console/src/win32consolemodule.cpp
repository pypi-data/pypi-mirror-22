...
PyObject *PyConsoleScreenBuffer::PyScrollConsoleScreenBuffer(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOOOH:ScrollConsoleScreenBuffer", keywords,
		...
		return NULL;
	if (PyWinObject_AsSMALL_RECT(obscrollrect, &pscrollrect, FALSE) && PyWinObject_AsSMALL_RECT(obcliprect, &pcliprect, TRUE) && PyWinObject_AsCOORD(obdestcoord, &pdestcoord, FALSE) && PyWinObject_AsSingleWCHAR(obfillchar, &char_info.Char.UnicodeChar))
...
static PyObject *PyAddConsoleAlias(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (PyWinObject_AsWCHAR(obsource, &source, FALSE) && PyWinObject_AsWCHAR(obtarget, &target, TRUE) && PyWinObject_AsWCHAR(obexename, &exename, FALSE))
...
PyObject *PyGetConsoleTitle(PyObject *self, PyObject *args)
{
	...
	while (TRUE)
	{
		...
		{ // returned length does *not* includes the NULL terminator
...
PYWIN_MODULE_INIT_FUNC(_win32console)
{
	PYWIN_MODULE_INIT_PREPARE(_win32console, win32console_functions,

...
static PyObject *PyEnumObjectItems(PyObject *self, PyObject *args)
{
	...
	Py_END_ALLOW_THREADS
		...
		// PDH_MORE_DATA
		if (pdhStatus != ERROR_SUCCESS && pdhStatus != PDH_MORE_DATA) return PyWin_SetAPIError("EnumObjectItems for buffer size", pdhStatus);
...
static PyObject *PyEnumObjects(PyObject *self, PyObject *args)
{
	...
	Py_END_ALLOW_THREADS
		...
		// it appears NT/2k will return 0, while XP will return
		// PDH_MORE_DATA
		if (pdhStatus != ERROR_SUCCESS && pdhStatus != PDH_MORE_DATA) return PyWin_SetAPIError("EnumObjects for buffer size", pdhStatus);
...
static PyObject *PyGetCounterInfo(PyObject *self, PyObject *args)
{
	...
	Py_END_ALLOW_THREADS
		...
		if (pdhStatus != ERROR_SUCCESS && pdhStatus != PDH_MORE_DATA) return PyWin_SetAPIError("GetCounterInfo for size", pdhStatus);
...
static PyObject *PyGetFormattedCounterValue(PyObject *self, PyObject *args)
{
	...
	Py_END_ALLOW_THREADS if (pdhStatus != ERROR_SUCCESS) return PyWin_SetAPIError("GetFormattedCounterValue", pdhStatus);
...
PYWIN_MODULE_INIT_FUNC(_win32pdh)
{
	PYWIN_MODULE_INIT_PREPARE(_win32pdh, win32pdh_functions,

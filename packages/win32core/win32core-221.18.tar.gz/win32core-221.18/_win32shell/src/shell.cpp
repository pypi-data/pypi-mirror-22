	...
	*ppButtons = (TBBUTTON *)malloc(nButtons * sizeof(TBBUTTON));
	...
	for (UINT i = 0; i < nButtons; i++)
	{
		...
		ok = PyArg_ParseTuple(sub, "iiBB|OO",
							  ...
							  &obdata,
							  &obstring) &&
			 PyWinLong_AsVoidPtr(obdata, (void **)&pThis->dwData) && PyWinObject_AsResourceId(obstring, (TCHAR **)&pThis->iString);
		if (!ok)
			break;
	}
	if (!ok)
...
{
	PyObject *obhwnd;
	return PyArg_ParseTuple(obpmsg, "Oiiii(ii)", &obhwnd, &msg->message, &msg->wParam, &msg->lParam, &msg->time, &msg->pt.x, &msg->pt.y) && PyWinObject_AsHANDLE(obhwnd, (HANDLE *)&msg->hwnd);
}
...
PYWIN_MODULE_INIT_FUNC(_win32shell)
{
	PYWIN_MODULE_INIT_PREPARE(_win32shell, shell_methods,

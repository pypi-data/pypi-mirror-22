...
PYWIN_MODULE_INIT_FUNC(_win32help)
{
  PYWIN_MODULE_INIT_PREPARE(_win32help, win32help_functions,
  ...
  if (PyType_Ready(&PyHH_AKLINKType) == -1 || PyType_Ready(&PyHH_FTS_QUERYType) == -1 || PyType_Ready(&PyHH_POPUPType) == -1 || PyType_Ready(&PyHH_WINTYPEType) == -1 || PyType_Ready(&PyNMHDRType) == -1 || PyType_Ready(&PyHHN_NOTIFYType) == -1 || PyType_Ready(&PyHHNTRACKType) == -1)
    PYWIN_MODULE_INIT_RETURN_ERROR;

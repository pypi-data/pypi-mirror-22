...
def collect_extensions(parent='.'):
    ...
    for name in toposort_flatten(dependencies):
        ...
        while q:
            ...
            try:
                q.extend([
                    d for d in dependencies[name] if d not in f
...
def find_tools():
    ...
    return lib, dumpbin, paths


def generate_libs(libs, plat_name, build_temp):
    """Generate lib files to link against from python extensions."""
    modules = [__import__(lib) for lib in libs]
    lib, dumpbin, paths = find_tools()
    env = copy(os.environ)
    env['path'] = os.pathsep.join(paths)

    for module in modules:
        pyd = module.__file__
        basename = os.path.splitext(os.path.basename(pyd))[0]

        dll = os.path.join(build_temp, '{}.dll'.format(basename))
        dname = os.path.join(build_temp, '{}.def'.format(basename))
        lname = os.path.join(build_temp, '{}.lib'.format(basename))

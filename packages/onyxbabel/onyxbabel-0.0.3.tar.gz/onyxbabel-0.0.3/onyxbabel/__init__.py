...
from onyxbabel._compat import string_types

_DEFAULT_LOCALE = Locale.parse('fr')
...
class Babel(object):
    ...
    def __init__(self, app=None, default_locale='fr', default_timezone='UTC',
    ...
    def list_translations(self):
        ...
        try:
          import Onyx
          dirname = str(Onyx.__path__[0]) + "/onyx/translations"
        except:
...
class Domain(object):
    ...
    def get_translations_path(self, ctx):
        ...
        from onyx.skills.core import get_raw_name
        config = Config()
        path = []

        all_skills = get_raw_name(config.SKILL_FOLDER)
        for skill in all_skills:
            try:
                path.append(config.SKILL_FOLDER + skill + "/translations")
            except Exception as e:
                pass

        path.append(config.ONYX_PATH + "/translations")
        return path
    ...
    def get_translations(self):
        ...
        if translations is None:
            ...
            path = self.get_translations_path(ctx)

            for trans in path:
                translations.merge(support.Translations.load(trans, locale, domain=self.domain))

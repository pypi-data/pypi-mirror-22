...
def get_images(data):
    soup = BeautifulSoup(data, 'lxml')
    return [str(link) for link in soup.find_all('img', src=True)]

def get_scripts(data):
    soup = BeautifulSoup(data, 'lxml')
    return [str(link) for link in soup.find_all('script', src=True)]


def get_metas(data):
    soup = BeautifulSoup(data, 'lxml')
    return [str(link) for link in soup.find_all('meta', content=True)]

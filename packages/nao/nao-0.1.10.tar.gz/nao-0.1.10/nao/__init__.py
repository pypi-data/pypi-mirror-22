...
from ._yaml import load as yaml_load, dump as yaml_dump, odict, naodict
...
def object2xml(obj, level=1):
    ...
    if isinstance(obj, list):
        ...
        return '\n'.join([object2xml(i, level+1) for i in obj])
    ...
    for a in obj.__dict__:
        if not a.startswith('_'):
            ...
            ats.append('{}<attr name="{}">{}{}</attr>'.format(
                "\t"*level,
                a, 
                object2xml(val, level+1) if isinstance(val, list) else val,
...
class NaoPath(object):
    ...
    def __init__(self, *base):
        ...
        if not op.exists(b):
            raise NoPath("Non-existent path: {}".format(b))

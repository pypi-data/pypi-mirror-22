...
def request_promise(self, resolve, reject, verb, query, data):
    ...
    (self.queue.throttle()
        ...
        .then(lambda res: self.err_check(res, verb, query, data))
...
def err_check_promise(self, resolve, reject, res, verb, query, data):
    ...
    if res.get('body') and status_prefix != '2':
        ...
        if 'jwt expired' in str(res.get('body')):
            (self.refresh_token_method()
                ...
                .then(lambda x: self.request(verb, query, data))
...
class Connection:
    ...
    """
    def refresh_token_method(self, token=None):
    ...
    """
    def err_check(self, res, verb, query, data):
        return Promise(lambda resolve, reject: err_check_promise(self, resolve, reject, res, verb, query, data))

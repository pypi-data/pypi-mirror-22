...
class Connection:
    ...
    def set_client(self):
        ...
        try:
            ...
            self.client.on('connect_error', lambda *x: on_connect_error_lambda(self))
            # No ready event after connect? Something went wrong, reconnect
            self.client.on('connect', lambda *x: on_connect_lambda(self))

            # Resubscribe after disconnect
            self.resub()
        except StopIteration:
            # Catch socket.io 2.x error
            pass
    ...
    '''
    def reconnect(self):
        return Promise(lambda resolve, reject: reconnect_lambda(self, resolve, reject))
    ...
    def resub(self):
        ...
        self.client.on('reconnect', lambda *x: resub_on_reconnect(self))
...
def on_connect_error_lambda(self):
    self.connected = False
    self.reconnect()


def on_connect_lambda(self):
    self.client.on('ready', lambda *x: on_connect_lambda_ready_lambda)
    t = Timer(1, on_connect_timer, args=(self,))
    t.start()


def on_connect_lambda_ready_lambda(self):
    self.connected = True

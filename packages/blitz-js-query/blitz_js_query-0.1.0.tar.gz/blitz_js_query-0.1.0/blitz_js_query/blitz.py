...
class Blitz:
    ...
    def subscribe(self, endpoint):
        self.connection.client.emit('subscribe', endpoint)

    '''
    Event listening for socket.io
    '''
    def on(self, ev, fn):
        self.connection.client.on(ev, fn)

    '''
    Expose socket client emit
    '''
    def emit(self, ev, data):
        self.connection.client.emit(ev, data)
...
def connect_lambda(self, resolve, reject):
    ...
    self.connection.connect().then(lambda x: resolve(None))

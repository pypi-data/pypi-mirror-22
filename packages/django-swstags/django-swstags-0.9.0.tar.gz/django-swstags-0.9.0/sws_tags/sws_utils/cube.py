...
def dateCubeRouter(func_name):
    ''' Selects wich cube will be queried depending on the date range
    Will raise an error if the schema change date is within th date range
    or Sales Rep is used as filter o group column on the old schema'''
    def decorate(func):
        def wrapper(*args, **kwargs):
            ...
            # The query contains a data period that involves two schemas
            if from_date < request_data['var_settings']['CUBE_CHANGE_DATE'] and to_date > request_data['var_settings']['CUBE_CHANGE_DATE']:
                sendMessages(
                    request_data['var_settings']['USER_MESSAGE_CHANNEL'],
                    kwargs['redis'],
                    message='Date range can not contain two different schemas (schema changed on {0})'.format(request_data['var_settings']['CUBE_CHANGE_DATE']),
                    level='warning',
                    time=5000,
                    rsyslog=False
                )
                # Return depending on the decorated function
                if func_name == 'requestCube':
                    return ujson.dumps({"records":0,"total":0.0,"rows":[],"page":1})
                elif func_name == 'requestCubeFilters':
                    return ujson.dumps([])
                elif func_name == 'requestCubeHighcharts':
                    return {'v_name': ['wrong date period', 'wrong date period'], 'v_data': [], 'v_yAxis': [0, 1], 'v_type': ['line'], 'v_xAxis': ['2017-01-01']}

            # Route the connection to the legacy database
            elif to_date < request_data['var_settings']['CUBE_CHANGE_DATE']:
                # Can be string or iterable
                if hasattr(args[1], '__iter__'):
                    args[1][0] = '{0}_29'.format(args[1][0])
                else:
                    args[1] = '{0}_29'.format(args[1])
                if 'cube_config' in request_data:
                    # The deep copy is necessary because it comes from the session and would be modified system wide
                    args = copy.deepcopy(args)
                    request_data = args[5]
                    request_data['cube_config']['cube_db'] = '{0}_29'.format(request_data['cube_config']['cube_db'])

                # Send a message indicating that the Salesrep filter will have no effect
                if request_data.get('Salesrep', 'NULL') != 'NULL':
                    sendMessages(
                        request_data['var_settings']['USER_MESSAGE_CHANNEL'],
                        kwargs['redis'],
                ...
                for field in ('group_by', 'and_by', 'final_by'):
                    if request_data.get(field, 'NULL') == 'Salesrep':
                        sendMessages(
                            request_data['var_settings']['USER_MESSAGE_CHANNEL'],
                            kwargs['redis'],
            ...
            elif from_date > request_data['var_settings']['CUBE_CHANGE_DATE']:
                pass
...
@dateCubeRouter('requestCubeHighcharts')
...
@dateCubeRouter('requestCubeFilters')

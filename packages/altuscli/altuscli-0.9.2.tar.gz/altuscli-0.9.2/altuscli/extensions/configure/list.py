...
class ConfigureListCommand(BasicCommand):
    NAME = 'list'
    DESCRIPTION = (
        'List the Altus CLI configuration data.  This command will '
        ...
        'if you provide the Altus access key in an environment variable, this '
    ...
    SYNOPSIS = 'altus configure list [--profile profile-name]'
    EXAMPLES = (
        'To show your current configuration values::\n'
        '\n'
        '  $ altus configure list\n'
        '        Name                    Value             Type    Location\n'
        '        ----                    -----             ----    --------\n'
        '     profile                <not set>             None    None\n'
        '  access_key     ****************ABCD      config_file    ~/.altus/config\n'
        '  private_key    ****************ABCD      config_file    ~/.altus/config\n'
        '\n'
    )
    ...
    def _run_main(self, context, args, parsed_globals):
    ...
    def _lookup_credentials(self, context):
        ...
        access_key = self._lookup_config(context, ALTUS_ACCESS_KEY_ID_KEY_NAME)
        if access_key.value is not NOT_SET:
            private_key = self._lookup_config(context, ALTUS_PRIVATE_KEY_KEY_NAME)
            ...
            if credentials is None:
                ...
                # visible from altuscli.credentials.  I think

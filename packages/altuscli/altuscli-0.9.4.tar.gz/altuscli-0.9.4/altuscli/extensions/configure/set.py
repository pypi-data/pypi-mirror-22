...
from altuscli import ALTUS_ACCESS_KEY_ID_KEY_NAME
from altuscli import ALTUS_PRIVATE_KEY_KEY_NAME
from altuscli import DEFAULT_PROFILE_NAME
...
class ConfigureSetCommand(BasicCommand):
    ...
    DESCRIPTION = BasicCommand.FROM_FILE('configure', 'set',
                                         '_description.rst')
    SYNOPSIS = 'altus configure set varname value [--profile profile-name]'
    ...
    # the ~/.altus/credentials file instead of ~/.altus/config.
    _WRITE_TO_CREDS_FILE = [ALTUS_ACCESS_KEY_ID_KEY_NAME,

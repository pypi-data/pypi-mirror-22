...
class Array:
    ...
    def __str__(self):
        ...
        for i in range(self._size):
            line += str(self._elements.__getitem__(i))+ ' | '
        return line[:-1]
...
class DynamicArray:
    ...
    def append(self, value):
        ...
        :param value: the value of element.
        value - object of class Flight
        """
        if self._size == self._capacity:  # not enough room
            self._resize(int(1.5 * self._capacity)+1)  # so double capacity
        #self._elements[self._size] = value
        #'''
        #Changes of array.py
        self._elements[self._size] = Array(3)
        self._elements[self._size].__setitem__(0, value.agent.url)
        self._elements[self._size].__setitem__(1, value.__str__())
        self._elements[self._size].__setitem__(2, value.link)
        #logo | price | link
        #'''
        self._size += 1
    ...
    def __str__(self):
        line=''
        for i in range(self._size):
            line+=(self._elements.__getitem__(i).__str__())+'\n'
        return line[:-1] if line is not None else ''

...
class NetMagusSession(object):
    ...
    def __read_state_file(self):
        ...
        with contextlib.closing(
                shelve.Shelf(state_file, protocol=2, writeback=True)) as state_shelve:
            try:
                self.user_state = state_shelve['formula_state'.encode('utf-8')]
    ...
    def __write_state_file(self, stateobject):
        ...
        try:
            with contextlib.closing(shelve.Shelf(state_file, protocol=2, writeback=True)) as \
                    state_shelve:
                state_shelve['formula_state'.encode('utf-8')] = stateobject

...
class ZabbixServerProxy(object):
    ...
    def init_app(self, app, transport=None):
        zabbix_conf = app.config.get('ZABBIX_CONFIG', {})
        url = zabbix_conf.get('ZABBIX_URL')
        ...
        uname = zabbix_conf.get('ZABBIX_USER')
        password = zabbix_conf.get('ZABBIX_PASSWORD')
        self.user.login(user=uname, password=password)

...
from .udacity import build_session, root_url, result_filename, submission_filename, default_app_data_dir
...
class DiagnoseHelper(object):
  ...
  def act(self):
    jwt_path = os.path.join(default_app_data_dir(), 'jwt')
    data = {
      'proxies' : getproxies(),
      'default_jwt_path': jwt_path,
      'jwt_present': os.path.isfile(jwt_path),

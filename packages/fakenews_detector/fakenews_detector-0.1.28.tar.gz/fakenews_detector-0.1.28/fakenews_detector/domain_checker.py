...
from fakenews_detector.news_info import NewsVerdict, AbstractInfo
from fakenews_detector.url_utils import get_data_path
from fakenews_detector.categories_dict import opensource_cat, manually_added_cat
...
class OpenSourcesInfo(AbstractInfo):
    ...
    def get_info(self):
        ...
        descriptions = [opensource_cat[x] for x in self.categories]
...
class FakeNewsDBInfo(AbstractInfo):
    ...
    def get_info(self):
        verdicts = [self.get_verdict(x) for x in self.categories]
        categories = self.categories
        descriptions = [manually_added_cat[x] for x in self.categories]
        source_notes = [self.domain] + [self.source_desc] + self.source_notes
        if len(self.political_alignments) > 0:
            source_notes.append("Political alignment: " + self.political_alignments[0])
        return verdicts, categories, descriptions, source_notes

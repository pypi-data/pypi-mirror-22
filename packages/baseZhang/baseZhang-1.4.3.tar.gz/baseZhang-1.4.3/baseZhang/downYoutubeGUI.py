...
def get_m3u8(channel=110):
    channel = str(channel)
    url = 'http://vidol.tv/programmes/' + channel
    respose = requests.get(url)
    html = respose.content

    html_md = open('html.md', 'w')
    html_md.writelines(html)
    html_md.close()

    read_html = open('html.md', 'r')
    html_lines = read_html.readlines()
    read_html.close()
    os.remove('html.md')
    pub_video = []
    for line in html_lines:
        if 'pubId' in line:
            pub_video.append(line)
    pub_video_ID_value = pub_video[-1].split('?')[-1].split('"')[0].split('&amp;')

    m3u8_addr = 'http://c.brightcove.com/services/mobile/streaming/index/master.m3u8?' + pub_video_ID_value[-1] + '&' + \
                pub_video_ID_value[0]
...
def downVideoGUI():
    ...
    def addMark():
        ...
        split_first = 'ffmpeg -ss 00:00:00 -t 00:00:05 -i ' + new_video_path + ' -strict -2  -vcodec copy split1.mp4'
        os.system(split_first)

        split_second = 'ffmpeg -ss 00:00:05  -i ' + new_video_path + ' -strict -2  -vcodec copy split2.mp4'
...
boxborderw=5: x=100: y=100" -codec:a copy split0.mp4'''
        ...
        os.remove("c_2.ts")
        os.remove("c_1.ts")
        os.rename('combine.mp4', video_path.replace('.mp4', '_water.mp4'))
        if_no_create_it(video_path)
        os.rename(new_video_path, video_path)
    ...
    def vidolM3U8ZD():
        channel = M3U8_link_contend_gif.get()
        M3U8 = get_m3u8(channel)
        ffmpeg_cmd = 'ffmpeg -ss 00:01:00 -i "' + M3U8 + '" -c copy -bsf:a aac_adtstoasc -y ' + channel + '_' + nowTimeStr() + '.mp4'
    ...
    def vidolM3U8DRM():
        ...
        M3U8 = get_m3u8(119)
        ffmpeg_cmd = 'ffmpeg -ss 00:01:00 -i "' + M3U8 + '" -c copy -bsf:a aac_adtstoasc -y DRM_' + nowTimeStr() + '.mp4'
    ...
    def vidolM3U8WHD():
        # M3U8 = M3U8_link_contend_gif.get()
        M3U8 = get_m3u8(176)
        ffmpeg_cmd = 'ffmpeg -ss 00:01:30 -i "' + M3U8 + '" -c copy -bsf:a aac_adtstoasc -y WHD_' + nowTimeStr() + '.mp4'
        print ffmpeg_cmd
        info_entry.insert(1.0, '\nM3U8下载\n', 'a')
        info_entry.insert(1.0, ffmpeg_cmd, 'a')
        os.system(ffmpeg_cmd)
    ...
    M3U8_choose_button = Button(root, text='指定频道', command=vidolM3U8ZD, width=15)
    M3U8_choose_button.grid(row=2, column=2, columnspan=1)
    ...
    watermark_gif_button = Button(root, text='添加水印', command=addMark, width=15)
    watermark_gif_button.grid(row=3, column=4)

    info_entry = Text(root, width=50, height=6, )
    info_entry.grid(row=4, column=0, columnspan=5, rowspan=2)
    info_entry.tag_config('a', foreground='green')
    info_entry.config(font='helvetica 18')
    info_entry.insert(1.0,
                      '视频源：【1】youtube【2】vidol\n欢迎加入QQ群：》》》》532671563《《《《【咻是群主】\n【主要上传吴宗宪节目哦】\nVidol 下载只需输入频道号\n如国光帮：121周二爱玩客：116大主厨：122',
                      'a')

...
import click
import jsonpath_rw
from cloudmunch import request, utils, config
...
def validate_config(func):
  ...
  def wrapper(*args, **kwargs):
    if config.config.endpoint() == '<your-api-endpoint>':
      raise click.ClickException("No endpoint set. Execute cloudmunch configuration update endpoint <your-api-endpoint>")

    if config.config.apikey() == '<your-api-key>':
      raise click.ClickException("No apikey set. Execute cloudmunch configuration update apikey <your-api-key>")
...
def get_context_id(func):
  @wraps(func)
  def wrapper(**kwargs):
    logging.getLogger(__name__).debug("Before converting names to ids {}".format(kwargs.items()))

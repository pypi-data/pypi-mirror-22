...
import core.dstat as dstat
from core.experiments import idle, pot
from core import params, analysis
from core.interface import exp_window, adc_pot, plot_ui, data_view, save
from core.errors import InputError
from core.plugin import DstatPlugin, get_hub_uri
...
class Main(object):
    ...
    def on_serial_connect_clicked(self, data=None):
        ...
        try:
            ...
            dstat.state.dstat_version = dstat.comm.version_check(
            ...
            if not len(dstat.state.dstat_version) == 2:
                self.statusbar.push(self.error_context_id,
                    "Communication Error")
                return

            else:
                self.adc_pot.set_version(dstat.state.dstat_version)
                self.statusbar.push(
                    self.error_context_id,
                    "DStat version: {}.{}".format(
                                    str(dstat.state.dstat_version[0]),
                                    str(dstat.state.dstat_version[1])
                    )
                )
    ...
    def start_ocp(self, data=None):
        ...
        if (dstat.state.dstat_version[0] >= 1 and 
        dstat.state.dstat_version[1] >= 2):

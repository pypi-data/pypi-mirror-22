...
def setup_autocomplete():
    ...
    completion_script_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'bin', 'bmcs_autocomplete.sh')
    ...
    if click.confirm(confirm_prompt, default=True):
        with open(bash_config_location, mode='a+') as f:
            ...
            if bash_profile_line in content:
                click.echo("It looks like tab completion for bmcs is already configured in {bash_config_location}. If you want to re-run the setup command please remove the line containing '{bash_profile_line}' from {bash_config_location}.".format(bash_profile_line=bash_profile_line, bash_config_location=bash_config_location))
                return

...
var eurofiltersIU = function (config_data_out, callbackFunction, stoneselectDB){
	...
	*/
	...
	var aplyConfigSelect2 = function(name, selected){
		...
		self.filtersObjects[name] = $('.' + self.config_data['prefix'] + name).multiselect(opts);

		if (self.configData(name, 'search_input'))
		{
			self.filtersObjects[name].multiselectfilter();
		}
	...
	var styleFilters = function(name){
		$('.ui-multiselect-menu.ui-widget.ui-widget-content.ui-corner-all').css({'min-width': '170px'})
		// float_style = 'right'

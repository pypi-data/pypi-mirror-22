...
var eurofiltersIU = function (config_data_out, callbackFunction, stoneselectDB){
	...
	*/
	...
	var createSelectTag = function (name){
		...
		div_general.classList.add(self.config_data['width_class']);
		// width = ''
	...
	var addHtmlToContainer = function(name){
		var container = self.config_data['container'];
		if(self.options[name]['fieldset']){
			var fieldset_id = self.config_data['prefix']+self.options[name]['fieldset']['name']+'-fieldset';
			if($(self.config_data['container']+' '+'#'+fieldset_id).length == 0){
				var fieldset = createFieldsetTag(name, fieldset_id);
				$(self.config_data['container']).append(fieldset);
			}
			container += ' #'+fieldset_id;
		}
		if( $(container+' '+'#'+self.config_data['prefix']+name).length == 0 ){
	...
	var getDefaultParam = function(){
		opts_default = {header: true ,selectedList : 3, position: {my: 'left top',at: 'left bottom'} , max: 0, maxShow: 3, maxLetters: 3, numToShow: 10, multiple:true,checkAllText:'', uncheckAllText:'', noneSelectedText:self.config_data['noneSelectedText'], fieldset:null, /*show: ["bounce", 200],hide: ["bounce", 1000],*/
				...
				$.each(data, function(index, val){
					if('children' in val){
						...
						$.each(val['children'], function(index2, val2){
							...
							option = {'id': val_id, 'text': val2['text']}

import argparse
from cocoag.generator.settings_prod import DEFAULT_CONFIG_FILE
from cocoag.generator.settings_prod import OUTPUT_FILE_NAME
from cocoag.generator.settings_prod import PACKAGE_ROOT_PATH
...
parser.add_argument('--config', type=str, default=DEFAULT_CONFIG_FILE,
                    help='The config file specifying the directories the CoverageGenerator should operate on.')


class CoverageGenerator(object):
    _badge_output_location = os.path.join(PACKAGE_ROOT_PATH, OUTPUT_FILE_NAME)
    ...
    @classmethod
    def generate(cls, config_json: dict):
        config = cls._CoverageGeneratorConfig(bucket_root=config_json["bucketRoot"],
                                              project_root=config_json["projectRoot"],
                                              sub_directories=config_json["subDirectories"])
        for sub_directory in config.sub_directories:
            cls._process(active_sub_directory=sub_directory, config=config)
    ...
    def _upload_badge(cls, badge_info: BadgeInfo, config: CoverageGeneratorConfig, sub_directory: str) -> str:
        ...
        root_key_path = "badges/coverage/{branch_name}/{dir}".format(branch_name=badge_info.associated_branch,
                                                                     dir=sub_directory)
        full_key_path = os.path.join(root_key_path, OUTPUT_FILE_NAME)
...
if __name__ == "__main__":
    ...
    with open(args.config) as config_file:
        ...
        CoverageGenerator.generate(config_json=settings)
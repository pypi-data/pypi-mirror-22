...
class KafkaConsumerBase(Thread, CypressBase):
    ...
    """
    def __init__(self, bootstrap_server, topic, partition=0, group="group1", polling_timeout=30, connect_retries=100,
        ...
        self.config["topic.metadata.refresh.interval.ms"] = 10000
        if "queued.min.messages" not in self.config.keys():
            self.config["queued.min.messages"] = 10
        if "fetch.wait.max.ms" not in self.config.keys():
            self.config["fetch.wait.max.ms"] = 100
        ...
        self.consumer_message_task_queue = Queue.Queue()
    ...
    def run(self):
        ...
        """
        start_new_thread(self.get_message, ())
        while self.running:
            try:
                ...
                if message.error():
                    ...
                    try:
                        ...
                        self.consumer_message_task_queue.put(message)
    ...
    def get_message(self):
        while self.running:
            try:
                msg = self.consumer_message_task_queue.get(block=False, timeout=10)
            except:
                continue
            if msg.get("start_ts"):
                self.logger.debug("message spent {}s before processing".format(time() - msg["start_ts"]))

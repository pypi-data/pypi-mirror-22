...
class KafkaProducerBase(Thread, CypressBase):
    def __init__(self, bootstrap_server, config=None):
        ...
        if "queue.buffering.max.ms" not in self.config.keys():
            self.config["queue.buffering.max.ms"] = 0
...
class KafkaConsumerBase(Thread, CypressBase):
    ...
    def __init__(self, bootstrap_server, topic, partition=0, group="cypress_default", polling_timeout=30,
                 connect_retries=100, reconnect_interval=0.5, config=None, get_stats=False, stats_interval=1000,
                 cache=None):
        """
        ...
        self.cache = cache
        if get_stats and self.cache:
            self.config["stats_cb"] = self.stats_handler
            self.config["statistics.interval.ms"] = stats_interval
        if "fetch.wait.max.ms" not in self.config.keys():
            self.config["fetch.wait.max.ms"] = 0
        if "socket.blocking.max.ms" not in self.config.keys():
            self.config["socket.blocking.max.ms"] = 1
    ...
    def stats_handler(self, json_str):
        try:
            json_msg = json.loads(json_str)
            topics = json_msg.get("topics")
            if not topics:
                return

            partitions = topics.get(self.consumer_topic).get("partitions")
            for partition, info in partitions.iteritems():
                if partition == "-1" or info.get("hi_offset") < 0 or info.get("app_offset") < 0:
                    continue
                consumer_lag = info.get("hi_offset") - info.get("app_offset")
                if consumer_lag > 0:
                    self.cache.set_topic_partition_lag(self.consumer_topic, partition, consumer_lag)

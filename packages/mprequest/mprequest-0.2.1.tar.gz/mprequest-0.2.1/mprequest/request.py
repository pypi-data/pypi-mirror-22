...
_METHOD_FUNC_MAPPINGS = {
    HTTP_GET_METHOD: lambda s: s.get if s is not None else None,
    HTTP_POST_METHOD: lambda s: s.post if s is not None else None,
    HTTP_PUT_METHOD: lambda s: s.put if s is not None else None,
    HTTP_PATCH_METHOD: lambda s: s.patch if s is not None else None,
    HTTP_DELETE_METHOD: lambda s: s.delete if s is not None else None,
    HTTP_HEAD_METHOD: lambda s: s.head if s is not None else None,
    HTTP_OPTIONS_METHOD: lambda s: s.options if s is not None else None
}
...
class Request(object):
    ...
    def _make_request(self, session):
        session_func = _METHOD_FUNC_MAPPINGS.get(self.method)(session)
    ...
    def prepare_get(cls, url, ctx=None, **kwargs):
        return Request(url, HTTP_GET_METHOD, ctx, **kwargs)

    @classmethod
    def prepare_patch(cls, url, ctx=None, **kwargs):
        return Request(url, HTTP_PATCH_METHOD, ctx, **kwargs)

    @classmethod
    def prepare_head(cls, url, ctx=None, **kwargs):
        return Request(url, HTTP_HEAD_METHOD, ctx, **kwargs)

    @classmethod
    def prepare_options(cls, url, ctx=None, **kwargs):
        return Request(url, HTTP_OPTIONS_METHOD, ctx, **kwargs)

    @classmethod
    def prepare_delete(cls, url, ctx=None, **kwargs):
        return Request(url, HTTP_DELETE_METHOD, ctx, **kwargs)

    @classmethod
    def prepare_put(cls, url, ctx=None, **kwargs):
        return Request(url, HTTP_PUT_METHOD, ctx, **kwargs)

    @classmethod
    def prepare_post(cls, url, ctx=None, **kwargs):
        return Request(url, HTTP_POST_METHOD, ctx, **kwargs)

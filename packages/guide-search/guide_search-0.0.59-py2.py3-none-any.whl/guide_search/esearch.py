...
class Esearch(object):
    ...
    def init(self, host, control, **kwargs):
        self.host = host
        if len(control) == 0:
            self.control = '/guide_config/controls'
        else:
            control = control.split('/')
            for x in control:
                if len(x) > 0:
                    self.control = '{0}/controls'.format(x)
                    break
    ...
    def reset_session(self):
        if self.logger:
            self.logger.debug("reset elasticsearch connection and wait 60 secs")
...
# query functions
    ...
    def post_up(self, index, last, newcommit):
        ...
        up["seq"] = seq
        up["start"] = datetime.now(timezone.utc).astimezone().isoformat()
        up["finish"] = datetime.isoformat(datetime.min) + 'Z'

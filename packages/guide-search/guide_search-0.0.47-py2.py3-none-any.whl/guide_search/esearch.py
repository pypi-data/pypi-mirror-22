...
# query functions
    ...
    def get_article_by_snippet(self, index, snippet_type, snippet_id):
        url = self.make_url(index, "articles", "_search")
        dsl = {"_source": ["id"],
               "query": {"bool":
                         {"must": [{"term": {"snips.snip_type": snippet_type}},
                                   {"term": {"snips.snip_id": snippet_id}}]}}}
    ...
    def post_end(self, index, seq, newcommit, state, log=None):
        ...
        if not seq == last["seq"]:
            ...
            last["finish"] = datetime.datetime.isoformat(datetime.datetime.now())
            last["state"] = state
            if log:
                last["log"] = [log]
            else:

...
class Connection(object):
    ...
    def _read_message(self):
        ...
        # while 1的原因在于，为了安全stop，只要我有数据，就应该继续处理
        while 1:
            flag = self.zmq_client.poll(self.conn_timeout_ms)
            if flag <= 0:
                logger.debug('poll result: %s', flag)
                # 超过时间没有拿到数据
                ...
                while 1:
                    try:
                        data = self.zmq_client.recv(flags=zmq.NOBLOCK)
                ...
                for data in data_list:
                    ...
                    safe_call(self._on_read_complete, data)

                if not self.app.enable and self.closed():
                    # 如果是刚才已经全部收完，那么这里可以正常break
                    break

                # 否则有可能是处理data中，发送的kill -hup，那么就要保证全部再poll一遍没问题才行

...
from ..share.utils import import_module_or_string, safe_call
...
class Gateway(object):
    ...
    def _fetch_from_forwarder(self):
        ...
        # zmq支持sub多个topic，这样就可以接收到不区分node的消息了
        self.forwarder_client.setsockopt(zmq.SUBSCRIBE, self.config['NODE_ID_WILDCARD'])
    ...
    def _handle_task(self, task):
        ...
        if task.cmd == constants.CMD_WRITE_TO_CLIENT:
            conn = self.conn_dict.get(task.conn_id_num)
            if conn:
    ...
    def _send_task_to_worker(self):
        ...
        while True:
            ...
            self.inner_server.send(task.pack())
    ...
    def _register_outer_server_handlers(self):
        ...
        def create_conn(conn):
            ...
            task = Task()
            task.node_id_num = self.node_id
            task.conn_id_num = conn.id
            task.client_ip_num = conn.client_ip_num
            task.uid = conn.uid
        ...
        def handle_request(conn, box):
            # 转发到worker
            logger.debug('conn: %s, box: %s', conn, box)
            task = Task()
            task.node_id_num = self.node_id
            task.conn_id_num = conn.id
            task.client_ip_num = conn.client_ip_num
    ...
    def _worker_run(self, index):
        ...
        :return:
        """
        if self.config['GATEWAY_NODE_ID_LIST'] and len(self.config['GATEWAY_NODE_ID_LIST']) > index:
            # 直接指定node_id
            self.node_id = self.config['GATEWAY_NODE_ID_LIST'][index].decode('hex')

        if not self.node_id:
            self.node_id = uuid.uuid4().bytes

        setproctitle.setproctitle(self._make_proc_name('gateway:worker:%s:%s' % (index, self.node_id.encode('hex'))))
    ...
    def _user_renew_callback(self, conn):
        """
        当需要调用renew的时候
        :param conn:
        :return:
        """
        if conn.uid:
            safe_call(self.share_store.renew_user, conn.uid)
    ...
    def _login_client(self, conn, uid, userdata):
        if conn and conn.uid != uid:
            ...
            # 后写入存储
            safe_call(self.share_store.add_user, conn.uid, self.node_id)

            if self.config['REDIS_USER_MAX_AGE'] is not None:
                if not getattr(conn, 'user_renew_timer', None):
                    conn.user_renew_timer = Timer()

                conn.user_renew_timer.set(
                    self.config['REDIS_USER_MAX_AGE'] * self.config['REDIS_USER_RENEW_TIMEOUT_FACTOR'],
                    functools.partial(self._user_renew_callback, conn),
                    repeat=True,
                    force=True,
                )
    ...
    def _logout_client(self, conn):
        if conn and conn.uid is not None:
            safe_call(self.share_store.remove_user, conn.uid, self.node_id)

            if getattr(conn, 'user_renew_timer', None):
                conn.user_renew_timer.clear()

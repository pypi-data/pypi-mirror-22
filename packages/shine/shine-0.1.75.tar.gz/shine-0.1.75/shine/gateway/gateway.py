...
class Gateway(object):
    ...
    def run(self, debug=None):
        ...
        if self.config['GATEWAY_NODE_ID']:
            if workers != 1:
                logger.error('GATEWAY_NODE_ID is defined. len(GATEWAY_INNER_ADDRESS_LIST) should be 1.')
                return
    ...
    def _handle_task(self, task):
        ...
        if task.cmd == constants.CMD_WRITE_TO_CLIENT:
            conn = self.conn_dict.get(task.conn_id)
            if conn:
    ...
    def _worker_run(self, index):
        ...
        if self.config['GATEWAY_NODE_ID']:
            # 直接指定node_id
            self.node_id = self.config['GATEWAY_NODE_ID']
        else:

from integration_pip_inspector.FileHandler import generate_file, FLAT_POSTFIX, TREE_POSTFIX
...
class TreeHandler(object):
    ...
    def render_tree(self,  output_path=None):
        ...
        if output_path:
            generate_file(
                result, self.tree.name, output_path, TREE_POSTFIX)
        return result
    ...
    def render_flat(self, output_path=None):
        ...
        if output_path:
            generate_file(
                result, self.tree.name, output_path, FLAT_POSTFIX)
        return result

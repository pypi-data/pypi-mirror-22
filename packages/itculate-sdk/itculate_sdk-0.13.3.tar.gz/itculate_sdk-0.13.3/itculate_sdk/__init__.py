...
from .types import *
from .uploader import Provider, PayloadGenerator, GLOBAL_COLLECTOR_ID
...
_payload_generators = {}  # type: dict[str, PayloadGenerator]


def _check_init():
    global _payload_generators
    assert _payload_generators is not None and isinstance(_payload_generators, dict), "SDK was not initialized!"


# noinspection PyUnresolvedReferences
def _get_topology_payload_generator(collector_id):
    """
    Returns the payload generator for this collector ID
    :param basestring collector_id: Collector ID
    :rtype: PayloadGenerator
    """
    global _payload_generators, _tenant_id

    if isinstance(collector_id, six.text_type):
        collector_id = collector_id.encode()

    payload_generator = _payload_generators.get(collector_id)
    if payload_generator is None:
        payload_generator = PayloadGenerator(tenant_id=_tenant_id, collector_id=collector_id)
        _payload_generators[collector_id] = payload_generator

    return payload_generator


def _get_global_payload_generator():
    """
    Returns the global payload generator
    :rtype: PayloadGenerator
    """
    global _payload_generators

    payload_generator = _payload_generators.get(GLOBAL_COLLECTOR_ID)
    if payload_generator is None:
        payload_generator = PayloadGenerator(tenant_id=_tenant_id, collector_id=GLOBAL_COLLECTOR_ID)
        _payload_generators[GLOBAL_COLLECTOR_ID] = payload_generator
...
def init(provider=None, host=None, tenant_id=None, **kwargs):
    ...
    """
    global _tenant_id, _provider, _payload_generators
...
def add_vertex(collector_id,
    ...
    return _get_topology_payload_generator(collector_id).add_vertex(vertex_type=vertex_type,
                                                                    name=name,
                                                                    keys=keys,
...
def connect(source, target, topology, collector_id):
    ...
    _get_topology_payload_generator(collector_id).connect(source=source, target=target, topology=topology)
...
def add_samples(vertex, counter, timestamp_to_value):
    ...
    _get_global_payload_generator().add_counter_samples(vertex=vertex,
                                                        counter=counter,
...
# noinspection PyUnresolvedReferences
def vertex_event(collector_id, vertex, message, event_type="MESSAGE", severity="INFO", timestamp=None):
    """
    Generic event
    ...
    _get_topology_payload_generator(collector_id).vertex_event(collector_id=collector_id,
                                                               vertex=vertex,
                                                               message=message,
...
def vertex_unhealthy(collector_id, vertex, message=None, timestamp=None):
    """
...
def flush_topology(collector_id):
    ...
    if collector_id not in _payload_generators:
        return False

    return _provider.flush_now((_payload_generators[collector_id],)) > 0
...
def flush_all():
    ...
    global _payload_generators
    return _provider.flush_now(_payload_generators)

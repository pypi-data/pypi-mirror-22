...
class Dictionary(object):
    ...
    @classmethod
    def update_data_type(cls, dictionary_type, attribute, data_type, vertex_key=None, vertex_type=None):
        """
        Updates the dictionary repository with new meta data
        :param str dictionary_type: Dictionary item type. Either "vertex" or "timeseries"
        :param str vertex_key: Key of vertex owning this attribute (utf-8 encoded) (mutually exclusive with vertex_key)
        :param str vertex_type: Vertex type (utf-8 encoded) (mutually exclusive with vertex_key)
        :param str attribute: Name of attribute (utf-8 encoded)
        :param DataType data_type: data type to set
        """

        assert bool(vertex_key) != bool(vertex_type), "Either vertex_key OR vertex_type must be provided (bot not both)"

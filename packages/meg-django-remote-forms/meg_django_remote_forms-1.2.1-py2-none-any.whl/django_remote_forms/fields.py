...
class RemoteField(object):
    ...
    def as_dict(self):
        ...
        custom_module_dotted_path = getattr(settings, 'REMOTE_FORMS_WIDGETS', None)
        if self.field.widget.__class__.__name__ != '':
            remote_widget_class_name = 'Remote%s' % self.field.widget.__class__.__name__
        else:
            remote_widget_class_name = 'RemoteInput'
            logger.warning('Defaulting widget to %s for field %s', remote_widget_class_name, self.field)
        try:
            remote_widget_class = None
            if hasattr(widgets, remote_widget_class_name):
                remote_widget_class = getattr(widgets, remote_widget_class_name)
            else:
                if custom_module_dotted_path is not None:
                    custom_module = importlib.import_module(custom_module_dotted_path)
                    remote_widget_class = getattr(custom_module, remote_widget_class_name)

            remote_widget = remote_widget_class(self.field.widget, field_name=self.field_name)
        except Exception, e:
            widget_dict = {}
            logger.warning('Error serializing widget %s for field %s: %s', remote_widget_class_name, self.field, str(e))
        else:

...
INTRODUCTION:
    ...
    mandate of LSH, which is to return just the nearest neighbors. (A being
    a nearest neighbor of B and B being a nearest neighbor of C, in the
    sense such nearest neighbors are defined in the context of LSH, does
    not imply that A and C are sufficiently close to be considered
    similar.)  Nonetheless, if you believe that your datafile consists of
    non-overlapping data clusters, this module may do a decent job of
    finding those clusters.
...
CAN NEAREST NEIGHBORS RETURNED BY LSH BE USED TO CLUSTER THE DATA?
    ...
    considers data samples A and B to be similar because the algorithm has
    placed them in the same bucket.  And, again, let's say that LSH
    considers the data samples B and C to be similar because the algorithm
    has placed them also in the same bucket (this bucket being different
    from the first bucket).  A being similar to B and B being similar to C
    in the sense described here may not imply that A is similar to C.  Just
    imagine three points on the same great circle on the surface of a
    sphere.  The angular interval between A and B, on the one hand, and
    between B and C, on the other, could be sufficiently small so that A
    and B would be considered similar and B and C would be considered
    similar.  Yet, the angular interval between A and C may exceed the
    threshold test for similarity.
    ...
    not likely to be true for most practical applications, modifications of
    the logic presented in this module may provide acceptable results when
    the data clusters are somewhat overlapping.  Consider, for example, the
    notion used in this module that two neighborhoods returned by LSH can
    be clustered together if they share data samples.  We could
    ...
    shared data samples closer to the means in each of the neighborhoods
    that are close to the mean must be trusted more with regard to the two
    neighborhoods belonging to the same cluster.  This additional logic
...
USAGE:
    ...
    datafile must look like.)  The parameter 'r' is number of rows for
    r-wise AND in each band of hash functions, and b is the number of bands
    for b-wise OR over all the bands.

    On the other hand, if you want to use this module for experimenting
    with clustering your data using LSH for discovering the neighbors of
    the individual data samples, your usage of the module will look like:
...
CONSTRUCTOR PARAMETERS:
    ...
    datafile:            Must be a ".csv" file.  Each record in this file 
                         ...
                         Each records must have associated with it a 
    ...
    r:                   The number of rows in one band of hash functions.
                         An r-wise AND operator is applied to the buckets
                         in each such band.

    b:                   The number of r-row bands of hash functions.  A
                         b-wise OR operator is applied to the buckets 
...
ACKNOWLEDGMENTS:
    ...
    The author has learned much from all his LSH-related discussions with
    Tommy Chang who is currently finishing his Ph.D. in the Robot Vision
    Lab at Purdue.  Tommy is using LSH to solve a truly big-data problem:
    the problem of creating concise training and testing datasets for
    land-cover classification using hundreds of satellite images that may
    cover a region of interest whose size may be as large as hundreds of

...
class Registry(object):
    ...
    class RegistryKey(object):
        ...
        def open(self, key, subkey):
            ...
            if not hkey:
                raise RegistryError("could not find registry key for %s" % key)
                raise RegistryError("could not find registry key for %s" % key)
            try:
        ...
        def create(self, key, subkey):
            ...
            if not hkey:
                raise RegistryError("could not find registry key for %s" % key)
            try:
                self._hkey = wreg.CreateKeyEx(hkey, subkey, 0,
                                              wreg.KEY_ALL_ACCESS)
            try:
                self._hkey = wreg.CreateKeyEx(hkey, subkey, 0,
                                              wreg.KEY_ALL_ACCESS)
        ...
        def openkey(self, index):
            ...
            # NOTE: The index is starting from 1,not zero!
                ...
                r.setkey(
                    wreg.OpenKey(self._hkey, subkey, 0, wreg.KEY_ALL_ACCESS))
            if subkey:

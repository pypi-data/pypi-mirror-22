        ...
        for block in scope.traverse_blocks():
            ...
            for phi in phis:
                if len(phi.defblks) == len(block.preds):
                    ...
                    for idx, defblk in enumerate(phi.defblks):
                    ...
                    if len(phi.args) == 1:
                        ...
                        replace_item(block.stms, phi, mv)
...
class PathExpTracer(object):
    ...
    def traverse_dtree_(self, blk):
        ...
        if jump.is_a(JUMP):
            ...
            if blk.path_exp:
                ...
                #assert len(jump.true.preds) == 1
                #assert len(jump.false.preds) == 1
    ...
    def _reconstruct_phi(self, scope):
        for block in scope.traverse_blocks():
            phis = block.collect_stms(PHIBase)
            phi_predicates = [pred.path_exp if pred.path_exp else CONST(1) for pred in block.preds]
            for phi in phis:
                for idx, defblk in enumerate(phi.defblks):
                    if defblk in block.preds:
                        continue

...
class SparkBot(Flask):
    ...
    def __init__(self, spark_bot_name, spark_bot_token=None,
        ...
        # The list of commands the bots listens for
        # Each key in the dictionary is a command
        # The value is the help message sent for the command


        # these will be overriden by most implementations
        self.commands = {
        "/echo": {"help": "Reply back with the same message sent.", "callback": self.send_echo},
    ...
    # to the bots from other web sites.  "CORS" Requests
    ...
    def process_webhook(self):
        ...
        self.process_incoming_message(post_data)
        return ""
    ...
    #@self.app.route("/hello/<email>", methods=["GET"])
    ...
    # Function to Setup the WebHook for the bots
        ...
        try:
            for h in webhooks:  # Efficiently iterates through returned objects
                if h.name == name:
                    ...
                    wh = self.spark.webhooks.update(webhookId=h.id, name=name, targetUrl=targeturl)
                    # Stop searching
                    break
            # If there wasn't a Webhook found
            if wh is None:
                sys.stderr.write("Creating new webhook.\n")
                wh = self.spark.webhooks.create(name=name, targetUrl=targeturl, resource="messages", event="created")
        except:
            sys.stderr.write("Creating new webhook.\n")
            wh = self.spark.webhooks.create(name=name, targetUrl=targeturl, resource="messages", event="created")

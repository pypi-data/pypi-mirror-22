 ...
 * Copyright (c) 2017 S-Cube, tim.lin@sub-salt.com
...
class VTRModel(object):
    ...
    def __init__(self, file_name, read_model=True):
        """Initializes a VTR model with an existing vtr file

        :param file_name: file name of VTR file to read
        :type file_name: str
        :param read_model: whether to read the actual model properties, defaults to True
        ...
        if self._file_exists():
            ...
            # TODO implement an "empty" VTR Model
            # for now, throw exception if file doesn't exist
    ...
    def _parse_metadata(self):
        ...
        self.nx1 = header[6]  # vtr files always has first dimension (nz)
        if (self.num_dimensions == 3):
            self.nx2 = header[7]  # ny
            self.nx3 = header[8]  # nx
            self.shape = (self.nx3, self.nx2, self.nx1)
            self.shape_fortran = (self.nx1, self.nx2, self.nx3)
        elif (self.num_dimensions == 2):
            self.nx2 = 1
            self.nx3 = header[7]
            self.shape = (self.nx3, self.nx1)
            self.shape_fortran = (self.nx1, self.nx3)
        elif (self.num_dimensions == 2):
            self.nx2 = 1
            self.nx3 = 1
            self.shape = (self.nx1,)
            self.shape_fortran = (self.nx1,)
        else:
    ...
    def _parse_model(self):
        ...
        values_in_fastDim = self.num_properties * self.nx1
        if (self.num_dimensions == 3):
            model = model[10:]  # would be 8 for 1D models, 10 for 3D models
            model = model.reshape((self.nx3, self.nx2, values_in_fastDim + 2))
            model = model[:, :, 1:(values_in_fastDim + 1)]
            model = model.reshape((self.nx3, self.nx2, self.nx1, self.num_properties))
            ...
            model = model.reshape((self.nx3, values_in_fastDim + 2))
            model = model[:, 1:(values_in_fastDim + 1)]
            model = model.reshape((self.nx3, self.nx1, self.num_properties))
            for prop_index in range(0, self.num_properties):
                self.arrays[prop_index] = numpy.squeeze(model[:, :, prop_index])
            ...
            model = model[1:(values_in_fastDim + 1)]
            model = model.reshape((self.nx1, self.num_properties))
            for prop_index in range(0, self.num_properties):
                self.arrays[prop_index] = numpy.squeeze(model[:, prop_index])

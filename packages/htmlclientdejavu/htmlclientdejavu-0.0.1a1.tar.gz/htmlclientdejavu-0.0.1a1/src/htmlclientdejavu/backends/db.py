...
from htmlclientdejavu import SessionKey
...
class SessionStore(SessionBase):
    ...
    def Session(self):
        if self._request_session is None:
            self._request_session = (
                self.modelchemy.hinted_session(self.Table)
            )
    ...
    def Entity(self):
        if self._entity is None:
            self._entity = (
                dbs.router.resolve(SessionKey).default().Entity
            )
    ...
    def Table(self):
        if self._table is None:
            self._table = (
                dbs.router.retrieve(SessionKey).default().table
            )
    ...
    def clear_expired(cls):
        ...
        t_cookie_sessions = dbs.router.retrieve(SessionKey).default()

        db_session = dbs.guesser.hinted_session(t_cookie_sessions)

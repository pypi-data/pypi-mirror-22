...
try:
    ...
    print("ujson not found... Working with JSON")
    import json
...
def search_news(**kwargs):
    ...
    try:
        ...
        if query is not None:
            ...
            query = query_string.replace(" ", "+").replace('"', '%22')
        else:
        ...
        url = "https://www.google.{}/search?hl={}&as_q={}&tbm=nws&tbs=qdr:d".format(address, lang, query)
        open_url = rq.get(url)
        ...
        for link in html.find_all('a'):
            ...
            if 'play.google' in final_url or 'google.com' in final_url or 'youtube.com' in final_url or 'mail.google' in final_url or 'drive.google' in final_url or 'google.es' in final_url or '/preferences' in final_url or 'accounts.google.com' in final_url or '/search' in final_url or '/webhp' in final_url or 'maps.google' in final_url or '/support' in final_url or '/tools' in final_url or final_url == '/' or '/intl' in final_url or '/services' in final_url or '/advanced' in final_url or '/language' in final_url or 'abogacia.es' in final_url or '#' in final_url:
                final_url = None
...
def search(**kwargs):
    ...
    try:
        ...
        url = "http://www.google.{}/search?hl={}&q={}".format(address, lang, query)
        open_url = rq.get(url)
        data = open_url.content
        html = BeautifulSoup(data, "lxml")
        html.prettify()

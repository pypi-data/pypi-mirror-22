...
"""
from zbsmsa.utils.utilities import load_selectors, get_range_from_results, concat_selectors
from zbsmsa.utils.exceptions import ItemAddError, ItemFindError, SelectionError
...
class Stock:
    ...
    def stock_tab(self):
        ...
        :return: 
        """
        self.site.wait.clickable(self.stock_page_selectors["stStockTabButton"])
    ...
    def stock_tab_search(self):
        ...
        :return: 
        """
        self.site.wait.clickable(self.stock_page_selectors["stStockTabSearchButton"])
    ...
    def stock_tab_mutate(self):
        ...
        :return: 
        """
        self.site.wait.clickable(self.mutate_selectors["stMutateButton"])
    ...
    def stock_tab_product_chooser(self):
        ...
        """
        self.stock_tab()
        self.site.wait.clickable(self.product_chooser_selectors["prodchooserSelect"])

    def mutate_stock(self, note, qty="1"):
        """
        Mutates stock that is selected from different
        inventory types
        ...
        # TODO: Figure out how to properly catch error messages!
        # TODO: Messages to catch: 'Some stock is already of the selected inventory type',
        # TODO:                    'All stock must be available'
        ...
        self.site.wait._for(1)

        # Go back a page to get ride of prompt
        self.site.driver.execute_script("window.history.go(-1)")

        # We will try to click cancel if we are given an error that does not
        # close the mutation window. If we are successful, hooray! if not then
        # there was no message and we go back to the stock page and
        # continue with our lives
        try:
            self.site.wait._for(1)
            self.site.wait.clickable(self.mutate_selectors["stMutateCancelButton"], time_out=2)
            return True
    ...
    def iterate_search_table(self):
        ...
        results = self.site.get_inner_html(self.stock_page_selectors["stStockTabResults"])

        if results.lower() == "no records":
        ...
        for tr in trs:
            ...
            for element in td:
                ...
                try:
                    if not info.__contains__(element.text):
            ...
            data.append({'row': row, 'tr': tr, 'td': td, 'text': '|'.join(info)})
...
class ProductChooser():
    ...
    def iterate_search_table(self):
        ...
        for tr in trs:
            ...
            for element in td:
                # for each cell in the row, attempt to get its text
                try: info.append(element.text)
                # otherwise, set the text to none
                except Exception as error: info.append('None')

...
class _LiticConnection:
    ...
    def post_service_info(self):
        urlpath = self.serviceurl + "/info"
        response = self.session.post_json(urlpath)
        return response
    ...
    def post_job_status(self, jobid):
        ...
        response = self.session.post_json(url, {"job_id": jobid})
        if DEBUG_LEVEL > 4:
...
class LiticService:
    ...
    def __init__(self, serviceurl, apikey, input_to_disc=False):
        ...
        self.info = self.connection.post_service_info()

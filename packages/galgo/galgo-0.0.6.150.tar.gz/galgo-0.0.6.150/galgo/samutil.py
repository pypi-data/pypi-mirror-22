...
def _get_read_bases(rec):
    ...
    >>> _get_read_bases(attrdict(seq='AATCAGTA', cigar=Cigar.parse('2I3M3D3M').values))
    ['T', 'C', 'A', '-', '-', '-', 'G', 'T', 'A']
    >>> _get_read_bases(attrdict(seq='AATCAGTACC', cigar=Cigar.parse('3M2I3M2S').values))
    ['A', 'A', 'TCA', 'G', 'T', 'A']
    '''
    seq = rec.seq
    if seq is None:

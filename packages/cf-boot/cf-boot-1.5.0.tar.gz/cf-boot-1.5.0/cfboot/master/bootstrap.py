...
TOP = pkg_resources.resource_filename(__name__, "..")
...
def get_template_var ( atom ):
    ...
    if atom and isinstance(atom, basestring) and atom[0] == "$":
        return atom[1:]
...
def run_jobs ( jobs, free_variables ):
    ...
    for (i, job) in enumerate(jobs):
        assert_required_fields(job, job_required_fields)
        script = job["script"]
...
def main (  ):
    ...
    if args["free_vars"]:
        json.dump(list(free_vars(jobs)), sys.stdout, indent = 2)
        sys.exit(0)
        ...
        job_order = args["sort_vars_by_job_order"]
        json.dump(accumulated_vars, sys.stdout, indent=2,
                  sort_keys = not job_order)
        with open( args["output"], "w") as fh:
            json.dump(accumulated_vars, fh, indent=2)

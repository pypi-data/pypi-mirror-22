...
if input_json.get("if_exists_delete"):
    cf_home = input_json.get("cf_home")
    # TODO need to delete service bindings first. call cf-delete-service subscript
    print ("deleting {} if exists...".format(instance_name), file = sys.stderr )
    child_env["CF_HOME"] = cf_home
    subprocess.Popen(["cf", "delete-service", "-f", instance_name],
                     env = child_env, stdout = sys.stderr).wait()
    print ("done with deletion...".format(), file = sys.stderr )

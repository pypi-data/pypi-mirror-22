...
class DataObj(object):
    ...
    def upload_data(self):
        ...
        source_files = glob('/Users/luke/GitProjects/WaveGliDA/test_data/STS_sensor/txt/*.txt')
        for source_name in source_files:
            dest_name = "./app/data/{}".format(os.path.split(source_name)[1])
            copyfile(source_name, dest_name)
        file_list = glob('./app/data/*.txt')
        df = read_wg_flist(file_list)
        self.df = calcs(df)
        self.number_of_files = "{} files uploaded".format(len(file_list))

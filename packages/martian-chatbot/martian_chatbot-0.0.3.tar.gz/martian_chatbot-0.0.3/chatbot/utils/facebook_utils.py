...
class FacebookUtils(object):
        ...
        if response.response_type == 'text':
            ...
            for button in response.message['buttons']:
                if button['type'] == 'web_url':
                    b = {"type": button['type'], "title": button['title'], "url": button['url'], "webview_height_ratio": button['webview_height_ratio']}
                    buttons.append(b)
                else:
                    ...
                    if messaging.get('message'):
                        if messaging['message'].get('text'):
                            ...
                            if messaging['message'].get('quick_reply'):
                                ...
                                return FacebookMessage(sender, message, message_type, image, location)
                            else:
    ...
    def typing_on(self, user_id):
        ...
        resp = requests.post("https://graph.facebook.com/v2.6/me/messages?access_token=" + self.fbaccesstoken,
                             json=data)
        print(self.CLASS_NAME + resp.content)
    ...
    def whitelist(self, whitelist):
        resp = requests.post(
            "https://graph.facebook.com/v2.6/me/thread_settings?access_token=" + self.fbaccesstoken,
            json=whitelist)
        print(self.CLASS_NAME + resp.content)
    ...
    def get_started_button(self, getstarted):
        resp = requests.post(
            "https://graph.facebook.com/v2.6/me/thread_settings?access_token=" + self.fbaccesstoken, json=getstarted)
        print(self.CLASS_NAME + resp.content)

    def setup_persistant_menu(self, menu):
        resp = requests.post(
            "https://graph.facebook.com/v2.6/me/thread_settings?access_token=" + self.fbaccesstoken, json=menu)
        print(self.CLASS_NAME + resp.content)
    ...
    def upload_image(self, image):
        ...
        resp = requests.post("https://graph.facebook.com/v2.6/me/message_attachments?access_token=" + self.fbaccesstoken,
                             json=response)
        print(self.CLASS_NAME + resp.content)
        return resp

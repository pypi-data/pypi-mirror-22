...
from chatbot.utils.chatterbot_utils import ChatterbotUtils
...
class ContentGenerator(object):
    ...
    def __init__(self, config):
        ...
        self.chatterbot_utils = ChatterbotUtils(config.chatterbot)
        self.fb_utils = FacebookUtils(config.facebook['verification_token'])
    ...
    def handle_input(self, message):
        ...
        if message.message_type == 'USER_TEXT':
            if self.db_utils.is_input_expected(message.sender):
                ...
                val = self.chatterbot_utils.generate_chatterbot_response(message.message)
                res = dict()
                try:
                    res = simplejson.loads(str(val))
                except simplejson.decoder.JSONDecodeError:
                    res['data'] = str(val)
                    res['type'] = "text_response"
                response = ResponseMessage(message.sender, message.message_service(), res['data'], res['type'], None)
                self.run_method(res['type'], response)
        else:
    ...
    def send_response(self, response, time):
        print("sender = {0}".format(response.sender))
        if response.service == 'facebook':
            self.fb_utils.typing_on(response.sender)
            sleep(time)
    ...
    def create_button(self, type, text, payload, url=None, wv_height=None, extensions=None, fallback=None):
        button = dict()
        if type == 'text' or type == 'postback':
            button['type'] = type
            button['title'] = text
    ...
    def create_default_action(self, type, title, url, extensions=None, wv_height=None, fallback=None):
        action = dict()
        action['type'] = type
        action['title'] = title
        action['url'] = url

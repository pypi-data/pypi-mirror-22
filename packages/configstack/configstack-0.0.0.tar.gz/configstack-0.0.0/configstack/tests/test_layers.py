...
class ConfigLayer_BaseTestCase(unittest.TestCase):
    """ Base for test cases for `ConfigLayer` and derived classes. """
    ...
    def test_has_default_source_instance_of_configsource_class(self):
        """ Should default to `source` instance of `configsource_class`. """
        del self.test_configlayer_args['source']
        with unittest.mock.patch.object(
                self.configlayer_class, 'configsource_class',
                new=FakeConfigSource):
            test_configlayer_instance = self.configlayer_class(
                    **self.test_configlayer_args)
        self.assertIsInstance(
                test_configlayer_instance.source, FakeConfigSource)
...
class ConfigLayer_TestCase(ConfigLayer_BaseTestCase):
    ...
    def test_raises_error_when_configsource_class_not_implemented(self):
        """ Should have `source` value NotImplemented. """
        del self.test_configlayer_args['source']
        test_configlayer_instance = self.configlayer_class(
                **self.test_configlayer_args)
        self.assertIs(test_configlayer_instance.source, NotImplemented)
...

class DefaultConfigLayer_TestCase(ConfigLayer_BaseTestCase):
    """ Test cases for class `DefaultConfigLayer`. """

    configlayer_class = layers.DefaultConfigLayer
    configsource_class = configlayer_class.configsource_class

    def setup_test_configsource_args(self):
        """ Set up the test configuration source to use. """
        self.test_configsections = {
    ...
    def test_has_default_name(self):
        """ Should have name matching `default_name`. """
        del self.test_configlayer_args['name']
        self.test_configlayer_instance = self.configlayer_class(
                **self.test_configlayer_args)
        expected_name = self.configlayer_class.default_name
        self.assertEqual(self.test_configlayer_instance.name, expected_name)


class EnvironmentConfigLayer_TestCase(ConfigLayer_BaseTestCase):
    """ Test cases for class `EnvironmentConfigLayer`. """

    configlayer_class = layers.EnvironmentConfigLayer
    configsource_class = configlayer_class.configsource_class
...

class ConfigparserConfigLayer_TestCase(ConfigLayer_BaseTestCase):
    """ Test cases for class `ConfigparserConfigLayer`. """

    configlayer_class = layers.ConfigparserConfigLayer
    configsource_class = configlayer_class.configsource_class
...

class FakeConfigLayer(layers.ConfigLayer):
    """ Fake subclass of `ConfigLayer`. """

    configsource_class = FakeConfigSource


class Stack_TestCase(
    ...
    def setup_fake_layers(self):
        ...
        for layer_params in self.layers_params:
            layer = FakeConfigLayer(name=layer_params['name'])

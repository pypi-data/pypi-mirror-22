                ...
                if not isExecuting:
                    if self.lastJobTime == -1:
                        self.lastJobTime = time.time()                    
                            ...
                            self._print('clear exemgr(%s) %d threads'%(self.name,self.curCnt))
                            self.curCnt = 0
                            del self.thredList[:]
            time.sleep(0.1)
...
class Test():
    ...
    def test(self,):
        startTime = time.time()
        t = ExcutorMgr(4,"mgr",logable= True)
        
        t.execute(Job(self.test2,2,"job1xx"))
        t.execute(Job(self.test2,2,"job2xx"))
        
        t.execute(Job(self.test2,2,"job3xx"))
        t.execute(Job(self.test2,2,"job4xx"))
        t.execute(Job(self.test2,2,"job5xx"))

...
class siMeshElt:
  ...
  def feval(self,fun,**kwargs):
    ...
    assert np.isscalar(fun) or isinstance(fun, type(lambda: None)), "1st argument must be a scalar, a def function or a lambda function. Given a %s"%str(fun)
    if isinstance(fun, type(lambda: None)): # fun is a 'def' function or a 'lambda' function
      nargin=fun.__code__.co_argcount

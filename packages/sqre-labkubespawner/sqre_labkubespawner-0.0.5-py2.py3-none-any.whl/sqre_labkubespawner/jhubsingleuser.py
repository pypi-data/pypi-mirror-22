...
from .auth import HubOAuth, HubOAuthenticated, HubOAuthCallbackHandler
...
class JupyterHubLoginHandler(LoginHandler):
    ...
    def is_token_authenticated(handler):
        ...
        if getattr(handler, '_cached_hub_user', None) is None:
            # ensure get_user has been called, so we know if we're
            # token-authenticated
    ...
    def get_user(handler):
        ...
        # patch in HubAuthenticated class for querying the Hub for cookie
        # authentication
...
class SingleUserNotebookApp(NotebookApp):
    ...
    def init_webapp(self):
        ...
        s['csp_report_uri'] = self.hub_host + \
            url_path_join(self.hub_prefix, 'security/csp-report')
    ...
    def patch_templates(self):
        ...
        self.jinja_template_vars[
            'logo_url'] = self.hub_host + url_path_join(self.hub_prefix, 'logo')

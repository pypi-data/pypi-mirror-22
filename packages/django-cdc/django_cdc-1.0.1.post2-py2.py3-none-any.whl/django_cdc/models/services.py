...
class CommonUtils(object):
    def get_function_name(self, table_name):
        function_name = "{0}{1}{2}".format(settings.LAMBDA_FUNCTION_PREFIX, "-", table_name)
...
class lambda_service(object):
    ...
    def put_data_entry(self, name, *args, **kwargs):
        ...
        try:
            ...
            logger.error("Error Occured while invoking lambda function %s" % str(e))

    def get_serverless_func(self, lambda_name):
        function_val = {'name': lambda_name, 'handler': 'handler.push_data_to_kinesis',
...
class kinesis_service(object):
    ...
    def put_data_entry(self, name, *args, **kwargs):
        try:
            ...
            records = []
            for package in args:
                record = {'Data': json.dumps(package, cls=PythonObjectEncoder),
    ...
    def put_data_entry(self, name, *args, **kwargs):
        try:
            ...
            sns_client.publish(TargetArn=arn,
                               Message=json.dumps({'default': json.dumps(args, cls=PythonObjectEncoder), }),

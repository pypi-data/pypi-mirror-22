...
class DjangoCDCManager(models.Manager):
    ...
    def __init__(self, model, attname,services=[], exclude=[], foreign_keys={}, instance=None):
    ...
    def create_data_entry(self, instance,action_type,updates=None,
        ...
        if isinstance(instance, list):
            ...
            if getattr(instance,self.attname).is_tracking_enabled():
                ...
                logger.info("Tracking is disabled")
        if myList and self.services:
            for service in self.services:
                obj=Service.factory(service)
                obj.put_data_entry(name, *myList)
        elif myList:
            logger.info("Payload Data"+ myList)
...
class DjangoCDC(object):
    ...
    def __init__(self, exclude=[],services=[],foreign_keys={}):
        self._exclude = exclude
        self.foreign_keys = foreign_keys
        self.services=services

    def contribute_to_class(self, cls, name):
        self.manager_class(cls, name,self.services, self._exclude,
                           self.foreign_keys
    ...
    def finalize(self, sender, **kwargs):
        ...
        descriptor = DjangoCDCDescriptor(
            ...
            self._exclude,self.foreign_keys,self.services)

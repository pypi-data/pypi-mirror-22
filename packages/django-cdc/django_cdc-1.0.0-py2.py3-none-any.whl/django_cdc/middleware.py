...
from django_cdc import registration, settings
from django_cdc.models import fields
from django_cdc.models.managers import DjangoCDCManager

logger = logging.getLogger(__name__)


def _disable_django_cdc_managers(instance):
    for attr in dir(instance):
        try:
            if isinstance(getattr(instance, attr), DjangoCDCManager):
                getattr(instance, attr).disable_tracking()
        except AttributeError:
            pass


def _enable_django_cdc_managers(instance):
    for attr in dir(instance):
        try:
            if isinstance(getattr(instance, attr), DjangoCDCManager):
    ...
    def _update_post_save_info(self, user, session, sender, instance, created, **kwargs ):
        if created:
            ...
            if sender in registry:
                for field in registry.get_fields(sender):
                    setattr(instance, field.name, user)
                    _disable_django_cdc_managers(instance)
                    #instance.save()
                    _enable_django_cdc_managers(instance)

...
MATRICES = dict(
    PAM30 = {'*':1, 'A': 6, 'B': 6, 'C': 10, 'D': 8, 'E': 8, 'F': 9, 'G': 6, 'H': 9, 'I': 8, 'J': 6,
             'K': 7, 'L': 7, 'M': 11,'N': 8, 'P': 8, 'Q': 8, 'R': 8, 'S': 6, 'T': 7, 'V': 7, 'W': 13,
             'X': -1, 'Y': 10, 'Z': 6},
    BLOSSUM62 = {'*':1, 'A':4, 'B':4, 'C':9, 'D':6, 'E':5, 'F':6, 'G':6, 'H':8, 'I':4, 'K':5, 'L':4,
                 'M':5, 'N':6, 'P':7, 'Q':5, 'R':5, 'S':4, 'T':5, 'V':4, 'W':11, 'X':-1, 'Y':7, 'Z':4, }
)
...
class BSR_RecordSet:
    ...
    Attributes:
        ...
        matrix (str): Name of the scoring matrix used in the BLAST (PAM30 or BLOSUM62)
    All attributes are set by get_bsr()
    """

    def __init__(self, job_title, bsr_records, ref_fasta, comparison_fastas,
                 strain_names, files, matrix_name):
...
def get_initial_BSR_Records_with_ref_scores(ref_fasta, strain_name, matrix):
    """
...
def get_bsr(work_path, job_name, reference_fasta, comparison_fastas,
            strain_names, force_redo = False, matrix = 'PAM30'):
    ...
    bsr_records = _do_next_step_pickle(
        ...
        (reference_fasta, strain_names[0], matrix), # Don't forget to pass *args as tuple
    ...
    final_results = BSR_RecordSet(job_name, bsr_records, reference_fasta, comparison_fastas,
                                  strain_names, created_files, matrix)

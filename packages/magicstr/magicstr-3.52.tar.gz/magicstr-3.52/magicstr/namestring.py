    ...
    def merge_manager(self, other_manager):
        ...
        for key in other_ns.keys():
            ...
            if(self.namestring.get(key) == None):
                ...
                current_platform = self.namestring[key]['attrib'].get('platform', None)
                if(platform == None or current_platform == None):
                    print '[-] Unexpected error: fail to get platform of key "%s" when merging data.' % key
                    exit()
                ...
                # platform doesn't exist, check whether all the values are the same to insert platform.
                    ...
                    for file_id in other_ns[key]['content'].keys():
                        ...
                        current_value = self.namestring[key]['content'].get(file_id)

...
- For every table that has PatientId, make a view mapping it to an integer.
- For every table that has CaseNumber, make a view mapping it to an integer.
- If a table is not PatientDetails (PK = PatientId) or CasesAll (PK = 
  CaseNumber), give it an AUTONUMBER integer PK (e.g. "crate_pk").
...
- Use that to hash the CaseNumber
  ... and make it the same key as the RID, since it's often the same number.
...
def process_master_patient_table(table: Table,
                                 ...
                                 progargs: Any) -> None:
    """
    Adds CRATE_COL_PT_NUMBER and CRATE_COL_NHS_NUMBER columns.
    """
...
def process_patient_table(table: Table, engine: Engine, progargs: Any) -> None:
    """
    For a patient table that is not the 
    Args:
        table: 
        engine: 
        progargs: 
...
def process_table(table: Table, engine: Engine, progargs: Any) -> None:
    ...
    is_patient_table = PCMIS_COL_PATIENT_ID in column_names
    assert not (is_case_table and is_patient_table)

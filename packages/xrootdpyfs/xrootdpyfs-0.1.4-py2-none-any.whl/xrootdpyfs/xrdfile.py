...
class XRootDPyFile(object):
    ...
    * ``a+`` - Open the file for reading and writing; create the file
    ...
    def read(self, sizehint=-1):
        ...
        If no ``sizehint`` is provided the entire file is read (up tp 4GiB)!
        Multiple calls to this method after EOF as been reached, will return
        an empty string.

        :param sizehint: Number of bytes to read from file object. Maximum 4GiB
            (i.e unsigned 32-bit integer).
        """
        if self.closed:
        ...
        chunksize = sizehint if sizehint > 0 else min(
            max(self.size - self._ipp, 0),
            2**32 - 1)

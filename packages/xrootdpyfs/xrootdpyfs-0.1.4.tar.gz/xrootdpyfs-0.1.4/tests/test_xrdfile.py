...
def test_seek_and_read(tmppath):
    """Test seek and the read."""
    fd = get_tsta_file(tmppath)
    xfile = XRootDPyFile(mkurl(fd['full_path']))
    xfile.seek(xfile.size - 2)
    assert xfile.read() == fd['contents'][-2:]

    xfile.seek(xfile.size - 2)
    pytest.raises(OverflowError, xfile.read, sizehint=2 ** 32)

    # Mock file size and try to read more than 4GB
    xfile.seek(xfile.size - 2)
    xfile._size = 2 * (2 ** 32)
    xfile.read()

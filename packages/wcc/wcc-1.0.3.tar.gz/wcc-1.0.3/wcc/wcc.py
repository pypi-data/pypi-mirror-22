...
class wcc:
    def __init__(self):
        self.apimap = {}
        oss_key_id = os.getenv('OSS_WCC_ACCESS_KEY_ID','null')
        oss_key_secret = os.getenv('OSS_WCC_ACCESS_KEY_SECRET','null')
        print(oss_key_id)
        print(oss_key_secret)
        oss_key_id = osskey.decode(oss_key_id)    
        oss_key_secret = osskey.decode(oss_key_secret)
        print(oss_key_id)
        print(oss_key_secret)
    wcc_headers = {
    ...
    def getFileInfo(file_url_original):
        ...
        for k in range(1,6):
            ...
            try:
                ...
                print("InfoxFile:"+file_url+" err("+str(k)+") "+str(err))
                file_size = 0
                mime_type = "null"
                time.sleep(1)
                if k == 5:
                    print("InfoxFile:"+file_url+" Abort")
            finally:
                pass
        ...
        try:
            ...
            for k in range(1,download_try_count+1):
                try:
                    ...
                    print("dwloadFile:"+file_url +" Fail "+str(k)+"/"+str(download_try_count))
                    print(err)
                    time.sleep(5)
    ...
    def urlget(url,headers = None):
        ...
        for k in range(1,11):
            try:
                request = requests.get(url,timeout=Cfg_TimeoutDelay,headers=HEAD)
    ...
    def AddFollows(self,user_follow_info,batch_id=0,thread_name=""):
        ...
        print("addFollows: m:%4d i:%4d x:%4d e:%2d s:%5.1f b:%s t:%s w:%4s"
    ...
    def AddUsers(self, users,batch_id=0,thread_name="N"):
        ...
        func_delay = pmc_func_t1 - pmc_func_t0
        func_time  = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        print("addArtUser: m:%4d i:%4d x:%4d e:%2d s:%5.1f b:%s t:%s w:%4s"
    ...
    def AddArticles(self, articles,batch_id=0,thread_name="N"):
        ...
        func_delay = pmc_func_t1 - pmc_func_t0
        func_time  = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        print("addArticle: m:%4d i:%4d x:%4d e:%2d s:%5.1f b:%s t:%s w:%4s"
    ...
    def existsFile(oss_bucket,file_localname):
        ...
        bucket_name = os.getenv('OSS_TEST_BUCKET',oss_bucket)

...
import json
import cgi
from .error import Http401, Http415
...
class Head(object):
    def process_request(self, req, resp):
        ...
        if 'application/json' not in req.content_type:
            Http415("application/json is required")
...
class Auth(object):
    def process_request(sef, req, resp):
        if not req.auth:
           Http401('missing authorization')
        Authorization().DeCode(req.auth)
        return

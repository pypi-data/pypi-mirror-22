...
class Base(object):
    ...
    def __init__(self, rootdir=os.curdir):
        rootdir = Path(rootdir or os.curdir).real
        self.rootdir = self.findroot(rootdir, self.directory) or rootdir
        self.current_info()
...
attribute is None, use that subclass as default."""
    ...
    @classmethod
    def findroot(cls, path, lookfor):
        if path is None:
            return path
        elif not isinstance(path, Path):
            path = Path(path)
        if lookfor is None and path.exists:
            return path
        elif (path + lookfor).exists:
            return path
        elif path == '.' or path == '' or path == '/':
            return None
        return cls.findroot(path.dirname, lookfor)

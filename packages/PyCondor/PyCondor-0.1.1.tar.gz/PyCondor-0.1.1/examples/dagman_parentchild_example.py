...
if __name__ == "__main__":
    ...
    extra_lines = ['#Isdfsdf', '#asdfasdf']
    job2 = pycondor.Job('examplejob2', 'savelist.py',
                   error=error, output=output,
                   log=log, submit=submit, extra_lines=extra_lines, verbose=2)
    # Adding arguments to job1
    ...
    dagman = pycondor.Dagman('exampledagman', submit=submit, extra_lines=extra_lines, verbose=2)
    # Add jobs to dagman
    dagman.add_job(job1)
    dagman.add_job(job2)

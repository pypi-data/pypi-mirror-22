...
from .resources import decode_resources, VALID_PARAMS
...
class BaseSystem(object):
    ...
    @abstractmethod
    def native_resources(self, target):
        return decode_resources(target)
...
class SystemSGE(BaseSystem):
    ...
    def native_resources(self, target):
        res = super(SystemSGE, self).native_resources(target)
...
class SystemLSF(BaseSystem):
    ...
    def native_resources(self, target):
        res = super(SystemLSF, self).native_resources(target)
        res_map = {
            "cpu": "-n {0}",
...
class SystemSLURM(BaseSystem):
    ...
    def native_resources(self, target):
        res = super(SystemSLURM, self).native_resources(target)

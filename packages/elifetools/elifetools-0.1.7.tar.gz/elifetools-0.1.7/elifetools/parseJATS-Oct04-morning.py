...
def render_raw_body(tag):
    ...
    for tag in body_tags:
        if tag.name == "boxed-text" and not raw_parser.title(tag) and not raw_parser.label(tag):
            ...
            body_content.append(tag_content)
    import json
    print json.dumps(body_content, indent=4)
    return body_content
...
def body_block_content_render(tag):
    ...
    p_content_omit = False
    if tag.name == "p":
        if "content" in body_block_paragraph_render(tag):
            # Complicated paragraph, only add its content, not itself
            p_content_omit = True
            for content_item in body_block_paragraph_render(tag)["content"]:
                #pass
                if content_item["type"] != "paragraph":
                    tag_content[content_item["type"]] = content_item
        else:
            # No content inside the paragraph, just add itself
            tag_content = body_block_paragraph_render(tag)
    ...
    for child_tag in tag:
        ...
        if child_tag.name == "p":
            if body_block_paragraph_render(child_tag) != {}:
                #tag_content_content.append(body_block_paragraph_render(child_tag))
                
                if "content" in body_block_paragraph_render(child_tag):
                    # Complicated paragraph, only add its content, not itself
                    for content_item in body_block_paragraph_render(child_tag)["content"]:
                        tag_content_content.append(content_item)
                else:
                    # No content inside the paragraph, just add itself
                    tag_content_content.append(body_block_paragraph_render(child_tag))
    ...
    if len(tag_content_content) > 0 and p_content_omit is False:
        tag_content["content"] = tag_content_content
    #import json
    #print json.dumps(tag_content, indent=4)
    return tag_content
...
def body_block_paragraph_render(p_tag):
    ...
    tag_content = body_block_content(p_tag)
    tag_content_content = []
    nodenames = body_block_nodenames()
    print "paragraph render"
    if len(p_tag.find_all(nodenames)) > 0:
    
        paragraph_content = u''
        for child_tag in p_tag:
            print "p child tag", child_tag.name
        ...
        if paragraph_content != '':
            tag_content_content.append(body_block_paragraph_content(paragraph_content))
        #else:
        #    tag_content_content.append(body_block_content_render(p_tag))

    if len(tag_content_content) > 0:
        tag_content["content"] = tag_content_content
    #tag_content = tag_content_content
    return tag_content

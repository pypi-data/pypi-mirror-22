...
def references_json(soup):
    ...
    for ref in refs(soup):
        ...
        if ref.get("authors"):
            ...
            for ref_author in ref.get("authors"):
                author_json = OrderedDict()
                author_json["type"] = "person"
                author_name = OrderedDict()
                ...
                author_name["index"] = author_index_name(ref_author.get("surname"),
                                                         ...
                                                         ref_author.get("suffix"))
                author_json["name"] = author_name
                author_group_type = ref_author.get("group-type")
            ...
            for key, value in all_authors.iteritems():
                if len(value) > 0:
                    # TODO fix for multiple types of person records
                    author_key = key + "s"
                    ref_content[author_key] = []
                    for person in value:
                        ref_content[author_key].append(person)
...
"""
        ...
        fake_author = OrderedDict()
        fake_author["type"] = "person"
        fake_author["name"] = OrderedDict()
        fake_author["name"]["preferred"] = "Person One"
        fake_author["name"]["index"] = "One, Person"
        ref_content["authors"] = []
        ref_content["authors"].append(fake_author)

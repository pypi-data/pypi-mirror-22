...
class Worker(AbstractWorker):
    ...
    def __getitem__(self, item):
        return self._subs.get(item)

    def _try_sub(self, key):
        sub = self[key]
        if sub is not None:
            return sub
        else:
            return self.context[self.config.get(key)]

    @property
    def input(self):
        return self._try_sub('input')

    @input.setter
    def input(self, value):
        self._set_sub('input', value)

    @property
    def output(self):
        return self._try_sub('output')

    @output.setter
    def output(self, value):
        self._set_sub('output', value)

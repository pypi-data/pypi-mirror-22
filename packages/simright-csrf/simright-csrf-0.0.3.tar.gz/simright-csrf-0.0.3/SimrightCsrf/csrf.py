...
def generate_csrf(secret_key=None, token_key=None):
    ...
    secret_key = _get_config(
        secret_key, 'CSRF_SECRET_KEY', current_app.secret_key,
        message='A secret key is required to use CSRF.'
    )
    field_name = _get_config(
        token_key, 'CSRF_FIELD_NAME', 'csrf_token',
    ...
    s = URLSafeTimedSerializer(secret_key, salt='simright-csrf-token')

    return s.dumps(session[field_name])
...
def validate_csrf(data, secret_key=None, time_limit=None, token_key=None):
    ...
    :raises ValidationCsrfError: Contains the reason that validation failed.
    ...
    time_limit = _get_config(
        time_limit, 'CSRF_TIME_LIMIT', 3600, required=False
    )

    if not data:
        raise ValidationCsrfError('The CSRF token is missing.')

    if field_name not in session:
        raise ValidationCsrfError('The CSRF session token is missing.')
    ...
    try:
        ...
        raise ValidationCsrfError('The CSRF token has expired.')
    except BadData:
        raise ValidationCsrfError('The CSRF token is invalid.')

    if not safe_str_cmp(session[field_name], token):
        raise ValidationCsrfError('The CSRF tokens do not match.')
...
class ValidationCsrfError(ValueError):
    """
    Raised when a validator fails to csrf its input.
    """
...
#             )
#         except ValidationCsrfError as e:
...
class CSRFProtect(object):
    ...
    header sent with JavaScript requests. 
    ...
    def init_app(self, app):
        ...
        generate_csrf.methods = ['GET']
        app.add_url_rule('/csrf_token', generate_csrf)
    ...
    def protect(self):
        if request.method not in current_app.config['CSRF_METHODS']:
            return
        ...
        if request.is_secure and current_app.config['CSRF_SSL_STRICT']:
